...
// Which stores and package managers carry the app, and whether each is
// live yet, plus the signing and provenance facts the verify section shows.
const CHANNELS = require('../config/mail-app-channels');

const logger = require('#helpers/logger');

// Repository the desktop and mobile apps are released from.
// <https://github.com/forwardemail/mail.forwardemail.net>
const { REPO_URL } = CHANNELS;
...
const KNOWN_PLATFORMS = new Set(GROUPS.flatMap((group) => group.platforms));

// Checked at load rather than at render, so a typo in the channel config
// fails the boot instead of quietly rendering a channel under no platform.
for (const channel of CHANNELS.channels) {
  if (!KNOWN_PLATFORMS.has(channel.platform)) {
    throw new TypeError(
      `Channel "${channel.key}" names unknown platform "${channel.platform}"`
    );
  }

  if (!CHANNEL_STATES.has(channel.state)) {
    throw new TypeError(
      `Channel "${channel.key}" has unknown state "${channel.state}"`
    );
  }
}

/**
 * The channels a platform's card lists, in config order. "na" channels are
 * recorded in the config but never rendered.
 *
...
function getChannels(platform) {
  return CHANNELS.channels.filter(
    (channel) => channel.platform === platform && channel.state !== 'na'
  );
}
...
function getAppDownloads(release) {
    ...
    platforms: group.platforms
      .map((platform) => {
        ...
        return {
          ...
          channels: getChannels(platform),
          // False only while the config says this platform's installers are
          // not code-signed yet; the card carries the SmartScreen note then.
          signed: CHANNELS.signing[platform] !== false
        };
      })
  ...
  // The self-hosted F-Droid repository is both an Android channel and a
  // verification item (its index is signed with its own key), so the verify
  // section reads it from here rather than digging through the platforms.
  const fdroid = CHANNELS.channels.find(
    (channel) => channel.key === 'fdroid-repo' && channel.state !== 'na'
  );
  ...
  return {
    ...
    verify: {
      minisign: CHANNELS.verify.minisign,
      provenance: CHANNELS.verify.provenance,
      fdroid: fdroid || null
    },
    groups,
