...
function createNote(obj) {
  ...
  note.pushType = 'alert';
  note.topic = certs.Calendar.topic;
...
// Send APN for Calendar (CalDAV) changes
...
async function sendApnCalendar(client, id) {
  ...
  if (
    ...
    // Extract topic from Calendar certificate (similar to Mail topic extraction)
    // The topic is the UID from the certificate subject
    if (!certs.Calendar.topic) {
      const cert = new crypto.X509Certificate(certs.Calendar.certificate);
      certs.Calendar.topic = splitLines(cert.subject)[0]
        .split('UID=')[1]
        .trim();
    }
    ...
    provider = new apn.Provider({
      logger,
      cert: certs.Calendar.certificate,
      key: certs.Calendar.privateKey,
  ...
  await pMap(alias.aps, async (obj) => {
    try {
      ...
      //
      const key = `aps_calendar_check:${revHash(obj.account_id)}:${revHash(
        obj.device_token
      )}`;
      ...
      if (Array.isArray(result.failed) && result.failed.length > 0) {
        ...
        if (unregisteredDeviceTokens.length === 0) {
          const err = new TypeError('APS Calendar failed');
          err.isCodeBug = true;
          err.result = result;
          logger.fatal(err);
          return;
...
module.exports = sendApnCalendar;
