...
//
// Backstop for rekeys the sqlite-worker did not settle (run every few
// minutes by jobs/cleanup-stuck-rekeys.js).
//
...
// interrupted jobs from the reliable queue.  The job runs on the bree host,
...
async function alertAdmins(client, alias) {
...
//
// Settles the rekeys that are certainly lost and alerts about the ones only
// the worker can settle.  Resolves with how many were rolled back and how
// many alerts were sent.
//
async function cleanupStuckRekeys(client, { now = Date.now() } = {}) {
    ...
    try {
      ...
      if (alias.rekey_swap_ino) {
        const swappedAt = alias.rekey_swapped_at
          ...
          : startedAt;
        if (
          now - swappedAt >= REKEY_PROCESSING_STALE_THRESHOLD &&
          (await alertAdmins(client, alias))
        )
          alerted++;
        continue;
      }
  ...
  return { cleared, alerted };
}

module.exports = cleanupStuckRekeys;
