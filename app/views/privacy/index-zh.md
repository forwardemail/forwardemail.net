...
**除非是[错误日志](#error-logs)、[外发 SMTP 邮件](#outbound-smtp-emails)，和/或检测到垃圾邮件或恶意活动（例如用于速率限制）时：**

* 我们不将任何转发的邮件存储到磁盘存储或数据库中。
* 我们不将任何转发邮件的元数据存储到磁盘存储或数据库中。
* 我们不将任何日志或 IP 地址存储到磁盘存储或数据库中。
* 我们不使用任何第三方分析或遥测服务。
...
* 我们存储您提供给我们的电子邮件地址。
* 我们存储您提供给我们的域名、别名和配置。
* 您自愿提供给我们的任何额外信息，例如通过电子邮件或我们的<a href="/help">帮助</a>页面提交的评论或问题。
...
* 我们为您的网站流量存储会话 Cookie。
* Cookie 为 HTTP-only，签名，并使用 SameSite 保护。
* 会话 Cookie 在 30 天无活动后过期。
* 我们不会为机器人或爬虫创建会话。
* 我们使用 Cookie 用于：
  * 认证和登录状态
  * 两步验证“记住我”功能
