...
> \[!TIP]
> アーキテクチャ、セキュリティ実装、ロードマップの技術的詳細については、[Technical Whitepaper](https://forwardemail.net/technical-whitepaper.pdf)をご覧ください、およびCure53による独立した[セキュリティ監査](https://forwardemail.net/pentest-report_forward-email.pdf)をご覧ください。
...
### 2026 - RFC準拠と高度なフィルタリング、および独立セキュリティ監査 {#2026---rfc-compliance-and-advanced-filtering} {#2026---rfc-compliance-and-advanced-filtering-2026---rfc-compliance-and-advanced-filtering}
...
**2026年5月**: 信頼性と互換性の大幅な改善が出荷されました。iOS/macOSでのリアルタイムメール配信のためのApple Push Notification（APN）サポート、iOS同期互換性のための包括的なCalDAV修正（PRODID、LAST-MODIFIED、CREATED、SEQUENCEの修復と正しいhrefエンコーディング）、エイリアスごとのメール保持ポリシー、MIMEヘッダーデコードとList-*ヘッダー保持によるSieveフィルタリングの強化、カスタムラベルとのIMAPキーワード同期、大量送信者向けのBSONオーバーフロー保護が含まれます。IMAP/POP3クライアント向けのレガシーTLSサポートが追加され、プッシュ通知APIがトークンCRUDおよび配信パイプラインエンドポイントで拡張されました。

**2026年6月**: Forward Emailは、業界で最も尊敬されるセキュリティ研究企業の一つである[Cure53](https://cure53.de/)による初の独立した第三者セキュリティ監査を公開しました。完全なペネトレーションテストレポートは[pentest-report_forward-email.pdf](/pentest-report_forward-email.pdf)で入手できます。監査はオープンソースアーキテクチャ全体とサーバーインフラストラクチャをカバーし、すべての発見事項は継続的な再テストを通じて対処・検証されました。SSRF防止のためのDNSピニング、招待トークン認証、Domain Connect URL検証を含む追加のセキュリティ強化が適用されました。
