...
* [Хронологія](#timeline)
  ...
  * [2026 - Відповідність RFC та розширене фільтрування та незалежний аудит безпеки {#2026---rfc-compliance-and-advanced-filtering}](#2026---rfc-compliance-and-advanced-filtering-2026---rfc-compliance-and-advanced-filtering)
...
> \[!TIP]
> Для технічних деталей про нашу архітектуру, реалізацію безпеки та дорожню карту дивіться [Технічний Whitepaper](https://forwardemail.net/technical-whitepaper.pdf) та наш незалежний [аудит безпеки](https://forwardemail.net/pentest-report_forward-email.pdf) від Cure53.
...
**Травень 2026**: Були випущені значні покращення надійності та сумісності, включаючи підтримку Apple Push Notification (APN) для доставки пошти в реальному часі на iOS/macOS, комплексні виправлення CalDAV для сумісності синхронізації iOS (відновлення PRODID, LAST-MODIFIED, CREATED, SEQUENCE та правильне кодування href), політики зберігання електронної пошти для кожного псевдоніма, покращена фільтрація Sieve з декодуванням заголовків MIME та збереженням заголовків List-*, синхронізація ключових слів IMAP з користувацькими мітками та захист від переповнення BSON для відправників з великим обсягом. Була додана підтримка застарілого TLS для клієнтів IMAP/POP3, а API push-сповіщень було розширено кінцевими точками CRUD токенів та конвеєра доставки.

**Червень 2026**: Forward Email опублікував свій перший незалежний аудит безпеки третьою стороною, проведений [Cure53](https://cure53.de/), однією з найбільш поважних фірм з дослідження безпеки в галузі. Повний звіт про тестування на проникнення доступний за адресою [pentest-report_forward-email.pdf](/pentest-report_forward-email.pdf). Аудит охопив всю архітектуру з відкритим вихідним кодом та серверну інфраструктуру, всі виявлені проблеми були усунені та перевірені шляхом безперервного повторного тестування. Було застосовано додаткове зміцнення безпеки, включаючи DNS pinning для запобігання SSRF, автентифікацію токеном запрошення та валідацію URL Domain Connect.
...
**100% відкритий код**: На відміну від конкурентів, які відкривають лише фронтенд, залишаючи бекенд закритим, Forward Email зробив весь свій код — і фронтенд, і бекенд — доступним для публічного перегляду на [GitHub](https://github.com/forwardemail)—тепер підкріплений [незалежним аудитом безпеки](/pentest-report_forward-email.pdf) від Cure53.
...
Платформа продовжує розвиватися з регулярними оновленнями функцій та покращеннями інфраструктури, зберігаючи позицію єдиного 100% відкритого, зашифрованого, орієнтованого на конфіденційність, прозорого та квантово-стійкого поштового сервісу, доступного сьогодні—тепер підкріплений [незалежним аудитом безпеки](/pentest-report_forward-email.pdf) від Cure53.
