...
**Травень 2026**: Були випущені значні покращення надійності та сумісності, включаючи підтримку Apple Push Notification (APN) для доставки пошти в реальному часі на iOS/macOS, комплексні виправлення CalDAV для сумісності синхронізації iOS (відновлення PRODID, LAST-MODIFIED, CREATED, SEQUENCE та правильне кодування href), політики збереження електронної пошти для кожного псевдоніма, покращену фільтрацію Sieve з декодуванням заголовків MIME та збереженням заголовків List-*, синхронізацію ключових слів IMAP з користувацькими мітками та захист від переповнення BSON для відправників з великим обсягом. Було додано підтримку застарілого TLS для клієнтів IMAP/POP3, а API push-сповіщень було розширено кінцевими точками CRUD токенів та конвеєра доставки.

**Червень 2026**: Forward Email опублікував свій перший незалежний аудит безпеки третьою стороною, проведений компанією [Cure53](https://cure53.de/), однією з найбільш шанованих фірм з дослідження безпеки в галузі. Повний звіт про тестування на проникнення доступний за адресою [pentest-report_forward-email.pdf](/pentest-report_forward-email.pdf). Аудит охопив всю архітектуру з відкритим вихідним кодом та серверну інфраструктуру, при цьому всі виявлені проблеми були усунені та верифіковані шляхом безперервного повторного тестування. Були застосовані додаткові заходи зміцнення безпеки, включаючи закріплення DNS для запобігання SSRF, автентифікацію за токеном запрошення та валідацію URL Domain Connect.
