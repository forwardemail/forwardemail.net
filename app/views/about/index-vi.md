...
> \[!TIP]
> Để biết chi tiết kỹ thuật về kiến trúc, các triển khai bảo mật và lộ trình phát triển, xem [Bản trắng Kỹ thuật](https://forwardemail.net/technical-whitepaper.pdf) và [kiểm toán bảo mật](https://forwardemail.net/pentest-report_forward-email.pdf) độc lập của chúng tôi bởi Cure53.
...
**Tháng 5 năm 2026**: Các cải tiến đáng kể về độ tin cậy và khả năng tương thích đã được triển khai, bao gồm hỗ trợ Apple Push Notification (APN) cho việc gửi thư thời gian thực trên iOS/macOS, các bản sửa lỗi CalDAV toàn diện cho khả năng tương thích đồng bộ iOS (sửa chữa PRODID, LAST-MODIFIED, CREATED, SEQUENCE và mã hóa href đúng), chính sách lưu giữ email theo alias, lọc Sieve nâng cao với giải mã tiêu đề MIME và bảo tồn tiêu đề List-*, đồng bộ từ khóa IMAP với nhãn tùy chỉnh và bảo vệ tràn BSON cho người gửi khối lượng lớn. Hỗ trợ TLS cũ đã được thêm cho các client IMAP/POP3, và API thông báo đẩy được mở rộng với các endpoint CRUD token và pipeline giao hàng.

**Tháng 6 năm 2026**: Forward Email đã công bố kiểm toán bảo mật độc lập từ bên thứ ba đầu tiên, được thực hiện bởi [Cure53](https://cure53.de/), một trong những công ty nghiên cứu bảo mật được tôn trọng nhất trong ngành. Báo cáo kiểm tra xâm nhập đầy đủ có sẵn tại [pentest-report_forward-email.pdf](/pentest-report_forward-email.pdf). Kiểm toán bao gồm toàn bộ kiến trúc mã nguồn mở và cơ sở hạ tầng máy chủ, với tất cả các phát hiện được xử lý và xác minh thông qua kiểm tra lại liên tục. Các biện pháp tăng cường bảo mật bổ sung đã được áp dụng bao gồm DNS pinning để ngăn chặn SSRF, xác thực token mời và xác thực URL Domain Connect.
