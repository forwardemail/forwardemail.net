...
**Май 2026**: Были выпущены значительные улучшения надёжности и совместимости, включая поддержку Apple Push Notification (APN) для доставки почты в реальном времени на iOS/macOS, комплексные исправления CalDAV для совместимости синхронизации iOS (восстановление PRODID, LAST-MODIFIED, CREATED, SEQUENCE и правильная кодировка href), политики хранения электронной почты для каждого псевдонима, улучшенную фильтрацию Sieve с декодированием заголовков MIME и сохранением заголовков List-*, синхронизацию ключевых слов IMAP с пользовательскими метками и защиту от переполнения BSON для отправителей с большим объёмом. Была добавлена поддержка устаревшего TLS для клиентов IMAP/POP3, а API push-уведомлений был расширен конечными точками CRUD токенов и конвейера доставки.

**Июнь 2026**: Forward Email опубликовал свой первый независимый аудит безопасности третьей стороной, проведённый компанией [Cure53](https://cure53.de/), одной из самых уважаемых фирм по исследованию безопасности в отрасли. Полный отчёт о тестировании на проникновение доступен по адресу [pentest-report_forward-email.pdf](/pentest-report_forward-email.pdf). Аудит охватил всю архитектуру с открытым исходным кодом и серверную инфраструктуру, при этом все обнаруженные проблемы были устранены и верифицированы посредством непрерывного повторного тестирования. Были применены дополнительные меры по укреплению безопасности, включая закрепление DNS для предотвращения SSRF, аутентификацию по токену приглашения и валидацию URL Domain Connect.
