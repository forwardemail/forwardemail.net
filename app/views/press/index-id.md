# Press & Media Kit {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email press and media kit" class="rounded-lg" /> -->


## Table of Contents {#table-of-contents}

* [Tentang Forward Email](#about-forward-email)
* [Fakta & Statistik Utama](#key-facts--statistics)
* [Pengguna Terkenal](#notable-users)
* [Penghargaan & Pengakuan](#awards--recognition)
* [Font](#fonts)
* [Warna](#colors)
* [Logo](#logos)
* [Grafis](#graphics)


## About Forward Email {#about-forward-email}

Forward Email adalah satu-satunya layanan hosting email 100% open-source yang fokus pada keamanan dan privasi. Pelajari lebih lanjut tentang sejarah kami di [halaman Tentang kami](/about).

Layanan kami didirikan pada tahun 2017 dan mendukung email untuk lebih dari 500.000 domain – termasuk pengguna terkenal seperti [The U.S. Naval Academy](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [The Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [beberapa universitas](/blog/docs/alumni-email-forwarding-university-case-study) dan pemerintah, dan lainnya.

Tujuan kami adalah menjadi platform infrastruktur email dan keamanan kelas perusahaan – dan kami mematuhi [beberapa prinsip](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Jika Anda adalah anggota pers, jurnalis, atau perwakilan media dan ingin berbicara dengan kami, mengajukan pertanyaan, atau mempelajari lebih lanjut – silakan hubungi kami di `press@forwardemail.net`.


## Key Facts & Statistics {#key-facts--statistics}

| Metric                  | Value                                                                                            |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Didirikan**           | 2017                                                                                             |
| **Domain yang Didukung**| 500.000+                                                                                         |
| **Open Source**         | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Bahasa yang Didukung**| 25+                                                                                              |
| **Enkripsi**            | Quantum-safe ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                             |
| **Protokol**            | IMAP, POP3, SMTP, CalDAV, CardDAV ([lihat dokumen Kepatuhan RFC lengkap kami](/rfc))             |
| **Self-Hosting**        | [Tersedia melalui Docker](/self-hosted)                                                         |
| **Rating Trustpilot**   | 4.9/5 (140+ ulasan)                                                                             |


## Notable Users {#notable-users}

Forward Email dipercaya oleh organisasi terkemuka di berbagai industri:

* **Pemerintah:** The U.S. Naval Academy, Pemerintah South Australia, Pemerintah Republik Dominika
* **Teknologi:** Netflix Games, Canonical/Ubuntu, The Linux Foundation, The PHP Foundation, jQuery, LineageOS
* **Pendidikan:** University of Cambridge, University of Maryland, University of Washington, Tufts University, Swarthmore College
* **Pemimpin Industri:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Awards & Recognition {#awards--recognition}

* Terdaftar di [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8.800+ bintang GitHub)
* Terdaftar di [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270.000+ bintang GitHub)
* Ditampilkan dalam [Perbandingan Aliasing Email GitHub](https://github.com/fynks/email-aliasing-comparison)


## Fonts {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (bold; berat 700)
* **Isi:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (regular; berat 400)
* **Judul:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (bold; berat 700)
* **Ikon:** [Font Awesome 5](https://fontawesome.com/)
## Warna {#colors}

* **Biru:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Sian:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Hijau:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logo {#logos}

"Logo Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafik {#graphics}

"Freddy melambai dari luar angkasa"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy di dalam amplop"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy dengan parasut"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy melambai di ladang berumput"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
