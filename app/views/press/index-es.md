# Kit de Prensa y Medios {#press--media-kit}

<img loading="lazy" src="/img/articles/press.webp" alt="Forward Email kit de prensa y medios" class="rounded-lg" />


## Tabla de Contenidos {#table-of-contents}

* [Acerca de Forward Email](#about-forward-email)
* [Datos Clave y Estadísticas](#key-facts--statistics)
* [Usuarios Notables](#notable-users)
* [Premios y Reconocimientos](#awards--recognition)
* [Fuentes](#fonts)
* [Colores](#colors)
* [Logos](#logos)
* [Gráficos](#graphics)


## Acerca de Forward Email {#about-forward-email}

Forward Email es el único servicio de alojamiento de correo electrónico 100% de código abierto enfocado en la seguridad y privacidad. Aprende más sobre nuestra historia en [nuestra página Acerca de](/about).

Nuestro servicio fue fundado en 2017 y provee correo electrónico para más de 500,000 dominios – incluyendo usuarios notables como [La Academia Naval de EE.UU.](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [La Fundación Linux](/blog/docs/linux-foundation-email-enterprise-case-study), [varias universidades](/blog/docs/alumni-email-forwarding-university-case-study) y gobiernos, entre otros.

Nuestro objetivo es ser la plataforma de infraestructura de correo electrónico y seguridad de nivel empresarial – y nos adherimos a [varios principios](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Si eres miembro de la prensa, periodista o representante de medios y deseas hablar con nosotros, hacer preguntas o aprender más – por favor contáctanos en `press@forwardemail.net`.


## Datos Clave y Estadísticas {#key-facts--statistics}

| Métrica                 | Valor                                                                                           |
| :---------------------- | :---------------------------------------------------------------------------------------------- |
| **Fundado**             | 2017                                                                                           |
| **Dominios Gestionados**| 500,000+                                                                                       |
| **Código Abierto**      | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net))|
| **Idiomas Soportados**  | 25+                                                                                            |
| **Encriptación**        | ChaCha20-Poly1305 seguro contra computación cuántica, OpenPGP, WKD, S/MIME                     |
| **Protocolos**          | IMAP, POP3, SMTP, CalDAV, CardDAV ([ver nuestro documento completo de cumplimiento RFC](/rfc)) |
| **Autoalojamiento**     | [Disponible vía Docker](/self-hosted)                                                         |
| **Calificación Trustpilot** | 4.9/5 (más de 140 reseñas)                                                                  |


## Usuarios Notables {#notable-users}

Forward Email es confiado por organizaciones líderes en diversas industrias:

* **Gobierno:** La Academia Naval de EE.UU., Gobierno de Australia del Sur, Gobierno de República Dominicana
* **Tecnología:** Netflix Games, Canonical/Ubuntu, La Fundación Linux, La Fundación PHP, jQuery, LineageOS
* **Educación:** Universidad de Cambridge, Universidad de Maryland, Universidad de Washington, Universidad Tufts, Swarthmore College
* **Líderes de la Industria:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Premios y Reconocimientos {#awards--recognition}

* Listado en [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (más de 8,800 estrellas en GitHub)
* Listado en [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (más de 270,000 estrellas en GitHub)
* Destacado en la [Comparación de Alias de Correo en GitHub](https://github.com/fynks/email-aliasing-comparison)


## Fuentes {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (negrita; peso 700)
* **Cuerpo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (regular; peso 400)
* **Encabezados:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (negrita; peso 700)
* **Íconos:** [Font Awesome 5](https://fontawesome.com/)
## Colores {#colors}

* **Azul:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Cian:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Verde:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logotipos {#logos}

"Logotipo de Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Gráficos {#graphics}

"Freddy saludando desde el espacio exterior"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy en un sobre"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy con un paracaídas"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy saludando en un campo de hierba"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
