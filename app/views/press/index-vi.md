# Bộ Công Cụ Báo Chí & Truyền Thông {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email press and media kit" class="rounded-lg" /> -->


## Mục Lục {#table-of-contents}

* [Về Forward Email](#about-forward-email)
* [Số Liệu & Thống Kê Chính](#key-facts--statistics)
* [Người Dùng Nổi Bật](#notable-users)
* [Giải Thưởng & Công Nhận](#awards--recognition)
* [Phông Chữ](#fonts)
* [Màu Sắc](#colors)
* [Logo](#logos)
* [Đồ Họa](#graphics)


## Về Forward Email {#about-forward-email}

Forward Email là dịch vụ lưu trữ email mã nguồn mở 100% duy nhất tập trung vào bảo mật và quyền riêng tư. Tìm hiểu thêm về lịch sử của chúng tôi trên [trang Giới thiệu của chúng tôi](/about).

Dịch vụ của chúng tôi được thành lập vào năm 2017 và cung cấp email cho hơn 500.000 tên miền – bao gồm các người dùng nổi bật như [Học Viện Hải Quân Hoa Kỳ](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [Quỹ Linux](/blog/docs/linux-foundation-email-enterprise-case-study), [một số trường đại học](/blog/docs/alumni-email-forwarding-university-case-study) và các chính phủ, và nhiều hơn nữa.

Mục tiêu của chúng tôi là trở thành nền tảng hạ tầng email và bảo mật cấp doanh nghiệp – và chúng tôi tuân thủ [một số nguyên tắc](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Nếu bạn là thành viên báo chí, nhà báo hoặc đại diện truyền thông và muốn nói chuyện với chúng tôi, đặt câu hỏi hoặc tìm hiểu thêm – vui lòng liên hệ với chúng tôi tại `press@forwardemail.net`.


## Số Liệu & Thống Kê Chính {#key-facts--statistics}

| Chỉ Số                  | Giá Trị                                                                                          |
| :---------------------- | :------------------------------------------------------------------------------------------------ |
| **Thành lập**           | 2017                                                                                             |
| **Tên miền được hỗ trợ**| Hơn 500.000                                                                                      |
| **Mã nguồn mở**         | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Ngôn ngữ hỗ trợ**     | Hơn 25                                                                                          |
| **Mã hóa**              | Quantum-safe ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                            |
| **Giao thức**           | IMAP, POP3, SMTP, CalDAV, CardDAV ([xem tài liệu Tuân thủ RFC đầy đủ của chúng tôi](/rfc))       |
| **Tự lưu trữ**          | [Có sẵn qua Docker](/self-hosted)                                                               |
| **Đánh giá Trustpilot** | 4.9/5 (hơn 140 đánh giá)                                                                        |


## Người Dùng Nổi Bật {#notable-users}

Forward Email được tin tưởng bởi các tổ chức hàng đầu trong nhiều ngành:

* **Chính phủ:** Học Viện Hải Quân Hoa Kỳ, Chính phủ Nam Úc, Chính phủ Cộng hòa Dominica
* **Công nghệ:** Netflix Games, Canonical/Ubuntu, Quỹ Linux, Quỹ PHP, jQuery, LineageOS
* **Giáo dục:** Đại học Cambridge, Đại học Maryland, Đại học Washington, Đại học Tufts, Trường Swarthmore College
* **Lãnh đạo ngành:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Giải Thưởng & Công Nhận {#awards--recognition}

* Được liệt kê trên [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (hơn 8.800 sao trên GitHub)
* Được liệt kê trên [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (hơn 270.000 sao trên GitHub)
* Xuất hiện trong [So sánh Địa chỉ Email GitHub](https://github.com/fynks/email-aliasing-comparison)


## Phông Chữ {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (đậm; trọng số 700)
* **Nội dung:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (thường; trọng số 400)
* **Tiêu đề:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (đậm; trọng số 700)
* **Biểu tượng:** [Font Awesome 5](https://fontawesome.com/)
## Màu sắc {#colors}

* **Xanh dương:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Xanh lơ:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Xanh lá:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logo {#logos}

"Logo Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Đồ họa {#graphics}

"Freddy vẫy tay từ ngoài không gian"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy trong phong bì"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy với dù"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy vẫy tay trên cánh đồng cỏ"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
