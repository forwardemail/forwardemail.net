# Presse- og mediepakke {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email presse- og mediepakke" class="rounded-lg" /> -->


## Innholdsfortegnelse {#table-of-contents}

* [Om Forward Email](#about-forward-email)
* [Nøkkelfakta og statistikk](#key-facts--statistics)
* [Merkbare brukere](#notable-users)
* [Priser og anerkjennelser](#awards--recognition)
* [Skrifter](#fonts)
* [Farger](#colors)
* [Logoer](#logos)
* [Grafikk](#graphics)


## Om Forward Email {#about-forward-email}

Forward Email er den eneste 100 % åpen kildekode e-posttjenesten med fokus på sikkerhet og personvern. Lær mer om vår historie på [vår Om-side](/about).

Vår tjeneste ble grunnlagt i 2017 og driver e-post for over 500 000 domener – inkludert merkbare brukere som [The U.S. Naval Academy](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [The Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [flere universiteter](/blog/docs/alumni-email-forwarding-university-case-study) og regjeringer, og mer.

Vårt mål er å være e-post- og sikkerhetsinfrastrukturplattformen på bedriftsnivå – og vi følger [flere prinsipper](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Hvis du er medlem av pressen, journalist eller medierepresentant og ønsker å snakke med oss, stille spørsmål eller lære mer – vennligst kontakt oss på `press@forwardemail.net`.


## Nøkkelfakta og statistikk {#key-facts--statistics}

| Måleparameter           | Verdi                                                                                             |
| :---------------------- | :------------------------------------------------------------------------------------------------ |
| **Grunnlagt**           | 2017                                                                                             |
| **Domener drevet**      | 500 000+                                                                                        |
| **Åpen kildekode**      | 100 % (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Støttede språk**      | 25+                                                                                              |
| **Kryptering**          | Kvantesikker ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                             |
| **Protokoller**         | IMAP, POP3, SMTP, CalDAV, CardDAV ([se vårt fullstendige RFC Compliance-dokument](/rfc))          |
| **Selvhosting**         | [Tilgjengelig via Docker](/self-hosted)                                                          |
| **Trustpilot-vurdering**| 4,9/5 (140+ anmeldelser)                                                                         |


## Merkbare brukere {#notable-users}

Forward Email er betrodd av ledende organisasjoner på tvers av bransjer:

* **Regjering:** The U.S. Naval Academy, Government of South Australia, Government of Dominican Republic
* **Teknologi:** Netflix Games, Canonical/Ubuntu, The Linux Foundation, The PHP Foundation, jQuery, LineageOS
* **Utdanning:** University of Cambridge, University of Maryland, University of Washington, Tufts University, Swarthmore College
* **Bransjeledere:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Priser og anerkjennelser {#awards--recognition}

* Oppført på [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8 800+ GitHub-stjerner)
* Oppført på [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270 000+ GitHub-stjerner)
* Fremhevet i [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)


## Skrifter {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (fet; vekt 700)
* **Brødtekst:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (normal; vekt 400)
* **Overskrifter:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (fet; vekt 700)
* **Ikoner:** [Font Awesome 5](https://fontawesome.com/)
## Farger {#colors}

* **Blå:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Cyan:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Grønn:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logoer {#logos}

"Forward Email-logo"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafikk {#graphics}

"Freddy vinker fra verdensrommet"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy i en konvolutt"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy med fallskjerm"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy vinker på en gresskledd eng"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
