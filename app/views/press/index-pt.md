# Kit de Imprensa & Mídia {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email kit de imprensa e mídia" class="rounded-lg" /> -->


## Índice {#table-of-contents}

* [Sobre o Forward Email](#about-forward-email)
* [Fatos e Estatísticas Principais](#key-facts--statistics)
* [Usuários Notáveis](#notable-users)
* [Prêmios e Reconhecimentos](#awards--recognition)
* [Fontes](#fonts)
* [Cores](#colors)
* [Logotipos](#logos)
* [Gráficos](#graphics)


## Sobre o Forward Email {#about-forward-email}

Forward Email é o único serviço de hospedagem de e-mail 100% open-source focado em segurança e privacidade. Saiba mais sobre nossa história em [nossa página Sobre](/about).

Nosso serviço foi fundado em 2017 e alimenta e-mails para mais de 500.000 domínios – incluindo usuários notáveis como [A Academia Naval dos EUA](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [A Fundação Linux](/blog/docs/linux-foundation-email-enterprise-case-study), [várias universidades](/blog/docs/alumni-email-forwarding-university-case-study) e governos, entre outros.

Nosso objetivo é ser a plataforma de infraestrutura de e-mail e segurança de nível empresarial – e seguimos [vários princípios](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Se você é membro da imprensa, jornalista ou representante da mídia e gostaria de falar conosco, fazer perguntas ou saber mais – por favor, entre em contato pelo e-mail `press@forwardemail.net`.


## Fatos e Estatísticas Principais {#key-facts--statistics}

| Métrica                 | Valor                                                                                           |
| :---------------------- | :---------------------------------------------------------------------------------------------- |
| **Fundação**            | 2017                                                                                           |
| **Domínios Atendidos**  | 500.000+                                                                                       |
| **Open Source**         | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Idiomas Suportados**  | 25+                                                                                            |
| **Criptografia**        | ChaCha20-Poly1305 seguro contra computação quântica, OpenPGP, WKD, S/MIME                      |
| **Protocolos**          | IMAP, POP3, SMTP, CalDAV, CardDAV ([veja nosso documento completo de conformidade RFC](/rfc))  |
| **Auto-Hospedagem**     | [Disponível via Docker](/self-hosted)                                                          |
| **Avaliação Trustpilot**| 4.9/5 (140+ avaliações)                                                                         |


## Usuários Notáveis {#notable-users}

Forward Email é confiável por organizações líderes em diversos setores:

* **Governo:** A Academia Naval dos EUA, Governo da Austrália do Sul, Governo da República Dominicana
* **Tecnologia:** Netflix Games, Canonical/Ubuntu, A Fundação Linux, A Fundação PHP, jQuery, LineageOS
* **Educação:** Universidade de Cambridge, Universidade de Maryland, Universidade de Washington, Universidade Tufts, Swarthmore College
* **Líderes da Indústria:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Prêmios e Reconhecimentos {#awards--recognition}

* Listado em [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8.800+ estrelas no GitHub)
* Listado em [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270.000+ estrelas no GitHub)
* Destaque no [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)


## Fontes {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (negrito; peso 700)
* **Corpo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (regular; peso 400)
* **Títulos:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (negrito; peso 700)
* **Ícones:** [Font Awesome 5](https://fontawesome.com/)
## Cores {#colors}

* **Azul:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Ciano:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Verde:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logos {#logos}

"Logo do Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Gráficos {#graphics}

"Freddy acenando do espaço sideral"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy em um envelope"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy com um paraquedas"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy acenando em um campo gramado"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
