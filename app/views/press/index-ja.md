# プレス＆メディアキット {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Emailのプレスおよびメディアキット" class="rounded-lg" /> -->


## 目次 {#table-of-contents}

* [Forward Emailについて](#about-forward-email)
* [主な事実と統計](#key-facts--statistics)
* [著名なユーザー](#notable-users)
* [受賞歴と評価](#awards--recognition)
* [フォント](#fonts)
* [カラー](#colors)
* [ロゴ](#logos)
* [グラフィックス](#graphics)


## Forward Emailについて {#about-forward-email}

Forward Emailは、セキュリティとプライバシーに特化した唯一の100%オープンソースのメールホスティングサービスです。[私たちのAboutページ](/about)で歴史について詳しくご覧いただけます。

当サービスは2017年に設立され、50万以上のドメインのメールを支えています。著名なユーザーには、[米国海軍兵学校](/blog/docs/federal-government-email-service-section-889-compliant)、[Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study)、Netflix Games、[Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study)、[複数の大学](/blog/docs/alumni-email-forwarding-university-case-study)や政府機関などが含まれます。

私たちの目標はエンタープライズグレードのメールおよびセキュリティインフラプラットフォームとなることであり、[いくつかの原則](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles)を遵守しています。

プレス関係者、ジャーナリスト、メディアの方でご連絡、ご質問、ご相談をご希望の場合は、`press@forwardemail.net`までお問い合わせください。


## 主な事実と統計 {#key-facts--statistics}

| 指標                     | 値                                                                                              |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **設立年**               | 2017                                                                                             |
| **対応ドメイン数**       | 50万以上                                                                                        |
| **オープンソース**       | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **対応言語数**           | 25以上                                                                                          |
| **暗号化方式**           | 量子安全なChaCha20-Poly1305、OpenPGP、WKD、S/MIME                                              |
| **プロトコル**           | IMAP、POP3、SMTP、CalDAV、CardDAV ([完全なRFC準拠ドキュメントはこちら](/rfc))                    |
| **セルフホスティング**   | [Dockerで利用可能](/self-hosted)                                                                |
| **Trustpilot評価**       | 4.9/5 (140件以上のレビュー)                                                                     |


## 著名なユーザー {#notable-users}

Forward Emailは業界を超えた主要な組織に信頼されています：

* **政府機関:** 米国海軍兵学校、南オーストラリア政府、ドミニカ共和国政府
* **テクノロジー:** Netflix Games、Canonical/Ubuntu、Linux Foundation、PHP Foundation、jQuery、LineageOS
* **教育機関:** ケンブリッジ大学、メリーランド大学、ワシントン大学、タフツ大学、スワースモア大学
* **業界リーダー:** Isaac Z. Schlueter (npm)、David Heinemeier Hansson (Ruby on Rails)


## 受賞歴と評価 {#awards--recognition}

* [Awesome Privacy](https://github.com/Lissy93/awesome-privacy)に掲載（8,800以上のGitHubスター）
* [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted)に掲載（270,000以上のGitHubスター）
* [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)で紹介


## フォント {#fonts}

* **ロゴ:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans)（ボールド; 700ウェイト）
* **本文:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans)（レギュラー; 400ウェイト）
* **見出し:** [VC Honey](https://verycoolstudio.com/typefaces/honey)（ボールド; 700ウェイト）
* **アイコン:** [Font Awesome 5](https://fontawesome.com/)
## Colors {#colors}

* **青:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **シアン:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **緑:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logos {#logos}

"Forward Email ロゴ"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Graphics {#graphics}

"宇宙から手を振るFreddy"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"封筒の中のFreddy"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"パラシュートを持つFreddy"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"草原で手を振るFreddy"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
