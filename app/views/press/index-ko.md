# 보도자료 및 미디어 키트 {#press--media-kit}

<img loading="lazy" src="/img/articles/press.webp" alt="Forward Email press and media kit" class="rounded-lg" />


## 목차 {#table-of-contents}

* [Forward Email 소개](#about-forward-email)
* [주요 사실 및 통계](#key-facts--statistics)
* [주요 사용자](#notable-users)
* [수상 및 인정](#awards--recognition)
* [폰트](#fonts)
* [색상](#colors)
* [로고](#logos)
* [그래픽](#graphics)


## Forward Email 소개 {#about-forward-email}

Forward Email은 보안과 개인정보 보호에 중점을 둔 유일한 100% 오픈 소스 이메일 호스팅 서비스입니다. [소개 페이지](/about)에서 우리의 역사에 대해 더 알아보세요.

우리 서비스는 2017년에 설립되었으며, 500,000개 이상의 도메인에서 이메일을 지원합니다 – 여기에는 [미 해군사관학교](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [리눅스 재단](/blog/docs/linux-foundation-email-enterprise-case-study), [여러 대학](/blog/docs/alumni-email-forwarding-university-case-study) 및 정부 기관 등 주목할 만한 사용자들이 포함되어 있습니다.

우리의 목표는 엔터프라이즈급 이메일 및 보안 인프라 플랫폼이 되는 것이며 – [여러 원칙](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles)을 준수합니다.

언론인, 기자 또는 미디어 관계자이시며 저희와 대화하거나 질문이 있거나 더 알고 싶으시면 `press@forwardemail.net`으로 연락해 주세요.


## 주요 사실 및 통계 {#key-facts--statistics}

| 지표                     | 값                                                                                              |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **설립연도**             | 2017                                                                                             |
| **지원 도메인 수**       | 500,000+                                                                                        |
| **오픈 소스**            | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **지원 언어 수**         | 25+                                                                                            |
| **암호화**               | 양자 안전 ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                              |
| **프로토콜**             | IMAP, POP3, SMTP, CalDAV, CardDAV ([전체 RFC 준수 문서 보기](/rfc))                             |
| **셀프 호스팅**          | [Docker를 통한 사용 가능](/self-hosted)                                                        |
| **Trustpilot 평점**      | 4.9/5 (140+ 리뷰)                                                                              |


## 주요 사용자 {#notable-users}

Forward Email은 다양한 산업 분야의 선도 조직들로부터 신뢰받고 있습니다:

* **정부:** 미 해군사관학교, 남호주 정부, 도미니카 공화국 정부
* **기술:** Netflix Games, Canonical/Ubuntu, 리눅스 재단, PHP 재단, jQuery, LineageOS
* **교육:** 케임브리지 대학교, 메릴랜드 대학교, 워싱턴 대학교, 터프츠 대학교, 스와스모어 칼리지
* **산업 리더:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## 수상 및 인정 {#awards--recognition}

* [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) 목록 등재 (8,800+ GitHub 스타)
* [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) 목록 등재 (270,000+ GitHub 스타)
* [GitHub 이메일 별칭 비교](https://github.com/fynks/email-aliasing-comparison) 소개


## 폰트 {#fonts}

* **로고:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (볼드; 700 굵기)
* **본문:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (일반; 400 굵기)
* **헤딩:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (볼드; 700 굵기)
* **아이콘:** [Font Awesome 5](https://fontawesome.com/)
## 색상 {#colors}

* **파랑:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **청록:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **초록:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## 로고 {#logos}

"포워드 이메일 로고"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## 그래픽 {#graphics}

"우주에서 손 흔드는 프레디"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"봉투 안의 프레디"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"낙하산을 든 프레디"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"잔디밭에서 손 흔드는 프레디"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
