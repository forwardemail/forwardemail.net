# Sajtó és Média Készlet {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email sajtó és média készlet" class="rounded-lg" /> -->


## Tartalomjegyzék {#table-of-contents}

* [A Forward Emailről](#about-forward-email)
* [Főbb tények és statisztikák](#key-facts--statistics)
* [Jelentős felhasználók](#notable-users)
* [Díjak és elismerések](#awards--recognition)
* [Betűtípusok](#fonts)
* [Színek](#colors)
* [Logók](#logos)
* [Grafikák](#graphics)


## A Forward Emailről {#about-forward-email}

A Forward Email az egyetlen 100%-ban nyílt forráskódú e-mail szolgáltatás, amely a biztonságra és adatvédelemre fókuszál. Tudjon meg többet történetünkről a [Rólunk oldalon](/about).

Szolgáltatásunkat 2017-ben alapítottuk, és több mint 500 000 domain e-mailjeit működtetjük – köztük jelentős felhasználók, mint például a [U.S. Naval Academy](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [The Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [több egyetem](/blog/docs/alumni-email-forwarding-university-case-study) és kormányok, valamint még sokan mások.

Célunk, hogy vállalati szintű e-mail és biztonsági infrastruktúra platform legyünk – és betartunk [számos alapelvet](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Ha Ön sajtótag, újságíró vagy média képviselő, és szeretne velünk beszélni, kérdéseket feltenni vagy többet megtudni – kérjük, lépjen kapcsolatba velünk a `press@forwardemail.net` címen.


## Főbb tények és statisztikák {#key-facts--statistics}

| Mutató                  | Érték                                                                                            |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Alapítás éve**         | 2017                                                                                             |
| **Működtetett domainek**| 500 000+                                                                                        |
| **Nyílt forráskód**     | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Támogatott nyelvek**  | 25+                                                                                              |
| **Titkosítás**          | Kvantumbiztos ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                           |
| **Protokollok**         | IMAP, POP3, SMTP, CalDAV, CardDAV ([lásd teljes RFC megfelelőségi dokumentumunkat](/rfc))        |
| **Önhosztolás**         | [Elérhető Dockerrel](/self-hosted)                                                              |
| **Trustpilot értékelés**| 4.9/5 (140+ értékelés)                                                                           |


## Jelentős felhasználók {#notable-users}

A Forward Email vezető szervezetek bizalmát élvezi különböző iparágakban:

* **Kormányzat:** U.S. Naval Academy, Dél-Ausztrália Kormánya, Dominikai Köztársaság Kormánya
* **Technológia:** Netflix Games, Canonical/Ubuntu, The Linux Foundation, The PHP Foundation, jQuery, LineageOS
* **Oktatás:** Cambridge Egyetem, Maryland Egyetem, Washington Egyetem, Tufts Egyetem, Swarthmore College
* **Iparági vezetők:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Díjak és elismerések {#awards--recognition}

* Felsorolva az [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) listán (8 800+ GitHub csillag)
* Felsorolva az [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) listán (270 000+ GitHub csillag)
* Megjelenve a [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison) összehasonlításban


## Betűtípusok {#fonts}

* **Logó:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (félkövér; 700 súly)
* **Törzs:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (normál; 400 súly)
* **Címsorok:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (félkövér; 700 súly)
* **Ikonok:** [Font Awesome 5](https://fontawesome.com/)
## Színek {#colors}

* **Kék:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Cián:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Zöld:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logók {#logos}

"Forward Email logó"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafikák {#graphics}

"Freddy integet az űrből"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy egy borítékban"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy ejtőernyővel"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy integet egy füves mezőn"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
