# Прес-реліз та медіа-кит {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email прес-реліз та медіа-кит" class="rounded-lg" /> -->


## Зміст {#table-of-contents}

* [Про Forward Email](#about-forward-email)
* [Основні факти та статистика](#key-facts--statistics)
* [Відомі користувачі](#notable-users)
* [Нагороди та визнання](#awards--recognition)
* [Шрифти](#fonts)
* [Кольори](#colors)
* [Логотипи](#logos)
* [Графіка](#graphics)


## Про Forward Email {#about-forward-email}

Forward Email — єдиний 100% відкритий сервіс хостингу електронної пошти, орієнтований на безпеку та конфіденційність. Дізнайтеся більше про нашу історію на [нашій сторінці "Про нас"](/about).

Наш сервіс заснований у 2017 році і обслуговує електронну пошту для понад 500 000 доменів – серед яких відомі користувачі, такі як [Військово-морська академія США](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [Фонд Linux](/blog/docs/linux-foundation-email-enterprise-case-study), [кілька університетів](/blog/docs/alumni-email-forwarding-university-case-study) та уряди, і багато інших.

Наша мета — бути платформою корпоративного рівня для електронної пошти та інфраструктури безпеки – і ми дотримуємося [кількох принципів](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Якщо ви є представником преси, журналістом або медіа-представником і хочете поспілкуватися з нами, поставити запитання або дізнатися більше – будь ласка, зв’яжіться з нами за адресою `press@forwardemail.net`.


## Основні факти та статистика {#key-facts--statistics}

| Показник                | Значення                                                                                         |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Засновано**           | 2017                                                                                            |
| **Обслуговувані домени**| 500 000+                                                                                        |
| **Відкритий код**       | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Підтримувані мови**   | 25+                                                                                            |
| **Шифрування**          | Квантово-безпечний ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                     |
| **Протоколи**           | IMAP, POP3, SMTP, CalDAV, CardDAV ([див. наш повний документ відповідності RFC](/rfc))          |
| **Самохостинг**         | [Доступний через Docker](/self-hosted)                                                         |
| **Рейтинг Trustpilot**  | 4.9/5 (140+ відгуків)                                                                           |


## Відомі користувачі {#notable-users}

Forward Email довіряють провідні організації з різних галузей:

* **Уряд:** Військово-морська академія США, Уряд Південної Австралії, Уряд Домініканської Республіки
* **Технології:** Netflix Games, Canonical/Ubuntu, Фонд Linux, Фонд PHP, jQuery, LineageOS
* **Освіта:** Кембриджський університет, Університет Меріленду, Університет Вашингтона, Університет Тафтса, Коледж Суортмор
* **Лідери галузі:** Айзек З. Шлюетер (npm), Девід Хайнемайєр Ханссон (Ruby on Rails)


## Нагороди та визнання {#awards--recognition}

* Включено до списку [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8 800+ зірок на GitHub)
* Включено до списку [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270 000+ зірок на GitHub)
* Представлено у [порівнянні GitHub Email Aliasing](https://github.com/fynks/email-aliasing-comparison)


## Шрифти {#fonts}

* **Логотип:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (жирний; вага 700)
* **Основний текст:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (звичайний; вага 400)
* **Заголовки:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (жирний; вага 700)
* **Іконки:** [Font Awesome 5](https://fontawesome.com/)
## Кольори {#colors}

* **Синій:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Блакитний:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Зелений:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Логотипи {#logos}

"Логотип Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Графіка {#graphics}

"Фредді махає з космосу"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Фредді в конверті"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Фредді з парашутом"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Фредді махає на трав’яному полі"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
