# Presse & Medienkit {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email Presse- und Medienkit" class="rounded-lg" /> -->


## Inhaltsverzeichnis {#table-of-contents}

* [Über Forward Email](#about-forward-email)
* [Wichtige Fakten & Statistiken](#key-facts--statistics)
* [Bekannte Nutzer](#notable-users)
* [Auszeichnungen & Anerkennungen](#awards--recognition)
* [Schriftarten](#fonts)
* [Farben](#colors)
* [Logos](#logos)
* [Grafiken](#graphics)


## Über Forward Email {#about-forward-email}

Forward Email ist der einzige 100% Open-Source E-Mail-Hosting-Dienst, der sich auf Sicherheit und Datenschutz konzentriert. Erfahren Sie mehr über unsere Geschichte auf [unserer Über-Seite](/about).

Unser Dienst wurde 2017 gegründet und betreibt E-Mail für über 500.000 Domains – darunter namhafte Nutzer wie [die U.S. Naval Academy](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [The Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [mehrere Universitäten](/blog/docs/alumni-email-forwarding-university-case-study) und Regierungen sowie weitere.

Unser Ziel ist es, die E-Mail- und Sicherheitsinfrastrukturplattform auf Unternehmensniveau zu sein – und wir halten uns an [mehrere Prinzipien](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Wenn Sie Mitglied der Presse, Journalist oder Medienvertreter sind und mit uns sprechen, Fragen stellen oder mehr erfahren möchten – kontaktieren Sie uns bitte unter `press@forwardemail.net`.


## Wichtige Fakten & Statistiken {#key-facts--statistics}

| Kennzahl                | Wert                                                                                             |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Gegründet**           | 2017                                                                                            |
| **Betreute Domains**    | 500.000+                                                                                        |
| **Open Source**         | 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Unterstützte Sprachen** | 25+                                                                                           |
| **Verschlüsselung**     | Quantum-sicher ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                         |
| **Protokolle**          | IMAP, POP3, SMTP, CalDAV, CardDAV ([siehe unser vollständiges RFC-Compliance-Dokument](/rfc))   |
| **Eigenes Hosting**     | [Verfügbar via Docker](/self-hosted)                                                           |
| **Trustpilot-Bewertung**| 4,9/5 (140+ Bewertungen)                                                                        |


## Bekannte Nutzer {#notable-users}

Forward Email wird von führenden Organisationen aus verschiedenen Branchen vertraut:

* **Regierung:** Die U.S. Naval Academy, Regierung von South Australia, Regierung der Dominikanischen Republik
* **Technologie:** Netflix Games, Canonical/Ubuntu, The Linux Foundation, The PHP Foundation, jQuery, LineageOS
* **Bildung:** University of Cambridge, University of Maryland, University of Washington, Tufts University, Swarthmore College
* **Branchenführer:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Auszeichnungen & Anerkennungen {#awards--recognition}

* Gelistet auf [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8.800+ GitHub-Sterne)
* Gelistet auf [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270.000+ GitHub-Sterne)
* Vorgestellt im [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)


## Schriftarten {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (fett; Gewicht 700)
* **Fließtext:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (normal; Gewicht 400)
* **Überschriften:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (fett; Gewicht 700)
* **Symbole:** [Font Awesome 5](https://fontawesome.com/)
## Farben {#colors}

* **Blau:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Cyan:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Grün:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logos {#logos}

"Forward Email Logo"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafiken {#graphics}

"Freddy winkt aus dem Weltraum"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy in einem Umschlag"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy mit einem Fallschirm"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy winkt auf einer grünen Wiese"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
