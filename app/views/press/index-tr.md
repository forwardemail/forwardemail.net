# Basın & Medya Kiti {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email basın ve medya kiti" class="rounded-lg" /> -->


## İçindekiler {#table-of-contents}

* [Forward Email Hakkında](#about-forward-email)
* [Temel Bilgiler & İstatistikler](#key-facts--statistics)
* [Önemli Kullanıcılar](#notable-users)
* [Ödüller & Tanınma](#awards--recognition)
* [Fontlar](#fonts)
* [Renkler](#colors)
* [Logolar](#logos)
* [Grafikler](#graphics)


## Forward Email Hakkında {#about-forward-email}

Forward Email, güvenlik ve gizliliğe odaklanan tek %100 açık kaynaklı e-posta barındırma hizmetidir. Tarihimiz hakkında daha fazla bilgi için [Hakkımızda sayfamızı](/about) ziyaret edin.

Hizmetimiz 2017 yılında kuruldu ve 500.000’den fazla alan adı için e-posta hizmeti sağlıyor – bunlar arasında [ABD Deniz Harp Okulu](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [Linux Vakfı](/blog/docs/linux-foundation-email-enterprise-case-study), [birçok üniversite](/blog/docs/alumni-email-forwarding-university-case-study) ve hükümetler gibi önemli kullanıcılar bulunmaktadır.

Amacımız, kurumsal düzeyde e-posta ve güvenlik altyapısı platformu olmaktır – ve [birçok ilkeye](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles) bağlı kalıyoruz.

Basın mensubu, gazeteci veya medya temsilcisiyseniz ve bizimle konuşmak, sorular sormak ya da daha fazla bilgi almak isterseniz – lütfen `press@forwardemail.net` adresinden bizimle iletişime geçin.


## Temel Bilgiler & İstatistikler {#key-facts--statistics}

| Ölçüt                   | Değer                                                                                           |
| :---------------------- | :---------------------------------------------------------------------------------------------- |
| **Kuruluş Yılı**        | 2017                                                                                           |
| **Desteklenen Alan Adları** | 500.000+                                                                                     |
| **Açık Kaynak**         | %100 (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Desteklenen Diller**  | 25+                                                                                            |
| **Şifreleme**           | Kuantum-güvenli ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                        |
| **Protokoller**         | IMAP, POP3, SMTP, CalDAV, CardDAV ([tam RFC Uyumluluk dokümanımızı görün](/rfc))                |
| **Kendi Sunucunuzda Barındırma** | [Docker ile kullanılabilir](/self-hosted)                                                  |
| **Trustpilot Puanı**    | 4.9/5 (140+ yorum)                                                                             |


## Önemli Kullanıcılar {#notable-users}

Forward Email, sektörler arası önde gelen kuruluşlar tarafından güvenilmektedir:

* **Hükümet:** ABD Deniz Harp Okulu, Güney Avustralya Hükümeti, Dominik Cumhuriyeti Hükümeti
* **Teknoloji:** Netflix Games, Canonical/Ubuntu, Linux Vakfı, PHP Vakfı, jQuery, LineageOS
* **Eğitim:** Cambridge Üniversitesi, Maryland Üniversitesi, Washington Üniversitesi, Tufts Üniversitesi, Swarthmore Koleji
* **Sektör Liderleri:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Ödüller & Tanınma {#awards--recognition}

* [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) listesinde yer aldı (8.800+ GitHub yıldızı)
* [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) listesinde yer aldı (270.000+ GitHub yıldızı)
* [GitHub E-posta Takma Adı Karşılaştırması](https://github.com/fynks/email-aliasing-comparison) içinde öne çıktı


## Fontlar {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (kalın; 700 ağırlık)
* **Metin:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (normal; 400 ağırlık)
* **Başlıklar:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (kalın; 700 ağırlık)
* **Simgeler:** [Font Awesome 5](https://fontawesome.com/)
## Renkler {#colors}

* **Mavi:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Camgöbeği:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Yeşil:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Logolar {#logos}

"Forward Email Logosu"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafikler {#graphics}

"Dış uzaydan el sallayan Freddy"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Bir zarf içindeki Freddy"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Paraşütle Freddy"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Çimenli bir alanda el sallayan Freddy"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
