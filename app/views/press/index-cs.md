# Tiskový a mediální balíček {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email tiskový a mediální balíček" class="rounded-lg" /> -->


## Obsah {#table-of-contents}

* [O Forward Email](#about-forward-email)
* [Klíčová fakta a statistiky](#key-facts--statistics)
* [Významní uživatelé](#notable-users)
* [Ocenění a uznání](#awards--recognition)
* [Fonty](#fonts)
* [Barvy](#colors)
* [Loga](#logos)
* [Grafika](#graphics)


## O Forward Email {#about-forward-email}

Forward Email je jediná 100% open-source služba pro hosting e-mailů zaměřená na bezpečnost a soukromí. Více o naší historii se dozvíte na [naší stránce O nás](/about).

Naše služba byla založena v roce 2017 a zajišťuje e-mail pro více než 500 000 domén – včetně významných uživatelů jako [The U.S. Naval Academy](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [The Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [několik univerzit](/blog/docs/alumni-email-forwarding-university-case-study) a vlád, a další.

Naším cílem je být platformou pro e-mailovou a bezpečnostní infrastrukturu na úrovni podniků – a dodržujeme [několik principů](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Pokud jste členem tisku, novinářem nebo mediálním zástupcem a chcete s námi mluvit, klást otázky nebo se dozvědět více – kontaktujte nás prosím na `press@forwardemail.net`.


## Klíčová fakta a statistiky {#key-facts--statistics}

| Metrika                 | Hodnota                                                                                          |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Založeno**            | 2017                                                                                            |
| **Domény v provozu**    | 500 000+                                                                                        |
| **Open Source**         | 100 % (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Podporované jazyky**  | 25+                                                                                            |
| **Šifrování**           | Kvantově bezpečné ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                      |
| **Protokoly**           | IMAP, POP3, SMTP, CalDAV, CardDAV ([viz náš kompletní dokument o souladu s RFC](/rfc))          |
| **Vlastní hosting**     | [Dostupné přes Docker](/self-hosted)                                                           |
| **Hodnocení Trustpilot**| 4,9/5 (140+ recenzí)                                                                            |


## Významní uživatelé {#notable-users}

Forward Email důvěřují přední organizace napříč odvětvími:

* **Vláda:** The U.S. Naval Academy, vláda Jižní Austrálie, vláda Dominikánské republiky
* **Technologie:** Netflix Games, Canonical/Ubuntu, The Linux Foundation, The PHP Foundation, jQuery, LineageOS
* **Vzdělávání:** University of Cambridge, University of Maryland, University of Washington, Tufts University, Swarthmore College
* **Lídři průmyslu:** Isaac Z. Schlueter (npm), David Heinemeier Hansson (Ruby on Rails)


## Ocenění a uznání {#awards--recognition}

* Zařazeno na [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (8 800+ hvězd na GitHubu)
* Zařazeno na [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (270 000+ hvězd na GitHubu)
* Uvedeno v [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)


## Fonty {#fonts}

* **Logo:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (tučný; váha 700)
* **Text:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (běžný; váha 400)
* **Nadpisy:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (tučný; váha 700)
* **Ikony:** [Font Awesome 5](https://fontawesome.com/)
## Barvy {#colors}

* **Modrá:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Azurová:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Zelená:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Loga {#logos}

"Logo Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Grafika {#graphics}

"Freddy mává z vesmíru"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Freddy v obálce"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Freddy s padákem"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Freddy mává na travnatém poli"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
