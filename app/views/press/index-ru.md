# Пресс- и Медиа-набор {#press--media-kit}

<!-- <img loading="lazy" src="/img/articles/press.webp" alt="Forward Email пресс- и медиа-набор" class="rounded-lg" /> -->


## Содержание {#table-of-contents}

* [О Forward Email](#about-forward-email)
* [Основные факты и статистика](#key-facts--statistics)
* [Известные пользователи](#notable-users)
* [Награды и признание](#awards--recognition)
* [Шрифты](#fonts)
* [Цвета](#colors)
* [Логотипы](#logos)
* [Графика](#graphics)


## О Forward Email {#about-forward-email}

Forward Email — единственный 100% открытый сервис хостинга электронной почты, ориентированный на безопасность и конфиденциальность. Узнайте больше о нашей истории на [нашей странице «О нас»](/about).

Наш сервис был основан в 2017 году и обслуживает электронную почту более чем для 500 000 доменов — включая известных пользователей, таких как [Военно-морская академия США](/blog/docs/federal-government-email-service-section-889-compliant), [Canonical/Ubuntu](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Netflix Games, [Linux Foundation](/blog/docs/linux-foundation-email-enterprise-case-study), [несколько университетов](/blog/docs/alumni-email-forwarding-university-case-study) и правительства, и другие.

Наша цель — стать платформой корпоративного уровня для электронной почты и инфраструктуры безопасности — и мы придерживаемся [нескольких принципов](https://forwardemail.net/blog/docs/best-quantum-safe-encrypted-email-service#principles).

Если вы представитель прессы, журналист или медиапредставитель и хотите связаться с нами, задать вопросы или узнать больше — пожалуйста, свяжитесь с нами по адресу `press@forwardemail.net`.


## Основные факты и статистика {#key-facts--statistics}

| Показатель              | Значение                                                                                         |
| :---------------------- | :----------------------------------------------------------------------------------------------- |
| **Основан**             | 2017                                                                                            |
| **Обслуживаемые домены**| Более 500 000                                                                                   |
| **Открытый исходный код**| 100% (GitHub: [forwardemail/forwardemail.net](https://github.com/forwardemail/forwardemail.net)) |
| **Поддерживаемые языки**| Более 25                                                                                       |
| **Шифрование**          | Квантово-безопасный ChaCha20-Poly1305, OpenPGP, WKD, S/MIME                                     |
| **Протоколы**           | IMAP, POP3, SMTP, CalDAV, CardDAV ([см. полный документ о соответствии RFC](/rfc))              |
| **Самостоятельный хостинг**| [Доступен через Docker](/self-hosted)                                                        |
| **Рейтинг Trustpilot**  | 4.9/5 (более 140 отзывов)                                                                       |


## Известные пользователи {#notable-users}

Forward Email доверяют ведущие организации из разных отраслей:

* **Правительство:** Военно-морская академия США, Правительство Южной Австралии, Правительство Доминиканской Республики
* **Технологии:** Netflix Games, Canonical/Ubuntu, Linux Foundation, PHP Foundation, jQuery, LineageOS
* **Образование:** Кембриджский университет, Университет Мэриленда, Университет Вашингтона, Университет Тафтса, Колледж Суортмор
* **Лидеры отрасли:** Айзек З. Шлютер (npm), Дэвид Хайнемайер Ханссон (Ruby on Rails)


## Награды и признание {#awards--recognition}

* Включён в список [Awesome Privacy](https://github.com/Lissy93/awesome-privacy) (более 8 800 звёзд на GitHub)
* Включён в список [Awesome Self-Hosted](https://github.com/awesome-selfhosted/awesome-selfhosted) (более 270 000 звёзд на GitHub)
* Упомянут в [GitHub Email Aliasing Comparison](https://github.com/fynks/email-aliasing-comparison)


## Шрифты {#fonts}

* **Логотип:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (жирный; вес 700)
* **Основной текст:** [Nunito Sans](https://fonts.google.com/specimen/Nunito+Sans) (обычный; вес 400)
* **Заголовки:** [VC Honey](https://verycoolstudio.com/typefaces/honey) (жирный; вес 700)
* **Иконки:** [Font Awesome 5](https://fontawesome.com/)
## Цвета {#colors}

* **Синий:** `#0066FF` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#0066FF;"></span>
* **Голубой:** `#9DE5F9` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#9DE5F9;"></span>
* **Зеленый:** `#269C32` <span style="vertical-align:middle;display:inline-block;padding:10px;background:#269C32;"></span>


## Логотипы {#logos}

"Логотип Forward Email"

![Forward Email Logo](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/logo-square.svg>


## Графика {#graphics}

"Фредди машет из космоса"

![Freddy waving](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-onboard.gif>

"Фредди в конверте"

![Freddy in an envelope](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/domain-verified.gif>

"Фредди с парашютом"

![Freddy with a parachute](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/self-test.gif>

"Фредди машет на травяном поле"

![Freddy waving in a grassy field](https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif)

> <https://raw.githubusercontent.com/forwardemail/forwardemail.net/master/assets/img/emails/welcome.gif>
