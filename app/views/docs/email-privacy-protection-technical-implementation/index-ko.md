...
* [이메일 전달 작동 방식: 기술적 설명](#how-email-forwarding-works-the-technical-explanation)
  ...
  * [SRS(발신자 재작성 계획)의 역할](#the-role-of-srs-sender-rewriting-scheme)
...
* [경쟁사보다 Forward Email을 선택하는 이유](#why-choose-forward-email-over-competitors)
...
* [이메일 전달의 일반적인 사용 사례](#common-use-cases-for-email-forwarding)
  ...
  * [개인 정보 보호 의식이 강한 개인을 위해](#for-privacy-conscious-individuals)
* [이메일 전달을 위한 모범 사례](#best-practices-for-email-forwarding)
  * [1. 설명적 주소를 사용하세요](#1-use-descriptive-addresses)
  * [2. 적절한 인증 구현](#2-implement-proper-authentication)
  * [3. 정기적으로 전달 사항을 검토하세요](#3-regularly-review-your-forwards)
  * [4. 원활한 답변을 위해 "메일 보내기" 설정](#4-set-up-send-mail-as-for-seamless-replies)
  * [5. Catch-All 주소를 신중하게 사용하세요](#5-use-catch-all-addresses-cautiously)
* [결론](#conclusion)

## 서문 {#foreword}

이메일 전달은 온라인 커뮤니케이션을 관리하는 방식을 바꿀 수 있는 강력한 도구입니다. 사용자 지정 도메인으로 전문적인 이메일 주소를 만들고자 하는 사업주이든, 기본 이메일을 보호하고자 하는 개인 정보 보호 의식이 강한 개인이든, 유연한 이메일 관리가 필요한 개발자이든, 오늘날의 디지털 환경에서 이메일 전달을 이해하는 것은 필수적입니다.

Forward Email에서 우리는 세계에서 가장 안전하고, 사적이며, 유연한 이메일 전달 서비스를 구축했습니다. 이 포괄적인 가이드에서 우리는 이메일 전달이 어떻게 작동하는지(기술적, 실용적 관점에서) 설명하고, 간단한 설정 프로세스를 안내하고, 우리의 서비스가 경쟁사와 다른 이유를 강조할 것입니다.
...
SRS는 신뢰할 수 있는 이메일 전달에 필수적이기 때문에 특별한 주의를 기울여야 합니다. 이메일이 전달될 때, 발신자의 주소를 다시 작성하여 이메일이 최종 목적지에서 SPF 검사를 통과하도록 해야 합니다.

SRS가 없으면 전달된 이메일은 종종 SPF 검증에 실패하여 스팸으로 표시되거나 완전히 거부됩니다. SRS를 구현하면 전달된 이메일이 안정적으로 전달되고 원래 발신자 정보는 투명한 방식으로 유지됩니다.

## 이메일 전달 작동 방식: 간단한 설명 {#how-email-forwarding-works-the-simple-explanation}

기술적인 세부 사항이 너무 어려워 보인다면 이메일 전달을 이해하는 더 간단한 방법은 다음과 같습니다.

이메일 전달은 실제 우편물에 대한 우편물 전달과 비슷하다고 생각하세요. 새 집으로 이사하면 우편 서비스에 이전 주소에서 새 주소로 모든 우편물을 전달해 달라고 요청할 수 있습니다. 이메일 전달은 비슷하게 작동하지만 디지털 메시지의 경우입니다.
...
Forward Email의 가장 큰 장점 중 하나는 설정이 얼마나 쉬운가입니다. 단계별 가이드는 다음과 같습니다.

### 1. 계정 가입 {#1-sign-up-for-an-account}

[forwardemail.net](https://forwardemail.net)을(를) 방문하여 무료 계정을 만드세요. 가입 절차는 1분도 채 걸리지 않습니다.

### 2. 도메인 추가 {#2-add-your-domain}

로그인한 후 이메일 전달에 사용할 도메인을 추가합니다. 아직 도메인을 소유하지 않은 경우 먼저 도메인 등록 기관에서 도메인을 구매해야 합니다.

### 3. DNS 레코드 구성 {#3-configure-dns-records}

귀하의 도메인에 추가해야 할 정확한 DNS 레코드를 제공해 드립니다. 일반적으로 여기에는 다음이 포함됩니다.

* 이메일 서버를 가리키는 MX 레코드 추가
* 확인 및 보안을 위해 TXT 레코드 추가

대부분 도메인 등록 기관은 이러한 레코드를 추가하기 위한 간단한 인터페이스를 가지고 있습니다. 우리는 모든 주요 도메인 등록 기관에 대한 자세한 가이드를 제공하여 이 과정을 가능한 한 원활하게 만듭니다.

### 4. 이메일 전달 만들기 {#4-create-email-forwards}

DNS 레코드가 검증되면(보통 몇 분 정도 걸립니다) 이메일 전달을 만들 수 있습니다. 간단히 다음을 지정하세요.
...
그게 다입니다! 사용자 지정 도메인 주소로 전송된 이메일은 이제 지정된 목적지로 전달됩니다. 필요한 만큼 많은 전달을 만들 수 있으며, 도메인의 모든 주소로 전송된 모든 이메일을 전달하는 catch-all 주소도 포함됩니다.

## 이메일 전달의 고급 기능 {#advanced-features-of-forward-email}

기본 이메일 전달 기능 자체만으로도 강력하지만 Forward Email은 우리를 차별화하는 여러 가지 고급 기능을 제공합니다.

### 일회용 주소 {#disposable-addresses}

귀하의 메인 계정으로 전달되는 특정 또는 익명의 이메일 주소를 만드십시오. 이러한 주소에 라벨을 지정하고 언제든지 활성화하거나 비활성화하여 받은 편지함을 정리할 수 있습니다. 실제 이메일 주소는 절대 노출되지 않습니다.

### 여러 수신자 및 와일드카드 {#multiple-recipients-and-wildcards}

단일 주소를 여러 수신자에게 전달하여 팀과 정보를 쉽게 공유할 수 있습니다. 와일드카드 주소(catch-all 전달)를 사용하여 도메인의 모든 주소로 전송된 이메일을 받을 수도 있습니다.
...
이메일 전달 시장에는 여러 업체가 있지만 Forward Email은 몇 가지 중요한 면에서 두드러집니다.

### 1. 100% 오픈 소스 {#1-100-open-source}

우리는 백엔드 코드를 포함하여 완전히 오픈 소스인 유일한 이메일 전달 서비스입니다. 이러한 투명성은 신뢰를 구축하고 독립적인 보안 감사를 허용합니다. 다른 서비스는 오픈 소스라고 주장하지만 백엔드 코드를 공개하지 않습니다.

### 2. 개인 정보 보호 중심 {#2-privacy-focused}

우리는 귀하의 프라이버시 권리 때문에 이 서비스를 만들었습니다. TLS를 사용한 강력한 암호화를 사용하고, SMTP 로그를 저장하지 않으며(오류 및 아웃바운드 SMTP 제외), 귀하의 이메일을 디스크 스토리지에 쓰지 않습니다.

### 3. 제3자 의존 없음 {#3-no-third-party-reliance}

Amazon SES나 다른 타사 서비스에 의존하는 경쟁사와 달리, 당사는 인프라에 대한 완벽한 제어권을 유지하여 안정성과 개인 정보 보호 기능을 모두 향상시킵니다.

### 4. 비용 효율적인 가격 {#4-cost-effective-pricing}

저희의 가격 책정 모델을 통해 비용 효율적으로 확장할 수 있습니다. 사용자당 요금을 청구하지 않으며, 스토리지에 대해 사용하면서 지불할 수 있습니다. 월 $3로 Gandi(월 $3.99)와 같은 경쟁사보다 더 저렴한 가격으로 더 많은 기능을 제공합니다.

### 5. 무제한 리소스 {#5-unlimited-resources}

당사는 많은 경쟁사처럼 도메인, 별칭 또는 이메일 주소에 인위적인 제한을 두지 않습니다.

### 6. 주요 기관의 신뢰를 받음 {#6-trusted-by-major-organizations}

당사 서비스는 [미국 해군사관학교](/blog/docs/federal-government-email-service-section-889-compliant), Netflix, [리눅스 재단](/blog/docs/linux-foundation-email-enterprise-case-study), [캐노니컬/우분투](/blog/docs/canonical-ubuntu-email-enterprise-case-study), Disney Ad Sales 등 유명 조직을 포함하여 50만 개 이상의 도메인에서 사용되고 있습니다.
...
전달 가능성을 극대화하기 위해 도메인에 적절한 SPF, DKIM 및 DMARC 레코드가 있는지 확인하세요. Forward Email은 가이드 설정으로 이를 쉽게 해줍니다.

### 3. 정기적으로 전달 사항을 검토하세요. {#3-regularly-review-your-forwards}

주기적으로 이메일 전달을 감사하여 더 이상 필요하지 않거나 과도한 스팸을 수신하는 이메일 전달을 비활성화하세요.

### 4. 원활한 답장을 위한 "메일 보내기" 설정 {#4-set-up-send-mail-as-for-seamless-replies}

전달된 이메일에 답장할 때 일관된 환경을 얻으려면 사용자 정의 도메인 주소로 메일을 보내도록 기본 이메일 클라이언트를 구성하세요.

### 5. 포괄적인 주소를 신중하게 사용하세요. {#5-use-catch-all-addresses-cautiously}

포괄적인 주소는 편리하지만, 잠재적으로 더 많은 스팸을 받을 수 있습니다. 중요한 커뮤니케이션에 대한 특정 전달을 만드는 것을 고려하세요.

## 결론 {#conclusion}

이메일 전달은 이메일 커뮤니케이션에 전문성, 프라이버시, 단순함을 가져다주는 강력한 도구입니다. Forward Email을 사용하면 가장 안전하고, 프라이버시가 보장되며, 유연한 이메일 전달 서비스를 이용할 수 있습니다.
...
이메일 경험을 혁신할 준비가 되셨나요? 지금 바로 [무료로 가입하세요](https://forwardemail.net)에 가입하고 이미 50만 개 이상의 도메인이 저희 서비스의 혜택을 누리고 있는 데 동참하세요.
