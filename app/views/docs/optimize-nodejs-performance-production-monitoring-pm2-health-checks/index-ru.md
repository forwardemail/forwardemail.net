...
* **Контролируемое использование swap** сохраняет хосты приложений и другие хосты без баз данных без swap и подбирает их размер так, чтобы обычные рабочие нагрузки оставались в оперативной памяти, в то время как хосты MongoDB и Redis используют файл подкачки только для root с `vm.swappiness=1` строго в качестве страховки от нехватки памяти; активность swap базы данных отслеживается и расследуется
...
> \[!NOTE]
> Использование swap в рабочей среде — это страховка от нехватки памяти, а не обычная емкость. Хосты приложений остаются без swap, и ожидается, что обычные рабочие нагрузки будут оставаться в оперативной памяти.
