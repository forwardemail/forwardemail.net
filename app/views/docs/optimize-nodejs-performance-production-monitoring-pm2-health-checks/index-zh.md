...
* **受控的 Swap 使用** 使应用程序和其他非数据库主机保持无 Swap 状态，并调整其规模以确保正常工作负载保留在 RAM 中，而 MongoDB 和 Redis 主机使用仅限 root 访问的 Swap 文件，并设置 `vm.swappiness=1`，严格作为内存不足时的安全网；数据库的 Swap 活动会受到监控和调查
* **禁用核心转储**，防止包含敏感信息的内存转储
* **阻止 USB 存储**，防止未经授权的数据访问
* **内核参数调优**，兼顾安全和性能

> \[!NOTE]
> 生产环境的 Swap 是内存不足时的安全网，而非正常容量。应用程序主机保持无 Swap 状态，正常工作负载预期保留在 RAM 中。
