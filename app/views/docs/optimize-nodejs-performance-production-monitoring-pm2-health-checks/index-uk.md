...
* **Контрольоване використання swap** зберігає хости застосунків та інші хости, що не є базами даних, без swap, а їх розмір налаштований так, щоб звичайні робочі навантаження залишалися в RAM, тоді як хости MongoDB та Redis використовують файл swap лише для root з `vm.swappiness=1` суворо як захисну сітку від нестачі пам'яті; активність swap бази даних відстежується та розслідується
...
> \[!NOTE]
> Використання swap у продакшені є захисною сіткою від нестачі пам'яті, а не звичайною ємністю. Хости застосунків залишаються без swap, і очікується, що звичайні робочі навантаження залишатимуться в RAM.
