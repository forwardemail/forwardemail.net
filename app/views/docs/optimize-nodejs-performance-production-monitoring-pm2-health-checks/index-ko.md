...
* **제어된 스왑 사용** 애플리케이션 및 기타 비 데이터베이스 호스트를 스왑 없이 유지하고 정상적인 워크로드가 RAM에 유지되도록 크기를 조정하는 반면, MongoDB 및 Redis 호스트는 `vm.swappiness=1`이 설정된 root 전용 스왑 파일을 메모리 부족 안전망으로만 엄격하게 사용하며 데이터베이스 스왑 활동은 모니터링되고 조사됩니다
* **코어 덤프 비활성화**로 민감한 정보를 포함한 메모리 덤프 방지
* **USB 저장 장치 차단**으로 무단 데이터 접근 방지
* 보안 및 성능을 위한 **커널 파라미터 튜닝**

> \[!NOTE]
> 프로덕션 스왑은 정상적인 용량이 아닌 메모리 부족 안전망입니다. 애플리케이션 호스트는 스왑 없이 유지되며 정상적인 워크로드는 RAM에 유지될 것으로 예상됩니다.
