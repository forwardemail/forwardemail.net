...
* **制御されたスワップの使用** アプリケーションおよびその他の非データベースホストをスワップフリーに保ち、通常のワークロードがRAMに収まるようにサイジングする一方で、MongoDBおよびRedisホストは、メモリ不足時のセーフティネットとして厳密に`vm.swappiness=1`のroot専用スワップファイルを使用します。データベースのスワップアクティビティは監視および調査されます
* **コアダンプ無効化**により、機密情報を含むメモリダンプを防止
* **USBストレージのブロック**により、不正なデータアクセスを防止
* **カーネルパラメータの調整**によるセキュリティとパフォーマンスの最適化

> \[!NOTE]
> 本番環境のスワップはメモリ不足時のセーフティネットであり、通常の容量ではありません。アプリケーションホストはスワップフリーのままであり、通常のワークロードはRAMに収まることが想定されています。
