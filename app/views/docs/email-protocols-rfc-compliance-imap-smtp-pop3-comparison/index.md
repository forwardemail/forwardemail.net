...
* [Email Message Encryption](#email-message-encryption)
  ...
  * [Header Protection (RFC 9788)](#header-protection-rfc-9788)
...
| **Header Protection (RFC 9788)** | ✅ [Supported](#header-protection-rfc-9788)                                                     | ❌ Not Supported                                                                    | ❌ Not Supported                                                                   | ❌ Not Supported                                       | ❌ Not Supported                                                                                                                                                      | ❌ Not Supported                                                                          | ❌ Not Supported                                                      | ❌ Not Supported                                                                | ❌ Not Supported                                                   |
...
| [RFC 9788](https://datatracker.ietf.org/doc/html/rfc9788) | Header Protection for Cryptographically Protected Email                                 | ✅ Supported | Protects user-facing headers (Subject, From, To, etc.) within S/MIME and PGP/MIME encrypted payloads. See [Header Protection](#header-protection-rfc-9788)                                           |
| [RFC 9787](https://datatracker.ietf.org/doc/html/rfc9787) | E-mail Header Field Confidentiality and Integrity (Definitions)                         | ✅ Supported | Companion definitions RFC for RFC 9788 header classification (Structural vs. User-Facing)                                                                                                            |
...
| **Header Protection** | RFC 9788 | ✅ Supported | Cryptographic header protection for S/MIME and PGP/MIME |
...
> \[!NOTE]
> Forward Email is one of the first email providers to implement RFC 9788 header protection for cryptographically protected email.

**Header Protection** per [RFC 9788](https://www.rfc-editor.org/rfc/rfc9788.html) ensures that user-facing email header fields (such as Subject, From, To, Cc, Date, and others) are cryptographically protected within S/MIME and PGP/MIME encrypted messages. Without header protection, these headers travel in cleartext on the outer envelope even when the message body is encrypted, leaking metadata about the communication.

Forward Email implements the **Cryptographic Payload** model defined in RFC 9788 (with companion definitions from [RFC 9787](https://www.rfc-editor.org/rfc/rfc9787.html)):

**Key Features:**

* **Inner Header Duplication** - Non-structural headers (Subject, From, To, Cc, Date, etc.) are copied into the encrypted inner payload, ensuring they are tamper-proof
* **HP-Outer Annotations** - The inner payload includes `HP-Outer` header fields that document what the outer (unprotected) envelope exposes, enabling receiving MUAs to detect header manipulation
* **Subject Confidentiality** - The outer Subject is replaced with `...` per the `hcp_baseline` policy, preventing metadata leakage while maintaining compatibility
* **Content-Type `hp` Parameter** - The inner Content-Type includes `hp="cipher"` (for encrypted messages) or `hp="clear"` (for signed-only messages) to signal header protection to compliant MUAs
* **Legacy Display Element** - A backward-compatible text block is prepended to the message body so that older email clients can still display the protected Subject and other confidential headers
* **Backward Compatible** - Messages remain fully readable by MUAs that do not support RFC 9788; they simply see the obscured outer Subject and the legacy display element

**How It Works:**

1. When encrypting a message (S/MIME or PGP/MIME), all non-structural headers are copied into the Cryptographic Payload
2. Confidential headers (Subject, Keywords, Comments, In-Reply-To, References) are obscured or removed from the outer envelope
3. `HP-Outer` entries are added to the inner payload documenting the outer header values
4. The inner Content-Type receives an `hp="cipher"` parameter
5. A Legacy Display Element is prepended for backward compatibility
6. The entire inner payload is encrypted, protecting the headers cryptographically

**Supported Protocols:**

| Protocol     | Header Protection | Implementation                                    |
| ------------ | ----------------- | ------------------------------------------------- |
| **S/MIME**   | ✅ Supported       | Via `encrypt-message-smime.js` with `hp="cipher"` |
| **PGP/MIME** | ✅ Supported       | Via `encrypt-message.js` with `hp="cipher"`       |
...
| **Header Protection (RFC 9788)** | ✅ Yes             | ✅ Yes              | N/A               |
