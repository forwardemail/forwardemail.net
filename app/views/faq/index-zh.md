...
| 级别 | 范围 | 默认限制 | 描述 |
| :---- | :---- | :-----------: | :---------- |
| 每别名 | 单个别名 | 无（使用域限制） | 可选。如果某个别名设置了自定义 `smtp_limit`，则该限制会首先被检查。 |
| 每域 | 一天内从某个域发送的所有邮件 | 300/day | 统计该域所有别名的所有外发邮件。 |
| 每用户 | 一天内由某个用户账户发送的所有邮件 | 300/day | 防止通过删除并重新创建别名或域名来规避限制。 |
...
所有速率限制均通过数据库计数（`Emails.countDocuments`）来执行，统计从当前日开始（UTC 午夜）创建的邮件。这意味着您的限制将在每天 UTC 午夜重置。

如果您需要更高的限额，请 [联系我们](https://forwardemail.net/help)。大多数请求会在 1-2 小时内得到处理。
...
* **HELO/EHLO 拒绝列表检查**：在 SMTP HELO/EHLO 问候期间提供的主机名将与我们的拒绝列表进行比对检查，即使 IPv6 发送者缺少反向 DNS 记录也能提供保护
...
* `HELO` —— 表示问候，用于识别发送者的完全限定域名（FQDN）、IP 地址或邮件处理程序名称。该值可能被伪造，因此我们不依赖此数据，而是使用连接 IP 地址的反向主机名查找。 不过，我们现在除了使用反向主机名查找外，还会将此值与我们的[拒绝列表](#do-you-have-a-denylist)进行比对检查，这为 IPv6 连接提供了额外的保护层，因为这些连接可能没有反向 DNS 记录。
