...
* [Могу ли я обеспечить обработку и хранение моей электронной почты в ЕС (размещение данных)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency)
* [Где обрабатывается и хранится входящая почта для моего домена и как долго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long)
* [Как указать Forward Email в качестве субобработчика в моем DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
* [Как увеличить объем хранилища или лимит исходящей отправки SMTP](#how-do-i-increase-my-storage-or-outbound-smtp-sending-limit)
* [Что входит в Enterprise License](#what-is-included-in-the-enterprise-license)
...
> \[!TIP]
> В ближайшее время мы можем объявить о нашем дата-центре в ЕС, размещенном на [forwardemail.eu](https://forwardemail.eu). Подписывайтесь на обсуждение по адресу <https://github.com/orgs/forwardemail/discussions/336> для обновлений.  На сегодня нет развертывания только в ЕС или возможности добровольного подключения — см. [Могу ли я обеспечить обработку и хранение моей электронной почты в ЕС (размещение данных)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).
...
### Поддерживаете ли вы фильтрацию электронной почты Sieve {#do-you-support-sieve-email-filtering}

Да. Forward Email поддерживает серверную фильтрацию [Sieve](https://en.wikipedia.org/wiki/Sieve_\(mail_filtering_language\)) на основе [RFC 5228](https://datatracker.ietf.org/doc/html/rfc5228). Скрипты фильтруют входящие сообщения до их доставки в почтовый ящик. Скрипт отклоняется, если он запрашивает недоступную возможность или использует расширение, не объявив его в `require`.

Полный список возможностей, подтверждённый реализацией, и заметки по RFC доступны в [документации протокола Sieve](/blog/docs/email-protocols-rfc-compliance-imap-smtp-pop3-comparison#sieve-rfc-5228).

#### Доступные возможности Sieve

| Категория | Возможности и поведение |
| --- | --- |
| Основной язык | `keep`, `discard`, `stop`, условные блоки и базовые тесты `address`, `header`, `exists`, `size` и логические тесты. |
| Доставка | `fileinto`, `copy`, `redirect`, `mailbox` для `fileinto :create`, и `special-use` для сопоставления стандартных папок `:specialuse` и `specialuse_exists`. |
| Тесты и сравнения | `envelope`, `body`, `date`, `index`, `regex`, `subaddress`, `relational`, `i;ascii-casemap`, и `i;octet`. |
| Состояние и переменные | `variables`, `imap4flags`, `duplicate`, и `ihave`. |
| Действия и ответы | `reject`, `ereject`, `vacation`, `vacation-seconds`, и `enotify` с использованием `mailto:`. Устаревшее объявление `require "notify"` принимается как входной алиас для `enotify`, но рекламируемой возможностью является `enotify`. |
| Обработка сообщений | `editheader`, `environment`, и `mime`, включая `foreverypart`, `break`, `extracttext`, и `replace`. |

`redirect` отправляется через обычную очередь исходящих сообщений. Оно подчиняется настроенной политике доменов для перенаправления, проверкам denylist и ограничениям по скорости. `editheader` не может изменять защищённые заголовки аутентификации или маршрутизации доставки.

#### Функции, которые не поддерживаются

| Функция | Причина |
| --- | --- |
| `enclose` | Создание нового сообщения, которое включает исходное сообщение, не реализовано. |
| `mailboxexists` | Forward Email не выполняет живых запросов состояния почтовых ящиков по IMAP. `fileinto :create` по-прежнему доступен через `mailbox`. |
| `include` | Глобальное и включаемое хранилище скриптов недоступно. |
| `mboxmetadata` / `servermetadata` | Интеграция IMAP METADATA недоступна. |
| `fcc` | Интеграция сохранения отправленных сообщений недоступна. |
| `encoded-character` | Синтаксис `${hex:...}` не реализован. |
| Внешние списки | Операции `valid_ext_list` и другие операции с внешними списками недоступны. |

#### Примеры Sieve-скриптов

**Разместить рассылки в папке:**
...
**Автоответ во время отсутствия на месте:**
...
**Создать папку при сортировке сообщения:**
...
Вы можете управлять Sieve-скриптами несколькими способами:

1. **Веб-интерфейс**: Перейдите в <a href="/my-account/domains" target="_blank" rel="noopener noreferrer" class="alert-link">Мой аккаунт <i class="fa fa-angle-right"></i> Домены</a> <i class="fa fa-angle-right"></i> Алиасы <i class="fa fa-angle-right"></i> Скрипты Sieve, чтобы создавать и управлять скриптами.

2. **Протокол ManageSieve**: Подключитесь с помощью любого клиента, совместимого с ManageSieve, например, аддона Sieve для Thunderbird или [sieve-connect](https://github.com/philpennock/sieve-connect), к `imap.forwardemail.net`. Используйте порт `2190` с STARTTLS или порт `4190` с неявным TLS.

3. **API**: Используйте [REST API](/api#sieve-scripts) для управления скриптами программно.
...
<div class="alert my-3 alert-primary">
  ...
  <span>
    Фильтрация Sieve применяется к входящим сообщениям перед доставкой в почтовый ящик. Скрипты выполняются в порядке приоритета, и первое подходящее действие определяет, как будет обработано сообщение.
  </span>
</div>
...
<div class="alert my-3 alert-warning">
  ...
  <span>
    Переадресации проверяются в соответствии с настроенной политикой и ограничениями по скорости. Автоответы (vacation) и уведомления ограничены по частоте, чтобы предотвратить злоупотребления.
  </span>
</div>
...
Мы вводим исходящие SMTP-лимиты на нескольких уровнях, чтобы предотвратить злоупотребления и при этом сохранять гибкость для законного использования. Каждый уровень проверяется по порядку — тот предел, который достигнут первым, временно отклонит сообщение с ошибкой `421` (что означает "попробуйте позже").

**Rate limit hierarchy:**

| Level | Scope | Default Limit | Description |
| :---- | :---- | :-----------: | :---------- |
| Per-alias | Конкретный алиас | None (uses domain limit) | Необязательно. Если у алиаса задан пользовательский `smtp_limit`, он проверяется первым. |
| Per-domain | Все письма, отправленные с домена в течение дня | 300/day | Учитывает все исходящие письма по всем алиасам домена. |
| Per-user | Все письма, отправленные учетной записью пользователя за день | 300/day | Предотвращает обход ограничений путем удаления и повторного создания алиасов или доменов. |

**How the effective limit is determined:**

* **Team plan domains** — фактический суточный лимит равен наибольшему `smtp_limit` среди всех администраторов домена. Например, если у одного администратора лимит 300, а у другого 500, эффективный лимит домена будет 500.
* **Enhanced Protection and other plans** — фактический суточный лимит — это собственный `smtp_limit` отправляющего пользователя (по умолчанию 300 сообщений в день).
* **Per-alias override** — администраторы домена могут при желании установить пользовательский `smtp_limit` для отдельных алиасов. Если он установлен, он проверяется первым (до доменного и пользовательского лимитов). Это полезно для ограничения объема отправки конкретных алиасов.
* **Значение по умолчанию для новых алиасов домена** — администраторы домена могут задать `alias_default_smtp_limit` для домена (через API или Advanced Settings в панели управления). При задании все вновь созданные алиасы на этом домене автоматически унаследуют это значение как их `smtp_limit`. Оно не может превышать эффективного SMTP‑лимита домена. Существующие алиасы не затрагиваются. Установите в `0`, чтобы отключить.

**System administrators** (Forward Email staff) освобождены от всех лимитов.

Все ограничения скоринга выполняются с использованием подсчёта в базе данных (`Emails.countDocuments`) по письмам, созданным с начала текущего дня (полуночи по UTC). Это означает, что ваш лимит сбрасывается ежедневно в полночь по UTC.

Если вам нужен более высокий лимит, пожалуйста, [свяжитесь с нами](https://forwardemail.net/help). Большинство запросов обрабатываются в течение 1–2 часов.
...
* **Принудительная аутентификация электронной почты**: Сообщения от отправителей, не входящих в список разрешённых, должны пройти хотя бы SPF или DKIM (аналогично требованиям Gmail, Outlook и Yahoo с 2024 года). Сообщения без какой-либо успешной аутентификации отклоняются с кодом ошибки 550. Применяются политики DMARC `p=reject` и `p=quarantine`.
* **Проверка HELO/EHLO по списку блокировки**: Имя хоста, представленное при SMTP HELO/EHLO приветствии, проверяется по нашему списку блокировки, обеспечивая защиту даже когда у IPv6-отправителей отсутствуют обратные DNS-записи
...
* **Контролируемое использование swap**: Хосты приложений и другие хосты без баз данных остаются без swap, а их размер и мониторинг настроены так, чтобы обычные рабочие нагрузки оставались в оперативной памяти. Хосты MongoDB и Redis используют файл подкачки только для root с `vm.swappiness=1` строго в качестве страховки от нехватки памяти, а не как обычную емкость. Повсеместное отключение swap может превратить кратковременную нехватку памяти базы данных в немедленное завершение процесса OOM. Активность swap базы данных отслеживается и расследуется.
...
* Подписанная обеими сторонами копия, а также индивидуальные условия DPA, доступны через [Enterprise License](#what-is-included-in-the-enterprise-license)
* Чтобы указать нас в собственном приложении о субобработчиках, см. [Как указать Forward Email в качестве субобработчика в моем DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
### Могу ли я сохранить обработку и хранение электронной почты в ЕС (локализация данных) {#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency}

> [!IMPORTANT]
> Пока нет. В настоящее время недоступны развертывание только в ЕС или опция выбора ЕС, и вы не должны описывать Forward Email как обработчика только в ЕС в вашей документации или соглашении об обработке данных.

**Где происходит обработка сегодня:** наши серверы в основном расположены в Denver, Colorado, United States (см. [Где расположены ваши серверы](#where-are-your-servers-located) и <https://forwardemail.net/ips>). Это относится ко всем функциям: входящая переадресация, журналы ошибок, хранение в почтовом ящике (IMAP/POP3/CalDAV/CardDAV) и исходящий SMTP.

**Что запланировано:** датацентр в ЕС в Амстердаме, размещенный на [forwardemail.eu](https://forwardemail.eu). Чтобы данные из ЕС были действительно изолированы — юридически и технически — работа должна вестись через отдельную компанию, базирующуюся в ЕС, а не только через серверы, находящиеся в ЕС. Даты запуска пока нет, и запись в список ожидания не меняет того, где обрабатывается ваша почта сегодня. Подпишитесь на <https://github.com/orgs/forwardemail/discussions/336> для объявлений; там мы опишем, как существующие клиенты смогут подключиться, когда это станет доступно.

**Что вы можете корректно указывать сегодня:**

* Обработчик: Forward Email LLC, компания, зарегистрированная в Delaware, United States
* Место обработки и хранения: United States
* Механизм передачи и гарантии: как описано на наших страницах [DPA](/dpa) и [GDPR](/gdpr)
* Наши собственные субобработчики: перечислены на нашей странице [GDPR](/gdpr) (все провайдеры из США)

См. [Где обрабатывается и хранится входящая почта для моего домена и как долго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) для разбивки по функциям и [Как мне указать Forward Email как субобработчика в моем DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa) для готовой к копированию записи в приложении.

### Где обрабатывается и хранится входящая почта для моего домена и как долго {#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long}

Вся обработка и хранение происходят на наших серверах в Denver, Colorado, United States. То, что сохраняется и на какой срок, зависит от функции:

| Функция | Что хранится | Срок хранения |
| --- | --- | --- |
| **Переадресация электронной почты** (алиас, который пересылает на другой адрес или вебхук) | Ничего. Сообщение обрабатывается полностью в памяти и ретранслируется на адресат; оно никогда не записывается на диск или в базу данных. | Нет — сообщение удаляется сразу после попытки доставки |
| **SMTP-журналы ошибок** (`4xx`/`5xx` ответы) | SMTP-ошибка, конверт и заголовки письма. **Не** сохраняются тело сообщения или вложения. | 7 дней (см. [Вы храните журналы ошибок](#do-you-store-error-logs)) |
| **Антиабузные кэши** (greylist, denylist, ограничение частоты) | Ключи, полученные из данных отправителя и получателя (адреса, домены, хэши), без содержимого сообщений. | До 30 дней (см. [У вас есть greylist](#do-you-have-a-greylist) и [У вас есть denylist](#do-you-have-a-denylist)) |
| **Хранилище почтового ящика** (IMAP/POP3/CalDAV/CardDAV, когда включено для алиаса) | Сообщения, контакты и календари в индивидуально зашифрованном почтовом ящике SQLite, который может открыть только ваш пароль. | До тех пор, пока вы не удалите сообщение, алиас, домен или аккаунт (см. [Как экспортировать и сделать резервную копию моего почтового ящика](#how-do-i-export-and-backup-my-mailbox)) |
| **Исходящий SMTP** | Записи очереди для доставки и отчетности об ошибках. Тела сообщений по умолчанию хранятся 0 дней после успешной доставки или постоянной ошибки; вы можете увеличить этот срок до максимум 30 дней на домен. | ~30 дней для записей очереди (см. нашу [Политику конфиденциальности](/privacy)) |

Две вещи, которые стоит указать в вашей документации:

* Пересланное сообщение доставляется в почтовый ящик назначения, который вы настроили (например, Google Workspace или Microsoft 365). То, что хранит этот провайдер, регулируется вашим соглашением с ним, а не нами.
* Доменные и учетные записи (ваш адрес электронной почты, имена доменов, алиасы, платежные данные) хранятся до тех пор, пока существует ваш аккаунт. Полный список см. в нашей [Политике конфиденциальности](/privacy).

Источники: <https://forwardemail.net/privacy>, <https://forwardemail.net/technical-whitepaper.pdf> и MX-обработчик в <https://github.com/forwardemail/forwardemail.net/blob/master/helpers/on-data-mx.js>.

### Как мне указать Forward Email как субобработчика в моем DPA {#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa}

Если вы являетесь контроллером (или сами обработчиком) по статье 28 GDPR и используете Forward Email для обработки почты ваших клиентов или пользователей, вам, как правило, нужно указать нас в собственном приложении о субобработчиках. Ниже — запись, которую написали бы мы:

| Поле | Значение |
| --- | --- |
| **Субобработчик** | Forward Email LLC |
| **Зарегистрирован в** | State of Delaware, United States |
| **Сервис** | Переадресация электронной почты (входящий MX) и, где включено, зашифрованное хранилище почтового ящика (IMAP/POP3/CalDAV/CardDAV) и исходящий SMTP |
| **Обрабатываемые персональные данные** | Адреса электронной почты, метаданные конверта и заголовков, а также содержимое сообщений в транзите (только в памяти для переадресации; шифрование на диске для хранилища почтового ящика) |
| **Место обработки** | United States (Denver, Colorado) |
| **Срок хранения** | См. [Где обрабатывается и хранится входящая почта для моего домена и как долго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) |
| **Механизм передачи** | Как указано на наших страницах [Data Processing Agreement](/dpa) и [GDPR](/gdpr) |
| **Наши субобработчики** | Перечислены на нашей странице [GDPR](/gdpr) и в [DPA](/dpa) (в настоящее время Cloudflare, DataPacket, DigitalOcean, GitHub, Vultr, Stripe и PayPal) |
| **Представитель в ЕС (ст. 27)** | Osano International Compliance Services Limited, Dublin, Ireland — полный адрес на нашей странице [GDPR](/gdpr) |
| **Представитель в Великобритании** | Osano UK Compliance LTD, Belfast — полный адрес на нашей странице [GDPR](/gdpr) |
| **Соглашение об обработке данных** | <https://forwardemail.net/dpa> (принимается вместе с нашими [Условиями](/terms); контрассигнованный экземпляр предоставляется в рамках Корпоративной лицензии) |
| **Контакт по вопросам безопасности** | <security@forwardemail.net> |

> [!WARNING]
> Наши [Условия](/terms) не допускают работу сервиса, который позволяет третьим лицам (кому-либо, кроме ваших сотрудников и подрядчиков) получать доступ к Forward Email, использовать его или извлекать из него выгоду без корпоративного соглашения. Если вы указываете нас как субобработчика, потому что обрабатываете почту ваших клиентов, вам нужна [Корпоративная лицензия](#what-is-included-in-the-enterprise-license) — это также способ получить контрассигнованный DPA.
...
* `HELO` – это приветствие для идентификации FQDN отправителя, IP-адреса или имени почтового обработчика. Это значение может быть подделано, поэтому мы не полагаемся на эти данные, а используем обратный поиск имени хоста по IP-адресу соединения. Однако теперь мы также проверяем это значение по нашему [списку блокировки](#do-you-have-a-denylist) в дополнение к обратному поиску имени хоста, что обеспечивает дополнительный уровень защиты, особенно для IPv6-соединений, где обратные DNS-записи могут быть недоступны.
...
Перейдите в <a href="/my-account/billing" target="_blank" rel="noopener noreferrer">Моя учетная запись → Выставление счетов</a> и прокрутите до раздела **Дополнения**, в котором есть две формы запроса:

* **Расширение хранилища** — выберите объем для добавления (+10, +20, +30, +40 или +50 GB), или выберите "Other", чтобы запросить произвольный объем.
* **Повышение лимита исходящего SMTP** — выберите количество для добавления (+1000, +2000 или +3000 писем ежедневно), или выберите "Other", чтобы запросить произвольное количество.

Отправка любой из форм направляет ваш запрос нашей команде на рассмотрение — с вас не будет списана оплата немедленно. После одобрения мы отправим вам по электронной почте защищенную платежную ссылку для завершения обновления. Одновременно у вас может быть только один ожидающий запрос каждого типа (хранилище или SMTP); повторная отправка в течение 3 дней после предыдущего запроса того же типа не допускается до окончания этого периода.


### Что входит в Корпоративную лицензию {#what-is-included-in-the-enterprise-license}

Корпоративная лицензия стоит **$250/month**, оформляется и выставляется счет напрямую, а не покупается через страницу с тарифами. Она включает все в тарифе Team плюс:

* **Исключение из Условий использования для третьих лиц** — вы можете запускать сервис или интеграцию, где ваши клиенты, студенты или заказчики (не только ваши сотрудники и подрядчики) используют Forward Email или получают от него выгоду. Наши стандартные [Условия](/terms) этого не допускают.
* **Контрассигнованное корпоративное соглашение и DPA** — отправляется на рассмотрение и подпись через DocuSign. Это способ получить подписанное [Data Processing Agreement](/dpa), а не копию, принимаемую вместе с нашими Условиями.
* **Индивидуальные договорные условия**, где мы можем их поддержать, например SLA по уведомлению о нарушениях (см. [Как вы обрабатываете уведомления о нарушениях данных](#how-do-you-handle-data-breach-notifications)).
* **Неограниченные запросы к API** без ограничения частоты.
* **Выделенный чат поддержки клиентов** с нашей инженерной командой.
* **Помощь при онбординге и поддержка инженеров в реальном времени.**

Это не меняет место обработки ваших данных: см. [Могу ли я сохранить обработку и хранение электронной почты в ЕС (локализация данных)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).

**Кому это нужно:** всем, чье использование сервиса выходит за рамки Условий (доступ третьих лиц), и всем, кому требуется контрассигнованный DPA, индивидуальные условия DPA или указанный инженерный контакт для целей соответствия.

**Как начать:** [свяжитесь с нами](/help), указав ваше юридическое название компании, зарегистрированный адрес, регистрационный номер компании и номер НДС, а также имя, должность и адрес электронной почты лица, которое будет подписывать. Мы ответим соглашением DocuSign для ознакомления.
