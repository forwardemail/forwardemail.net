...
### Что такое история пересылки писем {#what-is-forward-emails-history}
...
12. При появлении запроса «Пароль» вставьте из буфера обмена пароль, сгенерированный вами на шаге 2 выше.
...
Мы регулярно отслеживаем соответствие нашего [IP-адреса](#what-are-your-servers-ip-addresses) [все авторитетные списки запрещенных DNS](#how-do-you-handle-your-ip-addresses-becoming-blocked), **поэтому это, скорее всего, проблема, связанная с репутацией домена**.
...
* Перейдите в раздел <a href="/my-account/domains" class="alert-link" target="_blank" rel="noopener noreferrer">Моя учётная запись <i class="fa fa-angle-right"></i> Домены</a> <i class="fa fa-angle-right"></i> Настройки <i class="fa fa-angle-right"></i> Настройка исходящего SMTP и следуйте инструкциям по настройке. * Автоответчик не может быть включен для глобальных доменных имен (например, [одноразовые адреса](/disposable-addresses) не поддерживается).

* Автоответчик не может быть включен для псевдонимов с подстановочными знаками/универсальными адресами (`*`) или регулярными выражениями.

В отличие от почтовых систем, таких как `postfix` (например, которые используют расширение фильтра отпусков `sieve`), Forward Email автоматически добавляет вашу подпись DKIM, обеспечивает фиктивные защиты от проблем с подключением при отправке ответов об отпуске (например, из-за распространенных проблем с подключением SSL/TLS и устаревших поддерживаемых серверов) и даже поддерживает шифрование Open WKD и PGP для ответов об отпуске.
...
1. Мы отправляем письма только одному отправителю [в разрешенном списке](#do-you-have-an-allowlist) раз в 4 дня (что аналогично поведению Gmail).

* В нашем кэше Redis используются отпечатки `alias_id` и `sender`, где `alias_id` — это псевдоним идентификатора MongoDB, а `sender` — это либо адрес отправителя (если он внесён в разрешённый список), либо корневой домен в адресе отправителя (если он не внесён в разрешённый список). Для простоты срок действия этого отпечатка в кэше установлен на 4 дня.

* Наш подход, заключающийся в использовании корневого домена, проанализированного в поле «От кого» для отправителей, не внесенных в список разрешенных, предотвращает злоупотребления со стороны относительно неизвестных отправителей (например, злоумышленников), которые заполняют сообщениями службу поддержки во время отпуска.

2. Мы отправляем сообщения только в том случае, если поле MAIL FROM и/или From не пустое и не содержит (без учета регистра) [имя пользователя почтмейстера](#what-are-postmaster-addresses) (часть перед @ в письме).
...
5. Мы не отправляем сообщение, если часть имени пользователя в поле «От» адреса электронной почты имела вид `mdaemon` и заголовок `X-MDDSN-Message` нечувствителен к регистру.

6. Мы не отправляем данные, если в заголовке `content-type` присутствует регистронезависимый заголовок `multipart/report`.
...
<span>
Если вы используете Microsoft Outlook или Live.com, вам необходимо добавить <code>include:spf.protection.outlook.com</code> к вашей записи SPF <strong class="notranslate">TXT</strong>, например:
<br /><br />
...
**Чтобы настроить контакты, используйте URL-адрес CardDAV:** `https://carddav.forwardemail.net` (или просто `carddav.forwardemail.net`, если ваш клиент это разрешает)
...
Если вы хотите добавить дополнительные календари, просто добавьте новый URL-адрес календаря: `https://caldav.forwardemail.net/dav/principals/calendar-name` (**обязательно замените `calendar-name` на желаемое имя календаря**)

Вы можете изменить название и цвет календаря после его создания — просто используйте предпочитаемое вами приложение для работы с календарем (например, Apple Mail или [Тандербёрд](https://thunderbird.net)).
...
Более подробную информацию можно получить по ссылке [наш раздел о MTA-STS](#do-you-support-mta-sts).

### Как добавить фотографию профиля к моему адресу электронной почты {#how-do-i-add-a-profile-picture-to-my-email-address}

Если вы используете Gmail, выполните следующие действия:

1. Перейдите по адресу <https://google.com> и выйдите из всех учётных записей электронной почты.
...
6. Введите адрес электронной почты, соответствующий вашему доменному имени.
...
11. Перейдите по адресу <https://google.com> и в правом верхнем углу нажмите на значок своего профиля, а затем на кнопку «Изменить».
12. Загрузите новую фотографию или аватар для своей учётной записи.
13. Изменения вступят в силу примерно через 1–2 часа, но иногда это происходит очень быстро. 14. Отправьте тестовое электронное письмо, и фотография профиля должна появиться.
...
Да, вы можете прочитать больше по адресу <https://forwardemail.net/guides/newsletter-with-listmonk>.

Обратите внимание, что для поддержания репутации IP-адреса и обеспечения доставки в Forward Email применяется ручная проверка для каждого домена, **предназначенная для одобрения новостной рассылки**. Напишите на адрес <support@forwardemail.net> или откройте [запрос помощи](https://forwardemail.net/help) для одобрения. Обычно это занимает менее 24 часов, большинство запросов обрабатывается в течение 1-2 часов. В ближайшем будущем мы планируем сделать этот процесс мгновенным, добавив дополнительные средства защиты от спама и оповещения. Это гарантирует, что ваши письма попадут во входящие, а не будут помечены как спам.
...
Пожалуйста, просмотрите раздел [Электронные письма](/email-api#outbound-emails) в нашей документации по API для получения информации о параметрах, примерах и более подробной информации.

Чтобы отправлять исходящую электронную почту с помощью нашего API, вам необходимо использовать свой токен API, доступный по ссылке [Моя безопасность](/my-account/security).

### Поддерживаете ли вы получение электронной почты по протоколу IMAP {#do-you-support-receiving-email-with-imap}

Да, с 16 октября 2023 года мы поддерживаем получение электронной почты по протоколу IMAP в качестве дополнения для всех платных пользователей. **Пожалуйста, прочтите нашу подробную статью** по ссылке [как работает наша функция зашифрованного хранилища почтовых ящиков SQLite](/blog/docs/best-quantum-safe-encrypted-email-service).
...
5. При запросе порта сервера IMAP введите `993` (SSL/TLS) – при необходимости см. [альтернативные порты IMAP](/faq#what-are-your-imap-server-configuration-settings)
<div class="alert my-3 alert-warning">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong class="font-weight-bold">
Совет:
...
6. При запросе пароля сервера IMAP вставьте пароль из раздела <strong class="text-success"><i class="fa fa-key"></i> «Сгенерировать пароль»</strong>, указанного в шаге 2 выше.
...
Да, мы добавили эту функцию 5 февраля 2024 года. Наш сервер имеет код `caldav.forwardemail.net` и также отслеживается на нашей <a href="https://status.forwardemail.net" target="_blank" rel="noopener noreferrer">странице статуса</a>.
...
Да, с 12 июня 2025 года мы добавили эту функцию. Наш сервер имеет код `carddav.forwardemail.net` и также отслеживается на нашей <a href="https://status.forwardemail.net" target="_blank" rel="noopener noreferrer">странице статуса</a>.
...
| --------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Тандербёрд | Рабочий стол | [Configure OpenPGP in Thunderbird](https://support.mozilla.org/en-US/kb/openpgp-thunderbird-howto-and-faq#w_i-have-never-used-openpgp-with-thunderbird-before-how-do-i-setup-openpgp) | Thunderbird имеет встроенную поддержку OpenPGP. |
...
| Почта Apple | macOS | [Free-GPGMail](https://github.com/Free-GPGMail/Free-GPGMail?tab=readme-ov-file#installation) | Apple Mail не поддерживает OpenPGP, однако вы можете загрузить плагин с открытым исходным кодом [Free-GPGMail](https://github.com/Free-GPGMail/Free-GPGMail?tab=readme-ov-file#installation). |
| Почта Apple | iOS | [PGPro](https://github.com/opensourceios/PGPro/) или [FlowCrypt](https://apps.apple.com/us/app/flowcrypt-encrypted-email/id1591754995) (собственная лицензия) | Apple Mail не поддерживает OpenPGP, однако вы можете загрузить плагин с открытым исходным кодом [PGPro](https://github.com/opensourceios/PGPro/) или [FlowCrypt](https://flowcrypt.com/download). |
...
3. Загрузите свой открытый ключ по адресу <https://keys.openpgp.org/upload>.
...
Да! С 13 декабря 2023 года мы добавили поддержку паролей [из-за высокого спроса](https://github.com/orgs/forwardemail/discussions/182).
...
### Поддерживаете ли вы отказы веб-перехватчиков {#do-you-support-bounce-webhooks}?
...
Ищете документацию по вебхукам для электронной почты? Подробнее см. в разделе <a href="/faq#do-you-support-webhooks" class="alert-link">Поддерживаете ли вы вебхуки?</a>.
<span>
</span>
</div>

Да, с 14 августа 2024 года мы добавили эту функцию. Теперь вы можете перейти в раздел «Моя учётная запись» → «Домены» → «Настройки» → «URL-адрес возврата веб-перехватчика» и настроить URL-адрес `http://` или `https://`, на который будет отправляться запрос `POST` при возврате исходящих SMTP-писем.
...
* `list_id` (строка) — значение заголовка `List-ID` (без учёта регистра), если есть, из исходного исходящего письма
* `list_unsubscribe` (строка) — значение заголовка `List-Unsubscribe` (без учёта регистра), если есть, из исходного исходящего письма
* `feedback_id` (строка) — значение заголовка `Feedback-ID` (без учёта регистра), если есть, из исходного исходящего письма
* `recipient` (строка) — адрес электронной почты получателя, письмо которого вернулось или доставлено с ошибкой.
* `message` (строка) — подробное сообщение об ошибке возврата.
* `response` (строка) — сообщение об ответе SMTP.
* `response_code` (число) — проанализированный код ответа SMTP.
* `truth_source` (строка) — если код ответа получен из надежного источника, это значение будет заполнено именем корневого домена (например, `google.com` или `yahoo.com`).
* `bounce` (Объект) — объект, содержащий следующие свойства, которые подробно описывают статусы возврата и отклонения:
* `action` (Строка) — действие при возврате (например, `"reject"`)
* `message` (Строка) — причина возврата (например, `"Message Sender Blocked By Receiving Server"`)
* `category` (Строка) — категория возврата (например, `"block"`)
* `code` (Число) — код статуса возврата (например, `554`)
* `status` (строка) — код возврата из ответного сообщения (например, `5.7.1`)
* `line` (число) — номер проанализированной строки, если таковой имеется, [из списка разбора отказов Zone-MTA](https://github.com/zone-eu/zone-mta/blob/master/config/bounces.txt) (например, `526`)
* `headers` (объект) — пара «ключ-значение» заголовков исходящего электронного письма
* `bounced_at` (строка) — отформатированная дата возникновения ошибки возврата [ISO 8601](https://en.wikipedia.org/wiki/ISO\_8601)
...
* Если полезная нагрузка вебхука содержит значение `list_id`, `list_unsubscribe` или `feedback_id`, то при необходимости следует предпринять соответствующие действия для удаления `recipient` из списка.
* Если значение `bounce.category` было равно `"block"`, `"recipient"`, `"spam"` или `"virus"`, то пользователя следует обязательно удалить из списка.
* Если вам нужно проверить полезную нагрузку вебхука (чтобы убедиться, что она действительно поступает с нашего сервера), вы можете использовать [разрешить IP-адрес удаленного клиента и имя хоста клиента с помощью обратного поиска](https://nodejs.org/api/dns.html#dnspromisesreverseip) – это должен быть `smtp.forwardemail.net`.
...
* Рассчитайте и сравните значение `X-Webhook-Signature` из нашего запроса вебхука с вычисленным значением тела запроса, используя этот ключ. Пример того, как это сделать, доступен по адресу [этот пост на Stack Overflow](https://stackoverflow.com/a/68885281). * Подробнее см. обсуждение по адресу <https://github.com/forwardemail/free-email-forwarding/issues/235>.
* Мы будем ждать ответа вашей конечной точки вебхука с кодом статуса `200` в течение `5` секунд и повторим попытку в течение `1` времени.
* Если при попытке отправить запрос на ваш URL-адрес вебхука, возвращающий ошибку, мы будем отправлять вам электронное письмо с уведомлением раз в неделю.

### Поддерживаете ли вы веб-перехватчики {#do-you-support-webhooks}?
...
Ищете документацию по веб-хукам Bounce? Подробнее см. в разделе <a href="/faq#do-you-support-bounce-webhooks" class="alert-link">Поддерживаете ли вы веб-хуки Bounce?</a>.
<span>
</span>
</div>
...
Например, если я хочу, чтобы все электронные письма, отправляемые на `alias@example.com`, пересылались на новую тестовую конечную точку [запросить корзину](https://requestbin.com/r/en8pfhdgcculn?inspect):
...
Или, возможно, вы хотите, чтобы все электронные письма, отправляемые на адрес `example.com`, пересылались на эту конечную точку:
...
* Если вам нужно проверить полезную нагрузку вебхука (чтобы убедиться, что она действительно поступает с нашего сервера), вы можете использовать [разрешить IP-адрес удаленного клиента и имя хоста клиента с помощью обратного поиска](https://nodejs.org/api/dns.html#dnspromisesreverseip) – это должен быть либо `mx1.forwardemail.net`, либо `mx2.forwardemail.net`.
* Вы также можете проверить IP-адрес по [наши опубликованные IP-адреса](#what-are-your-servers-ip-addresses).
...
* Вы можете ротировать этот ключ в любое время в целях безопасности.
* Рассчитайте и сравните значение `X-Webhook-Signature` из нашего запроса вебхука с вычисленным значением тела запроса, используя этот ключ. Пример того, как это сделать, доступен по адресу [этот пост на Stack Overflow](https://stackoverflow.com/a/68885281).
* Подробнее см. обсуждение по адресу <https://github.com/forwardemail/free-email-forwarding/issues/235>.
* Если вебхук не отвечает кодом статуса `200`, мы сохраним его ответ в [создан журнал ошибок](#do-you-store-error-logs), что полезно для отладки.
* HTTP-запросы вебхука будут повторяться до 3 раз при каждой попытке SMTP-подключения с максимальным таймаутом 60 секунд на каждый POST-запрос конечной точки. **Обратите внимание, что это не означает, что он повторяет только 3 раза**, на самом деле он будет повторяться непрерывно, отправляя SMTP-код 421 (что указывает отправителю на необходимость повторной попытки позже) после третьей неудачной попытки HTTP POST-запроса. Это означает, что электронное письмо будет повторяться непрерывно в течение нескольких дней, пока не будет достигнут код статуса 200.
* Мы будем автоматически повторять попытки, основываясь на кодах состояния и ошибок по умолчанию, используемых в [метод повторных попыток суперагента](https://ladjs.github.io/superagent/#retrying-requests) (мы являемся разработчиками).
* Мы группируем HTTP-запросы веб-перехватчиков к одной и той же конечной точке в один запрос вместо нескольких (вместо нескольких) для экономии ресурсов и ускорения ответа. Например, если вы отправляете электронное письмо на адреса <webhook1@example.com>, <webhook2@example.com> и <webhook3@example.com>, и все они настроены на обращение к одному и тому же *точному* URL-адресу конечной точки, будет выполнен только один запрос. Мы группируем запросы по точному совпадению конечных точек со строгим равенством.
* Обратите внимание, что мы используем метод "simpleParser" библиотеки [почтовый парсер](https://nodemailer.com/extras/mailparser/) для преобразования сообщения в JSON-объект.
* Необработанное значение адреса электронной почты в виде строки задается свойством "raw".
* Результаты аутентификации представлены в виде свойств «dkim», «spf», «arc», «dmarc» и «bimi».
* Разобранные заголовки электронной почты представлены в виде свойства «headers», но обратите внимание, что для упрощения итерации и разбора можно использовать «headerLines».
* Сгруппированные получатели для этого веб-хука объединены в группу и представлены в виде свойства «recipients».
* Информация о сеансе SMTP представлена в виде свойства «session». Оно содержит информацию об отправителе сообщения, времени его получения, HELO и имени хоста клиента. Значение имени хоста клиента `session.clientHostname` представляет собой либо полное доменное имя (из обратного поиска PTR), либо `session.remoteAddress`, заключенное в скобки (например, `"[127.0.0.1]"`).
* Если вам нужен быстрый способ получить значение `X-Original-To`, вы можете использовать значение `session.recipient` (см. пример ниже). Заголовок `X-Original-To` — это заголовок, который мы добавляем к сообщениям для отладки с исходным получателем (до маскированной пересылки) сообщения.
* Если вам нужно удалить свойства `attachments` и/или `raw` из тела полезной нагрузки, просто добавьте `?attachments=false`, `?raw=false` или `?attachments=false&raw=false` в конечную точку веб-перехватчика в качестве параметра строки запроса (например, `https://example.com/webhook?attachments=false&raw=false`).
* Если есть вложения, они будут добавлены в массив `attachments` со значениями буфера. Вы можете преобразовать их обратно в контент, используя подход с JavaScript, например:
...
Мы поддерживаем два флага регулярных выражений, включая `i` и `g`. Флаг `i` без учёта регистра является постоянным значением по умолчанию и всегда применяется. Глобальный флаг `g` можно добавить, добавив к `/` `/g`.

Обратите внимание, что мы также поддерживаем нашу <a href="#can-i-disable-specific-aliases">disabled функцию псевдонима</a> для получателя с поддержкой регулярных выражений.
...
<div class="alert my-3 alert-secondary">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong>Пример подстановки фильтра с символом «плюс»:</strong> Если я хочу, чтобы все письма, отправленные на адрес `info@example.com` или `support@example.com`, пересылались на адрес `user+info@gmail.com` или `user+support@gmail.com` соответственно (с поддержкой подстановки) (<a href="https://regexr.com/66ho7" class="alert-link">просмотреть тест в RegExr</a>):
</div>
...
Хотите узнать, как написать регулярное выражение, или хотите протестировать свою замену? Вы можете перейти на бесплатный сайт тестирования регулярных выражений <a href="https://regexr.com" class="alert-link">RegExr</a> по адресу <a href="https://regexr.com/" class="alert-link">https://regexr.com</a>.
<span>
</span>
</div>

### Каковы ваши лимиты на исходящий SMTP-трафик {#what-are-your-outbound-smtp-limits}

Мы устанавливаем ограничение на количество исходящих SMTP-сообщений для пользователей и доменов — 300 в день. В среднем это более 9000 писем в месяц. Если вам необходимо превысить это количество или у вас постоянно большие письма, пожалуйста, [связаться с нами](https://forwardemail.net/help).

### Нужно ли мне одобрение для включения SMTP {#do-i-need-approval-to-enable-smtp}?

Да, обратите внимание, что для поддержания репутации IP-адреса и обеспечения доставки, Forward Email проходит ручную проверку для каждого домена при одобрении исходящих SMTP-сообщений. Напишите на адрес <support@forwardemail.net> или откройте [запрос помощи](https://forwardemail.net/help) для одобрения. Обычно это занимает менее 24 часов, большинство запросов обрабатывается в течение 1-2 часов. В ближайшем будущем мы планируем сделать этот процесс мгновенным, добавив дополнительные средства защиты от спама и оповещения. Это гарантирует, что ваши письма попадут во входящие, а не будут помечены как спам.

### Каковы настройки конфигурации вашего SMTP-сервера {#what-are-your-smtp-server-configuration-settings}

Наш сервер имеет код `smtp.forwardemail.net` и также отслеживается на нашей <a href="https://status.forwardemail.net" target="_blank" rel="noopener noreferrer">странице статуса</a>.

Он поддерживает как IPv4, так и IPv6 и доступен через порты `465` и `2465` для SSL/TLS и `587`, `2587`, `2525` и `25` для TLS (STARTTLS).
...
Пошаговые инструкции см. по ссылке [Поддерживаете ли вы отправку электронной почты через SMTP?](#do-you-support-sending-email-with-smtp).

### Каковы настройки конфигурации вашего сервера IMAP {#what-are-your-imap-server-configuration-settings}

Наш сервер имеет код `imap.forwardemail.net` и также отслеживается на нашей <a href="https://status.forwardemail.net" target="_blank" rel="noopener noreferrer">странице статуса</a>.

Он поддерживает как IPv4, так и IPv6 и доступен через порты `993` и `2993` для SSL/TLS.
...
Пошаговые инструкции см. по ссылке [Поддерживаете ли вы получение электронной почты по протоколу IMAP?](#do-you-support-receiving-email-with-imap).

### Каковы настройки конфигурации вашего POP3-сервера {#what-are-your-pop3-server-configuration-settings}

Наш сервер имеет код `pop3.forwardemail.net` и также отслеживается на нашей <a href="https://status.forwardemail.net" target="_blank" rel="noopener noreferrer">странице статуса</a>.

Он поддерживает как IPv4, так и IPv6 и доступен через порты `995` и `2995` для SSL/TLS.
...
Пошаговые инструкции см. по ссылке [Поддерживаете ли вы POP3?](#do-you-support-pop3).
...
Сторонняя проверка: <https://www.hardenize.com/report/forwardemail.net/1750312779> показывает оценку «Хорошо» для всех мер безопасности TLS и транспорта.
...
* **ARC**: Хотя точные данные не указаны, безупречные показатели соответствия сервиса предполагают комплексную обработку заголовков аутентификации
...
**Доказательства из кодовой базы MX**: Сервер MX обрабатывает электронные письма полностью в памяти, не записывая содержимое на диск. Основной обработчик электронной почты демонстрирует этот подход в памяти: <https://github.com/forwardemail/forwardemail.net/blob/master/helpers/on-data-mx.js>

Источники:

* <https://forwardemail.net/technical-whitepaper.pdf#page=10> (Аннотация)
* <https://forwardemail.net/technical-whitepaper.pdf#page=59> (Данные с нулевым разглашением)
...
* <https://forwardemail.net/technical-whitepaper.pdf#page=15> (Предотвращение атак Evil Maid)
...
* Соответствие требованиям PCI Merchant
* Сертификация CSA STAR Level 1
...
* **Stripe**: Сертифицировано по Data Privacy Framework — <https://stripe.com/legal/privacy-center>
* **PayPal**: Не сертифицировано по DPF — <https://www.paypal.com/uk/legalhub/privacy-full>

### Вы предлагаете Соглашение об обработке данных (DPA) {#do-you-offer-a-data-processing-agreement-dpa}
...
Для корпоративных клиентов, которым требуются особые соглашения об уровне обслуживания (SLA) по уведомлению о нарушениях, их следует обсудить в рамках соглашения о **корпоративной лицензии**.
...
**Обязательство по обеспечению бесперебойной работы**: доступность сервиса более 99,9% с прозрачным мониторингом, доступным по адресу <https://forwardemail.net>
...
Полную информацию о нашей системе соблюдения требований государственного регулирования, включая более широкие федеральные правила, можно найти в нашем комплексном исследовании: [Соответствие Федеральной правительственной электронной почты разделу 889](https://forwardemail.net/blog/docs/federal-government-email-service-section-889-compliant)

## Системные и технические сведения {#system-and-technical-details}
...
* `MAIL FROM` — это адрес отправителя конверта. Если значение указано, это должен быть действительный адрес электронной почты RFC 5322. Пустые значения разрешены. Мы используем [проверка на обратное рассеяние](#how-do-you-protect-against-backscatter) здесь, а также проверяем MAIL FROM на соответствие [список отрицаний](#do-you-have-a-denylist). Наконец, мы проверяем отправителей, не входящих в разрешённый список, на предмет ограничения скорости отправки (подробнее см. в разделе о [Ограничение скорости](#do-you-have-rate-limiting) и [белый список](#do-you-have-an-allowlist)).

* `RCPT TO` — здесь указывается получатель(и) письма. Это должны быть действительные адреса электронной почты RFC 5322. Мы допускаем не более 50 получателей в конверте на одно сообщение (это отличается от заголовка «Кому» в электронном письме). Мы также проверяем наличие действительного адреса [Схема перезаписи отправителя](https://en.wikipedia.org/wiki/Sender_Rewriting_Scheme) («SRS») для защиты от подмены с использованием нашего доменного имени SRS.
...
### Как обрабатывается электронное письмо для пересылки {#how-do-you-process-an-email-for-forwarding}

В этом разделе описывается наш процесс, связанный с командой протокола SMTP `DATA` в разделе [Как работает ваша система пересылки электронной почты?](#how-does-your-email-forwarding-system-work) выше, — то, как мы обрабатываем заголовки, тело, безопасность электронного письма, определяем, куда его следует доставить, и как мы обрабатываем соединения.
...
4. Используя отпечаток адреса электронной почты (см. раздел о [Дактилоскопирование](#how-do-you-determine-an-email-fingerprint)), мы проверим, предпринимались ли повторные попытки отправки сообщения в течение более 5 дней (что соответствует [поведение постфикса по умолчанию](http://www.postfix.org/postconf.5.html#maximal_queue_lifetime)), и если это так, сообщение будет отклонено с кодом ошибки 550.

5. Мы сохраняем в памяти результаты сканирования электронной почты с помощью [Спам-сканер](https://spamscanner.net).

6. Если спам-сканер выдал какие-либо произвольные результаты, они отклоняются с кодом ошибки 554. На момент написания статьи произвольные результаты включают только тест GTUBE. Подробнее см. <https://spamassassin.apache.org/gtube/>.
...
* Добавляется для каждого получателя во время пересылки по протоколу IMAP и/или маскированной пересылки (для защиты конфиденциальности).
...
* `X-Forward-Email-Sender` — список, разделенный запятыми, содержащий исходный адрес MAIL FROM конверта (если он не был пустым), полное доменное имя обратного PTR-клиента (если существует) и IP-адрес отправителя.
* `X-Forward-Email-ID` — применимо только для исходящих SMTP-сообщений и соответствует идентификатору электронной почты, сохранённому в разделе «Моя учётная запись» → «Электронная почта».
...
* Если сообщение не прошло проверку DMARC, а для домена установлена политика отклонения (например, `p=reject` [был в политике DMARC](https://wikipedia.org/wiki/DMARC)), оно отклоняется с кодом ошибки 550. Обычно политика DMARC для домена указана в записи <strong class="notranslate">TXT</strong> поддомена `_dmarc` (например, `dig _dmarc.example.com txt`).

* Если сообщение не прошло проверку SPF, а для домена установлена политика полного отказа (например, `-all` указана в политике SPF, а не `~all`, или политика вообще не указана), оно отклоняется с кодом ошибки 550. Обычно политика SPF для домена находится в записи <strong class="notranslate">TXT</strong> для корневого домена (например, `dig example.com txt`). Подробнее о SPF см. в этом разделе [отправка почты как с Gmail](#can-i-send-mail-as-in-gmail-with-this).
...
* Мы анализируем все записи <strong class="notranslate">TXT</strong>, которые начинаются с `forward-email=` (бесплатные тарифы) или `forward-email-site-verification=` (платные тарифы). Обратите внимание, что мы анализируем оба варианта для обработки электронных писем, пока пользователь повышает или понижает тарифный план.
* Из этих проанализированных записей <strong class="notranslate">TXT</strong> мы итеративно извлекаем конфигурацию пересылки (как описано в разделе [Как начать и настроить пересылку электронной почты](#how-do-i-get-started-and-set-up-email-forwarding) выше). Обратите внимание, что мы поддерживаем только одно значение `forward-email-site-verification=`, и если указано более одного, возникнет ошибка 550, и отправитель получит сообщение об отказе для этого получателя.
* Рекурсивно мы итеративно обрабатываем извлеченную конфигурацию пересылки, чтобы определить глобальную пересылку, пересылку на основе регулярных выражений и все другие поддерживаемые конфигурации пересылки, которые теперь называются нашими «адресами пересылки».
* Для каждого адреса пересылки мы поддерживаем один рекурсивный поиск (который запустит эту серию операций заново для указанного адреса). Если найдено рекурсивное совпадение, родительский результат будет удалён из списка адресов пересылки, а дочерние добавлены.
* Адреса пересылки проверяются на уникальность (так как мы не хотим отправлять дубликаты на один адрес или создавать дополнительные ненужные клиентские SMTP-подключения).
* Для каждого адреса пересылки мы ищем его доменное имя в нашей конечной точке API `/v1/max-forwarded-addresses` (чтобы определить, сколько адресов домену разрешено пересылать почту на каждый псевдоним, например, 10 по умолчанию — см. раздел о [максимальный лимит пересылки на псевдоним](#is-there-a-limit-on-the-number-of-email-addresses-i-can-forward-to-per-alias)). При превышении этого лимита возникнет ошибка 550, и отправитель получит сообщение об отказе для этого получателя.
* Мы ищем настройки исходного получателя в нашей конечной точке API `/v1/settings`, которая поддерживает поиск для платных пользователей (с резервным вариантом для бесплатных пользователей). Возвращает объект конфигурации для расширенных настроек для `port` (число, например, `25`), `has_adult_content_protection` (логическое значение), `has_phishing_protection` (логическое значение), `has_executable_protection` (логическое значение) и `has_virus_protection` (логическое значение).
* На основе этих настроек мы проверяем сообщение на наличие вирусов в результатах проверки спам-сканером, и если возникают какие-либо ошибки, сообщение отклоняется с кодом ошибки 554 (например, если включено `has_virus_protection`, мы проверяем результаты проверки спам-сканером на наличие вирусов). Обратите внимание, что все пользователи бесплатного тарифного плана будут включены для проверки на наличие контента для взрослых, фишинга, исполняемых файлов и вирусов. По умолчанию все пользователи платного тарифного плана также включены для проверки, но эту настройку можно изменить на странице настроек домена в панели управления пересылкой электронной почты.

10. Для каждого обработанного адреса пересылки получателя мы затем выполняем следующие операции:

* Адрес проверяется на соответствие нашей ссылке [список отрицаний](#do-you-have-a-denylist), и если он указан, возникает код ошибки 421 (указывает отправителю на необходимость повторить попытку позже).
...
12. Если есть получатели, мы перебираем их (группируя по одному хосту) и доставляем письма. Подробнее см. в разделе [Как вы решаете проблемы с доставкой электронной почты?](#how-do-you-handle-email-delivery-issues) ниже.

* Если при отправке электронных писем возникнут ошибки, мы сохраним их в памяти для последующей обработки.
* Мы возьмём наименьший код ошибки (если таковой имеется) из отправленных электронных писем и используем его в качестве кода ответа на команду `DATA`. Это означает, что не доставленные письма, как правило, будут повторно отправлены исходным отправителем, в то время как уже доставленные письма не будут повторно отправлены при следующей отправке (поскольку мы используем [Дактилоскопирование](#how-do-you-determine-an-email-fingerprint)).
* Если ошибок не произошло, мы отправим код статуса успешного ответа SMTP 250.
* Отказом считается любая попытка доставки, которая приводит к коду статуса >= 500 (постоянные сбои).
...
14. Если ошибки всё же возникли, мы будем отправлять уведомления о недоставке в фоновом режиме, вернув отправителю наименьший из всех кодов ошибок. Однако, если наименьший код ошибки >= 500, мы не будем отправлять уведомления о недоставке. Это связано с тем, что в противном случае отправители получат двойное уведомление о недоставке (например, одно от своего исходящего MTA, такого как Gmail, и одно от нас). Подробнее см. в разделе [Как защититься от обратного рассеяния?](#how-do-you-protect-against-backscatter) ниже.
...
Если получатель — вебхук, мы разрешим 60-секундный тайм-аут для выполнения запроса с возможностью до трёх повторных попыток (итого 4 запроса до сбоя). Обратите внимание, что мы корректно анализируем коды ошибок 408, 413 и 429 и сопоставляем их с кодом ответа SMTP 421.
...
Если возникнут какие-либо ошибки DNS или соединения, то мы вернем команде `DATA` код ответа SMTP 421, в противном случае, если будут ошибки уровня >= 500, будут отправлены сообщения об отказе.
...
Более подробную информацию о том, как подобные списки используются для создания эффективных систем электронной почты, можно найти по ссылке [RFC 5320 Раздел 4.6](https://datatracker.ietf.org/doc/html/rfc5230#section-4.6).
...
Мы публикуем наши IP-адреса по адресу <https://forwardemail.net/ips>.

### У вас есть белый список {#do-you-have-an-allowlist}

Да, у нас есть [список расширений доменных имен](#what-domain-name-extensions-are-allowlisted-by-default), которые по умолчанию занесены в белый список, а также динамический, кэшированный и обновляемый белый список, основанный на [строгие критерии](#what-is-your-allowlist-criteria).
...
Кроме того, следующие [брендовые и корпоративные домены верхнего уровня](https://en.wikipedia.org/wiki/List_of_Internet_top-level_domains#Brand_and_corporate_top-level_domains) по умолчанию включены в разрешенный список (например, `apple` для `applecard.apple` для банковских выписок по карте Apple Card):
...
По состоянию на 18 марта 2025 года мы также добавили в этот список следующие французские заморские территории ([по этому запросу GitHub](https://github.com/forwardemail/forwardemail.net/issues/327)):
...
* Корневой домен отправителя должен относиться к [расширение доменного имени, которое соответствует списку, который мы предлагаем в нашем бесплатном плане](#what-domain-name-extensions-can-be-used-for-free) (с добавлением `biz` и `info`). Мы также учитываем частичные совпадения `edu`, `gov` и `mil`, например, `xyz.gov.au` и `xyz.edu.au`.
* Корневой домен отправителя должен входить в число 100 000 лучших уникальных корневых доменов, проанализированных по запросу [Список популярности зонтов](http://s3-us-west-1.amazonaws.com/umbrella-static/index.html "Umbrella Popularity List") («UPL»).
* Корневой домен отправителя должен входить в число 50 000 лучших результатов поиска среди уникальных корневых доменов, появлявшихся как минимум 4 раза за последние 7 дней в UPL (\~50%+).

* Корневой домен отправителя не должен быть [категоризированный](https://radar.cloudflare.com/categorization-feedback/) как контент для взрослых или вредоносное ПО от Cloudflare.

* Корневой домен отправителя должен иметь либо запись A, либо запись MX.

* Корневой домен отправителя должен иметь либо запись A, либо запись MX, либо запись DMARC с квалификатором `p=reject` или `p=quarantine`, либо запись SPF с квалификатором `-all` или `~all`.
...
Да, у нас очень слабая политика ([серый список электронной почты](https://en.wikipedia.org/wiki/Greylisting_\(email\)). Серые списки применяются только к отправителям, не входящим в наш разрешённый список, и хранятся в нашем кэше в течение 30 дней.
...
Например, если письмо приходит с адреса `test.example.com` до того, как мы увидим письмо с адреса `example.com`, то любое письмо с адреса `test.example.com` и/или `example.com` будет ожидать 5 минут с момента первоначального подключения. Мы не заставляем адреса `test.example.com` и `example.com` ждать по отдельности (наша политика серых списков применяется на уровне корневого домена).

Обратите внимание, что серый список не применяется ни к одному отправителю на нашем [белый список](#do-you-have-an-allowlist) (например, Meta, Amazon, Netflix, Google, Microsoft на момент написания этой статьи).
...
1. [исходный отпечаток сообщения](#how-do-you-determine-an-email-fingerprint) добавляется в серый список при обнаружении спама или блокировке от «доверенного» отправителя (например, `gmail.com`, `microsoft.com`, `apple.com`).
* Если отправитель был внесён в разрешённый список, сообщение добавляется в серый список на 1 час.
* Если отправитель не внесён в разрешённый список, сообщение добавляется в серый список на 6 часов.
2. Мы анализируем ключи запрещённого списка из информации об отправителе и сообщении, и для каждого из этих ключей создаём (если он ещё не существует) счётчик, увеличиваем его на 1 и кэшируем на 24 часа.
* Для отправителей из разрешённого списка:
* Добавьте ключ для адреса электронной почты в конверте «MAIL FROM», если он прошел проверку SPF или не прошел проверку SPF, и не был [имя пользователя почтмейстера](#what-are-postmaster-addresses) или [имя пользователя без ответа](#what-are-no-reply-addresses).
* Если заголовок «From» был внесён в разрешённый список, добавьте ключ для адреса электронной почты в заголовке «From», если он прошёл проверку SPF или прошел проверку DKIM.
* Если заголовок «From» не был внесён в разрешённый список, добавьте ключ для адреса электронной почты в заголовке «From» и его корневого доменного имени после анализа.
* Для отправителей, не внесённых в разрешённый список:
* Добавьте ключ для адреса электронной почты в конверте «MAIL FROM», если он прошёл проверку SPF.
* Если заголовок «From» был внесён в разрешённый список, добавьте ключ для адреса электронной почты в заголовке «From», если он прошёл проверку SPF или прошел проверку DKIM. * Если заголовок «From» не был внесён в разрешённый список, добавьте ключ для адреса электронной почты из заголовка «From» и его корневого доменного имени, проанализированного.
* Добавьте ключ для удалённого IP-адреса отправителя.
* Добавьте ключ для имени хоста клиента, полученного путём обратного поиска по IP-адресу отправителя (если есть).
* Добавьте ключ для корневого домена для имени хоста клиента, полученного путём обратного поиска (если есть и если оно отличается от имени хоста клиента).
3. Если счётчик достигает 5 для отправителя и ключа, не внесённых в разрешённый список, мы добавляем ключ в разрешённый список на 30 дней и отправляем электронное письмо в нашу службу по борьбе с нарушениями. Эти значения могут меняться, и обновления будут отображаться здесь по мере того, как мы отслеживаем нарушения.
4. Если счётчик достигает 10 для отправителя и ключа, внесённых в разрешённый список, мы добавляем ключ в разрешённый список на 7 дней и отправляем электронное письмо в нашу службу по борьбе с нарушениями. Эти значения могут меняться, и обновления будут отображаться здесь по мере того, как мы отслеживаем нарушения.
...
* Вместо ограничения скорости входящей почты, получаемой по отдельным псевдонимам (например, `you@yourdomain.com`), мы ограничиваем скорость по самому доменному имени псевдонима (например, `yourdomain.com`). Это предотвращает переполнение почтовых ящиков всех псевдонимов вашего домена сообщениями типа `Senders` одновременно.
* У нас есть общие ограничения, которые применяются ко всем `Senders` в нашем сервисе независимо от получателя:

`Senders`, которые мы считаем «доверенными» как источник достоверной информации (например, `gmail.com`, `microsoft.com`, `apple.com`), ограничены отправкой 100 ГБ в день.


`Senders`, которые являются [в разрешенном списке](#do-you-have-an-allowlist), ограничены отправкой 10 ГБ в день.
...
* У нас есть определенный лимит на `Sender` и `yourdomain.com` в размере 1 ГБ и/или 1000 сообщений в день.

Серверы MX также ограничивают пересылку сообщений одному или нескольким получателям с помощью ограничения скорости, но это применимо только к `Senders`, а не к [белый список](#do-you-have-an-allowlist):

* Мы разрешаем не более 100 подключений в час на каждый корневой домен с полным доменным именем (FQDN) `Sender` (или) на удалённый IP-адрес `Sender` (если обратный PTR недоступен) и на каждого получателя конверта. Мы храним ключ ограничения скорости в виде криптографического хеша в нашей базе данных Redis.
...
* Если вы отправляете почту с домена, например, `test.abc.123.example.com`, то ограничение скорости будет наложено на `example.com`. Многие спамеры используют сотни поддоменов, чтобы обойти распространённые спам-фильтры, которые ограничивают скорость только для уникальных имён хостов, а не для уникальных корневых доменов с полными доменными именами (FQDN).

* `Senders`, превышающие лимит скорости, будут отклонены с ошибкой 421.

Наши серверы IMAP и SMTP ограничивают количество одновременных подключений к вашим псевдонимам более `60`.

Наши MX-серверы ограничивают отправителей [не входящий в список разрешенных](#do-you-have-an-allowlist) возможностью устанавливать более 10 одновременных подключений (с истечением срока действия кэша счетчика в 3 минуты, что отражает время ожидания нашего сокета в 3 минуты).
...
### Предотвращает возвраты сообщений от известных спамеров MAIL FROM {#prevent-bounces-from-known-mail-from-spammers}

Мы извлекаем список из [Backscatter.org](https://www.backscatterer.org/) (на основе [UCEPROTECT](https://www.uceprotect.net/)) по адресу <http://wget-mirrors.uceprotect.net/rbldnsd-all/ips.backscatterer.org.gz> каждый час и передаем его в нашу базу данных Redis (мы также заранее сравниваем разницу; на случай, если какие-либо IP-адреса были удалены и их необходимо сохранить).
...
Если IP-адрес отправителя указан в списке (и отсутствует в нашем списке [белый список](#do-you-have-an-allowlist)), мы отправляем ошибку 554 с сообщением `The IP ${session.remoteAddress} is blocked by https://www.backscatterer.org/index.php?target=test&ip=${session.remoteAddress}`. Мы получим уведомление, если отправитель есть как в списке Backscatterer, так и в нашем списке разрешенных адресов, чтобы мы могли решить проблему при необходимости.

Методы, описанные в этом разделе, соответствуют рекомендации «БЕЗОПАСНЫЙ РЕЖИМ» по адресу <https://www.backscatterer.org/?target=usage>, где мы проверяем IP-адрес отправителя только в том случае, если определенные условия уже выполнены.

### Предотвращайте ненужные возвраты для защиты от обратного рассеяния {#prevent-unnecessary-bounces-to-protect-against-backscatter}
...
2. Мы отправляем письма только один раз (мы используем рассчитанный ключ отпечатка возврата и храним его в кэше, чтобы предотвратить отправку дубликатов). Отпечаток возврата — это ключ, представляющий собой отпечаток сообщения, совмещённый с хэшем адреса возврата и его кода ошибки. Подробнее о том, как рассчитывается отпечаток сообщения, см. в разделе [Дактилоскопирование](#how-do-you-determine-an-email-fingerprint). Успешно отправленные отпечатки возврата удаляются через 7 дней в нашем кэше Redis.

3. Мы отправляем сообщения только в том случае, если поле MAIL FROM и/или From не пустое и не содержит (без учета регистра) [имя пользователя почтмейстера](#what-are-postmaster-addresses) (часть перед @ в письме).
...
6. Мы не отправляем сообщения, если часть имени пользователя в поле «От» адреса электронной почты содержит `mdaemon` и имеет нечувствительный к регистру заголовок `X-MDDSN-Message`.

7. Мы не отправляем данные, если в заголовке `content-type` `multipart/report` присутствует регистронезависимый заголовок.
...
Например, если я хочу, чтобы все электронные письма, отправляемые на адрес `example.com`, пересылались на SMTP-порт псевдонима получателя 1337 вместо 25:
...
Если вы хотите, чтобы для пересылки писем использовался `foo.example.com`, введите `foo` в качестве значения имени/хоста/псевдонима в настройках DNS (для записей MX и <strong class="notranslate">TXT</strong>).

### Пересылает ли это заголовки моих писем {#does-this-forward-my-emails-headers}

Да, конечно.

### Это хорошо протестированный {#is-this-well-tested}

Да, в нем есть тесты, написанные с использованием [ава](https://github.com/avajs/ava), а также покрытие кода.

### Вы передаете ответные сообщения SMTP и коды {#do-you-pass-along-smtp-response-messages-and-codes}

Да, конечно. Например, если вы отправляете письмо на адрес `hello@example.com`, а пересылка настроена на `user@gmail.com`, то вместо прокси-сервера mx1.forwardemail.net или mx2.forwardemail.net будет возвращено сообщение с ответом SMTP и кодом от SMTP-сервера gmail.com.

### Как защититься от спамеров и обеспечить хорошую репутацию при пересылке электронной почты {#how-do-you-prevent-spammers-and-ensure-good-email-forwarding-reputation}

См. наши разделы [Как работает ваша система пересылки электронной почты?](#how-does-your-email-forwarding-system-work), [Как вы решаете проблемы с доставкой электронной почты?](#how-do-you-handle-email-delivery-issues) и [Как вы справляетесь с блокировкой своих IP-адресов?](#how-do-you-handle-your-ip-addresses-becoming-blocked) выше.

### Как выполнять DNS-поиск по доменным именам {#how-do-you-perform-dns-lookups-on-domain-names}

Мы создали проект с открытым исходным кодом :tangerine: [мандарин](https://github.com/forwardemail/tangerine) и используем его для DNS-запросов. DNS-серверы по умолчанию — `1.1.1.1` и `1.0.0.1`, а DNS-запросы выполняются через [DNS через HTTPS](https://en.wikipedia.org/wiki/DNS_over_HTTPS) (DoH) на прикладном уровне.
...
### Предоставляете ли вы гарантию возврата денег за платные планы {#do-you-offer-a-money-back-guarantee-on-paid-plans}?
...
### Если я поменяю тарифный план, вы пропорционально распределите расходы и вернете разницу {#if-i-switch-plans-do-you-pro-rate-and-refund-the-difference}
...
### Могу ли я просто использовать эту службу пересылки электронной почты в качестве «запасного» или «резервного» MX-сервера {#can-i-just-use-this-email-forwarding-service-as-a-fallback-or-fallover-mx-server}
...
**Для тихого отклонения (отправителю кажется, что сообщение было успешно отправлено, но на самом деле оно никуда не отправляется) (код статуса `250`):** Если вы добавите к псевдониму префикс "!" (один восклицательный знак), то отправителям, пытающимся отправить письмо на этот адрес, будет возвращен код успешного статуса `250`, но сами письма никуда не отправятся (например, в черную дыру или `/dev/null`).

**Для мягкого отклонения (код статуса `421`):** Если вы добавите к псевдониму префикс «!!» (двойной восклицательный знак), то отправителям, пытающимся отправить письмо на этот адрес, будет возвращен код статуса мягкой ошибки `421`, а электронные письма часто будут отправляться повторно в течение 5 дней, прежде чем будут отклонены и возвращены.
...
Например, если я хочу, чтобы электронное письмо, отправленное на `hello@example.com`, было переслано на `user+a@gmail.com` и `user+b@gmail.com`, то моя запись <strong class="notranslate">TXT</strong> будет выглядеть следующим образом:
...
### Могу ли я иметь несколько глобальных получателей для приема всей почты домена {#can-i-have-multiple-global-catch-all-recipients}
...
Например, если я хочу, чтобы каждое письмо, отправляемое на адрес `*@example.com` (звездочка означает, что это подстановочный знак, то есть перехват всех сообщений), пересылалось на `user+a@gmail.com` и `user+b@gmail.com`, то моя запись <strong class="notranslate">TXT</strong> будет выглядеть следующим образом:
...
Да, можете, но вам всё равно необходимо соблюдать максимальный лимит. Если у вас есть `hello:linus@example.com` и `linus:user@gmail.com`, то письма на `hello@example.com` будут пересылаться на `linus@example.com` и `user@gmail.com`. Обратите внимание, что при попытке рекурсивной пересылки писем за пределы максимального лимита возникнет ошибка.

### Могут ли люди отменить регистрацию или зарегистрировать мою пересылку электронной почты без моего разрешения {#can-people-unregister-or-register-my-email-forwarding-without-my-permission}
...
**Да. Вы можете получить доступ к журналам ошибок по адресу [Моя учетная запись → Журналы](/my-account/logs) или [Мой аккаунт → Домены](/my-account/domains).**
...
Журналы ошибок позволяют проверять наличие пропущенных важных писем и минимизировать количество ложных срабатываний спама для [ваши домены](/my-account/domains). Они также являются отличным ресурсом для отладки проблем с [веб-перехваты электронной почты](#do-you-support-webhooks) (поскольку журналы ошибок содержат ответ конечной точки веб-перехватчика).
...
Более подробную информацию можно найти по ссылке [политика конфиденциальности](/privacy).

### Вы читаете мои письма {#do-you-read-my-emails}

Нет, конечно. См. нашу [политика конфиденциальности](/privacy).
...
Мы считаем, что у вас должно быть право на конфиденциальность, и мы строго его соблюдаем. Код, размещённый на сервере, [программное обеспечение с открытым исходным кодом на GitHub](https://github.com/forwardemail) для прозрачности и укрепления доверия.

### Могу ли я «отправлять почту как» в Gmail с помощью этого {#can-i-send-mail-as-in-gmail-with-this}
...
### Могу ли я «отправить почту как» в Outlook с помощью этого {#can-i-send-mail-as-in-outlook-with-this}
...
Более подробную информацию см. по ссылке <https://support.apple.com/en-us/102540>.

### Могу ли я пересылать неограниченное количество писем с помощью этого {#can-i-forward-unlimited-emails-with-this}?

Да, однако для «относительно неизвестных» отправителей установлено ограничение в 100 подключений в час на одно имя хоста или IP-адрес. См. разделы [Ограничение скорости](#do-you-have-rate-limiting) и [Серый список](#do-you-have-a-greylist) выше.

Под «относительно неизвестными» мы подразумеваем отправителей, которые не указаны в [белый список](#do-you-have-an-allowlist).
...
* [Практические примеры и документация для разработчиков](/blog/docs)
* [Ресурсы](/resources)
* [Гиды](/guides)
