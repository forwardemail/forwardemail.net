...
* [إمكانية الاحتفاظ بمعالجة بريدي الإلكتروني وتخزينه داخل الاتحاد الأوروبي (إقامة البيانات)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency)
* [مكان معالجة وتخزين البريد الإلكتروني الوارد لنطاقي ومدة الاحتفاظ به](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long)
* [كيفية إدراج Forward Email كمعالج فرعي في اتفاقية DPA الخاصة بي](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
* [كيف أزيد سعة التخزين أو حد إرسال SMTP الصادر](#how-do-i-increase-my-storage-or-outbound-smtp-sending-limit)
* [ما الذي يتضمنه Enterprise License](#what-is-included-in-the-enterprise-license)
...
> \[!TIP]
> قد نعلن قريبًا عن موقع مركز بياناتنا في الاتحاد الأوروبي المستضاف تحت [forwardemail.eu](https://forwardemail.eu). اشترك في النقاش على <https://github.com/orgs/forwardemail/discussions/336> للحصول على التحديثات.  لا يتوفر اليوم نشر يقتصر على الاتحاد الأوروبي فقط أو خيار الاشتراك – راجع [إمكانية الاحتفاظ بمعالجة بريدي الإلكتروني وتخزينه داخل الاتحاد الأوروبي (إقامة البيانات)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).
...
نفرض حدود معدل إرسال SMTP الصادرة على مستويات متعددة لمنع إساءة الاستخدام مع الحفاظ على المرونة للاستخدام المشروع. يتم فحص كل مستوى بالترتيب — أي حد يُستنفَد أولًا سيؤدي إلى رفض الرسالة مؤقتًا بخطأ `421` (أي "حاول مرة أخرى لاحقًا").

**Rate limit hierarchy:**

| Level | Scope | Default Limit | Description |
| :---- | :---- | :-----------: | :---------- |
| Per-alias | اسم مستعار فردي | None (uses domain limit) | اختياري. إذا تم تعيين `smtp_limit` مخصص لاسم مستعار، فإنه يتم التحقق منه أولًا. |
| Per-domain | جميع الرسائل المرسلة من نطاق خلال يوم واحد | 300/day | يحسب جميع الرسائل الصادرة عبر كل الأسماء المستعارة في النطاق. |
| Per-user | جميع الرسائل المرسلة بواسطة حساب مستخدم خلال يوم واحد | 300/day | يمنع التحايل عن طريق حذف وإعادة إنشاء الأسماء المستعارة أو النطاقات. |

**How the effective limit is determined:**

* **Team plan domains** — الحد اليومي الفعّال هو أعلى `smtp_limit` بين جميع أعضاء الإدارة في النطاق. على سبيل المثال، إذا كان لدى أحد المدراء حد 300 ولدى مدير آخر 500، فإن الحد الفعّال للنطاق هو 500.
* **Enhanced Protection and other plans** — الحد اليومي الفعّال هو `smtp_limit` الخاص بمستخدم الإرسال (والذي يكون افتراضيًا 300 رسالة في اليوم).
* **Per-alias override** — يمكن لمسؤولي النطاق اختيارياً تعيين `smtp_limit` مخصص على أسماء مستعارة فردية. عند التعيين، يتم التحقق منه أولًا (قبل حدود النطاق والمستخدم). هذا مفيد لتقييد أسماء مستعارة محددة بحجم إرسال أقل.
* **الإعداد الافتراضي للنطاق للأسماء المستعارة الجديدة** — يمكن لمشرفي النطاق تعيين `alias_default_smtp_limit` على النطاق (عبر الـ API أو الإعدادات المتقدمة في لوحة التحكم). عند تعيينه، سوف ترث جميع الأسماء المستعارة المنشأة حديثًا على ذلك النطاق هذه القيمة تلقائيًا كقيمة `smtp_limit` الخاصة بها. لا يمكن أن تتجاوز ذلك الحد الفعّال لـ SMTP للنطاق. الأسماء المستعارة الحالية لا تتأثر. اضبطها على `0` لتعطيلها.

**System administrators** (موظفو Forward Email) معفَوْن من جميع حدود المعدل.

يتم تطبيق جميع قيود المعدل باستخدام عدادات قاعدة البيانات (`Emails.countDocuments`) على الرسائل التي تم إنشاؤها منذ بداية اليوم الحالي (منتصف الليل بتوقيت UTC). هذا يعني أن حدك يعاد ضبطه يوميًا عند منتصف الليل بتوقيت UTC.

إذا كنت بحاجة إلى حد أعلى، يرجى [الاتصال بنا](https://forwardemail.net/help). يتم تلبية معظم الطلبات خلال ساعة إلى ساعتين.
...
* **تطبيق مصادقة البريد الإلكتروني**: يجب أن تجتاز الرسائل من المرسلين غير المدرجين في القائمة المسموحة واحدة على الأقل من SPF أو DKIM (مشابه لمتطلبات Gmail وOutlook وYahoo منذ 2024). يتم رفض الرسائل التي لا تحتوي على أي مصادقة ناجحة برمز خطأ 550. يتم تطبيق سياسات DMARC بما في ذلك `p=reject` و`p=quarantine`.
* **فحص HELO/EHLO ضد قائمة الحظر**: يتم التحقق من اسم المضيف المقدم أثناء تحية SMTP HELO/EHLO مقابل قائمة الحظر الخاصة بنا، مما يوفر حماية حتى عندما لا تتوفر سجلات DNS عكسية لمرسلي IPv6
...
* **الاستخدام المنضبط للتبديل**: تظل خوادم التطبيقات والخوادم الأخرى غير التابعة لقواعد البيانات خالية من التبديل ويتم تحديد حجمها ومراقبتها بحيث تبقى أعباء العمل العادية في ذاكرة الوصول العشوائي (RAM). تستخدم خوادم MongoDB و Redis ملف تبديل بصلاحيات root فقط مع `vm.swappiness=1` بشكل صارم كشبكة أمان لنفاد الذاكرة، وليس كسعة عادية. يمكن أن يؤدي تعطيل التبديل في كل مكان إلى تحويل ضغط ذاكرة قاعدة البيانات العابر إلى عملية قتل OOM فورية. يتم مراقبة نشاط التبديل في قواعد البيانات والتحقيق فيه.
...
* تتوفر نسخة موقّعة من الطرفين، وترتيبات DPA مخصّصة، عبر [Enterprise License](#what-is-included-in-the-enterprise-license)
* لإدراج اسمنا في ملحق المعالج الفرعي الخاص بك، راجع [كيفية إدراج Forward Email كمعالج فرعي في اتفاقية DPA الخاصة بي](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
### هل يمكنني إبقاء معالجة بريدي الإلكتروني وتخزينه داخل الاتحاد الأوروبي (إقامة البيانات) {#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency}

> [!IMPORTANT]
> ليس بعد. لا يتوفر حالياً نشر خاص بالاتحاد الأوروبي فقط أو خيار اشتراك خاص بالاتحاد الأوروبي، ولا يجب أن تصف Forward Email في سجلاتك أو في اتفاقية معالجة البيانات الخاصة بك على أنها معالجاً يقتصر على الاتحاد الأوروبي.

**أين تتم المعالجة اليوم:** تقع خوادمنا أساساً في Denver, Colorado, United States (راجع [أين تقع خوادمكم](#where-are-your-servers-located) و<https://forwardemail.net/ips>). وينطبق ذلك على كل ميزة: إعادة التوجيه الوارد، سجلات الأخطاء، تخزين صندوق البريد (IMAP/POP3/CalDAV/CardDAV)، وSMTP الصادر.

**ما المخطط له:** مركز بيانات داخل الاتحاد الأوروبي في أمستردام، مستضاف تحت [forwardemail.eu](https://forwardemail.eu). وللحفاظ على عزل حقيقي لبيانات الاتحاد الأوروبي – قانونياً وتقنياً – يجب أن يعمل عبر شركة مستقلة مقرها الاتحاد الأوروبي، وليس فقط عبر خوادم في موقع داخل الاتحاد الأوروبي. لا يوجد لدينا تاريخ إطلاق بعد، والانضمام إلى قائمة الانتظار لا يغيّر مكان معالجة بريدك اليوم. اشترك في <https://github.com/orgs/forwardemail/discussions/336> للحصول على الإعلانات؛ سنوضح هناك كيف يمكن للعملاء الحاليين الاشتراك عندما يصبح متاحاً.

**ما يمكنك التصريح به بدقة اليوم:**

* المعالج: Forward Email LLC، شركة مسجلة في Delaware, United States
* موقع المعالجة والتخزين: United States
* آلية النقل وضماناته: كما هو موصوف في صفحاتنا [DPA](/dpa) و[GDPR](/gdpr)
* المعالِجون الفرعيون لدينا: مذكورون في صفحة [GDPR](/gdpr) لدينا (جميعهم مزوّدون في الولايات المتحدة)

راجع [أين تتم معالجة البريد الوارد لنطاقي وتخزينه، ولمدة كم من الوقت](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) للاطلاع على تفصيل بحسب الميزة، و[كيف أسرد Forward Email كمعالج فرعي في اتفاقية المعالجة الخاصة بي](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa) للحصول على إدخال ملحق جاهز للنسخ.

### أين تتم معالجة البريد الوارد لنطاقي وتخزينه، ولمدة كم من الوقت {#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long}

تتم جميع عمليات المعالجة والتخزين على خوادمنا في Denver, Colorado, United States. ما يتم الاحتفاظ به، ولمدة كم من الوقت، يعتمد على الميزة:

| الميزة | ما الذي يُحتفَظ به | مدة الاحتفاظ |
| --- | --- | --- |
| **إعادة توجيه البريد الإلكتروني** (اسم مستعار يعيد التوجيه إلى عنوان آخر أو webhook) | لا شيء. تتم معالجة الرسالة بالكامل في الذاكرة ويتم ترحيلها إلى الوجهة؛ ولا تُكتب أبداً إلى القرص أو إلى قاعدة بيانات. | لا يوجد – تُتلف الرسالة بمجرد محاولة التسليم |
| **سجلات أخطاء SMTP** (`4xx`/`5xx` responses) | خطأ SMTP، والظرف (envelope)، وترويسات البريد الإلكتروني. وليس محتوى الرسالة أو المرفقات. | 7 أيام (راجع [هل تخزنون سجلات الأخطاء](#do-you-store-error-logs)) |
| **تخزينات مؤقتة لمكافحة الإساءة** (greylist, denylist, rate limiting) | مفاتيح مشتقة من بيانات المرسل والمستلم (عناوين، نطاقات، تجزئات)، دون أي محتوى للرسالة. | حتى 30 يوماً (راجع [هل لديكم قائمة رمادية](#do-you-have-a-greylist) و[هل لديكم قائمة رفض](#do-you-have-a-denylist)) |
| **تخزين صندوق البريد** (IMAP/POP3/CalDAV/CardDAV، عند تمكينه على اسم مستعار) | الرسائل وجهات الاتصال والتقويمات، في صندوق بريد SQLite مُشفّر بشكل فردي لا يمكن فتحه إلا بكلمة مرورك. | حتى تحذف الرسالة، أو الاسم المستعار، أو النطاق، أو الحساب (راجع [كيف أصدّر وأُنسخ احتياطياً صندوق بريدي](#how-do-i-export-and-backup-my-mailbox)) |
| **SMTP الصادر** | سجلات قائمة الانتظار للتسليم والإبلاغ عن الأخطاء. تُحتفَظ بهيئات الرسائل لمدة 0 يوم افتراضياً بعد التسليم الناجح أو حدوث خطأ دائم؛ يمكنك رفع ذلك إلى حد أقصى 30 يوماً لكل نطاق. | ~30 يوماً لسجلات قائمة الانتظار (راجع [سياسة الخصوصية](/privacy) الخاصة بنا) |

أمران يجدر ذكرهما في سجلاتك:

* تُسلَّم الرسالة المعاد توجيهها إلى صندوق البريد الوجهة الذي قمت بإعداده (على سبيل المثال Google Workspace أو Microsoft 365). وما يخزنه ذلك المزوّد يخضع لاتفاقك معه، وليس لاتفاقنا نحن.
* سجلات النطاق والحساب (عنوان بريدك الإلكتروني، أسماء النطاقات، الأسماء المستعارة، تفاصيل الفوترة) تُخزَّن ما دام حسابك موجوداً. راجع [سياسة الخصوصية](/privacy) للاطلاع على القائمة الكاملة.

المصادر: <https://forwardemail.net/privacy>, <https://forwardemail.net/technical-whitepaper.pdf>, وملف معالج MX على <https://github.com/forwardemail/forwardemail.net/blob/master/helpers/on-data-mx.js>.

### كيف أسرد Forward Email كمعالج فرعي في اتفاقية المعالجة الخاصة بي {#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa}

إذا كنت متحكّماً (أو معالجاً بدورك) بموجب المادة 28 من GDPR وتستخدم Forward Email للتعامل مع البريد لعملائك أو مستخدميك، فستحتاج عادةً إلى تسميتنا في ملحق المعالِجين الفرعيين لديك. الإدخال التالي هو ما كنا سنكتبه:

| الحقل | القيمة |
| --- | --- |
| **المعالج الفرعي** | Forward Email LLC |
| **مسجل في** | State of Delaware, United States |
| **الخدمة** | إعادة توجيه البريد الإلكتروني (inbound MX) وحيثما كان مُمكّناً، تخزين صندوق بريد مُشفّر (IMAP/POP3/CalDAV/CardDAV) وSMTP الصادر |
| **البيانات الشخصية المُعالَجة** | عناوين البريد الإلكتروني، وبيانات الظرف والترويسات، ومحتوى الرسائل أثناء العبور (في الذاكرة فقط لغرض إعادة التوجيه؛ ومشفّر أثناء التخزين لتخزين صندوق البريد) |
| **موقع المعالجة** | United States (Denver, Colorado) |
| **مدة الاحتفاظ** | راجع [أين تتم معالجة البريد الوارد لنطاقي وتخزينه، ولمدة كم من الوقت](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) |
| **آلية النقل** | كما هو منصوص عليه في صفحات [اتفاقية معالجة البيانات](/dpa) و[GDPR](/gdpr) لدينا |
| **المعالِجون الفرعيون لدينا** | مذكورون في صفحة [GDPR](/gdpr) وفي [DPA](/dpa) (حالياً Cloudflare وDataPacket وDigitalOcean وGitHub وVultr وStripe وPayPal) |
| **الممثل في الاتحاد الأوروبي (المادة 27)** | Osano International Compliance Services Limited، Dublin, Ireland – العنوان الكامل في صفحة [GDPR](/gdpr) لدينا |
| **الممثل في المملكة المتحدة** | Osano UK Compliance LTD، Belfast – العنوان الكامل في صفحة [GDPR](/gdpr) لدينا |
| **اتفاقية معالجة البيانات** | <https://forwardemail.net/dpa> (يتم قبولها مع [الشروط](/terms) الخاصة بنا؛ وتُوفَّر نسخة موقعة مقابِلة ضمن Enterprise License) |
| **جهة اتصال الأمان** | <security@forwardemail.net> |

> [!WARNING]
> لا تسمح [الشروط](/terms) لدينا بتشغيل خدمة تُمكّن أطرافاً ثالثة (أي أي شخص غير موظفيك ومقاوليك) من الوصول إلى Forward Email أو استخدامه أو الاستفادة منه دون اتفاقية مؤسسية. إذا كنت تسمينا كمعالج فرعي لأنك تعالج بريد عملائك، فأنت بحاجة إلى [Enterprise License](#what-is-included-in-the-enterprise-license) – وهذه أيضاً هي الطريقة التي تحصل بها على DPA موقّعة مقابِلة.
...
* `HELO` - يشير هذا إلى تحية لتحديد اسم المضيف المؤهل بالكامل للمرسل، أو عنوان IP، أو اسم معالج البريد. يمكن تزوير هذه القيمة، لذلك لا نعتمد على هذه البيانات ونستخدم بدلاً من ذلك البحث العكسي لاسم المضيف لعنوان IP الخاص بالاتصال. ومع ذلك، نتحقق الآن من هذه القيمة مقابل [قائمة الحظر](#do-you-have-a-denylist) بالإضافة إلى البحث العكسي عن اسم المضيف، مما يوفر حماية إضافية خاصة لاتصالات IPv6 التي قد لا تتوفر لها سجلات DNS عكسية.
...
انتقل إلى <a href="/my-account/billing" target="_blank" rel="noopener noreferrer">حسابي → الفوترة</a> ومرّر إلى قسم **الإضافات**، الذي يحتوي على نموذجين للطلب:

* **ترقية التخزين** — اختر كمية لإضافتها (+10, +20, +30, +40، أو +50 GB)، أو اختر "Other" لطلب كمية مخصصة.
* **ترقية حد SMTP الصادر** — اختر كمية لإضافتها (+1000, +2000، أو +3000 رسالة إلكترونية يوميًا)، أو اختر "Other" لطلب كمية مخصصة.

إرسال أي من النموذجين يوجّه طلبك إلى فريقنا للمراجعة — ولن يتم تحصيل رسوم منك فورًا. بعد الموافقة، سنرسل لك عبر البريد الإلكتروني رابط دفع آمن لإتمام الترقية. يمكنك أن يكون لديك طلب واحد قيد الانتظار لكل نوع (التخزين أو SMTP) في الوقت نفسه؛ ولا يُسمح بالإرسال مرة أخرى خلال 3 أيام من طلب سابق من النوع نفسه حتى تنقضي تلك المهلة.


### ما الذي يتضمنه Enterprise License {#what-is-included-in-the-enterprise-license}

Enterprise License هو **$250/month**، ويتم تسعيره وفوترة قيمته مباشرةً بدلاً من شرائه عبر صفحة التسعير. وهو يتضمن كل ما في خطة Team بالإضافة إلى:

* **استثناء من شروط الاستخدام للأطراف الثالثة** – يمكنك تشغيل خدمة أو تكاملاً حيث يستخدم عملاؤك أو طلابك أو عملاؤك بالوكالة (وليس فقط موظفيك ومقاوليك) Forward Email أو يستفيدون منه. [الشروط](/terms) القياسية لدينا لا تسمح بذلك.
* **اتفاقية مؤسسية موقعة مقابِلة وDPA** – تُرسل للمراجعة والتوقيع عبر DocuSign. هذه هي الطريقة التي تحصل بها على [اتفاقية معالجة البيانات](/dpa) موقعة بدلاً من النسخة المقبولة مع شروطنا.
* **شروط تعاقدية مخصصة** حيث يمكننا دعمها، مثل اتفاقيات مستوى الخدمة لإشعارات خرق البيانات (راجع [كيف تتعاملون مع إشعارات خرق البيانات](#how-do-you-handle-data-breach-notifications)).
* **طلبات API غير محدودة** بدون تحديد لمعدل الطلبات.
* **غرفة دردشة دعم عملاء مخصصة** مع فريقنا الهندسي.
* **مساعدة على الإعداد ودعم هندسي لحظي.**

هذا لا يغيّر مكان معالجة بياناتك: راجع [هل يمكنني إبقاء معالجة بريدي الإلكتروني وتخزينه داخل الاتحاد الأوروبي (إقامة البيانات)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).

**من يحتاج إليه:** أي شخص يقع استخدامه للخدمة خارج الشروط (وصول أطراف ثالثة)، وأي شخص يتطلب DPA موقّعة مقابِلة، أو شروط DPA مخصصة، أو جهة اتصال هندسية مُسمّاة لأغراض الامتثال.

**كيفية البدء:** [اتصل بنا](/help) مع اسم شركتك القانوني، وعنوانها المسجّل، وأرقام تسجيل الشركة وVAT، واسم ومنصب وعنوان البريد الإلكتروني للشخص الذي سيوقّع. سنرد باتفاقية DocuSign للمراجعة.
