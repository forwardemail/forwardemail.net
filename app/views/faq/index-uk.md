...
* [Чи можу я забезпечити обробку та зберігання моєї електронної пошти в ЄС (локалізація даних)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency)
* [Де обробляється та зберігається вхідна електронна пошта для мого домену і як довго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long)
* [Як зазначити Forward Email як субобробника у моєму власному DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
* [Як збільшити обсяг сховища або ліміт вихідного надсилання SMTP](#how-do-i-increase-my-storage-or-outbound-smtp-sending-limit)
* [Що входить до Enterprise License](#what-is-included-in-the-enterprise-license)
...
> \[!TIP]
> Незабаром ми можемо оголосити про розташування нашого датацентру в ЄС, розміщеного під [forwardemail.eu](https://forwardemail.eu). Підпишіться на обговорення за адресою <https://github.com/orgs/forwardemail/discussions/336> для оновлень.  Наразі немає розгортання лише для ЄС або опції — див. [Чи можу я забезпечити обробку та зберігання моєї електронної пошти в ЄС (локалізація даних)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).
...
| -------- | ---------- | --------------------------------------------------------------------------- |
| `sqlite` | `.sqlite` | Необроблений зашифрований знімок `sqlite` (.sqlite), що містить записи поштової скриньки, контактів, календаря та подій календаря; стандартна автоматична резервна копія IMAP. |
| `mbox` | `.zip` | Захищений паролем `.zip`, що містить файли поштової скриньки `mbox`, а також папки `Contacts` (VCF) та `Calendars` (ICS). |
| `eml` | `.zip` | Захищений паролем `.zip`, що містить окремі файли `.eml` (`eml`), а також папки `Contacts` (VCF) та `Calendars` (ICS). |

Усі формати експорту зберігають контакти, календарі та події календаря. Портативні ZIP-архіви EML/MBOX упорядковують файли VCF за адресною книгою в Contacts, а ресурси подій або завдань ICS — за календарем у Calendars. Необроблений SQLite зберігає ті самі записи в рідних таблицях.
...
Виклик ``DELETE /v1/account/api-token`` вимикає API-токен, після чого він більше не автентифікує запити HTTP API або WebSocket. Доступ до API можна знову ввімкнути лише через сторінку [/my-account/security](/my-account/security) шляхом скидання токена, що згенерує новий на заміну; автентифікація за допомогою псевдоніма та пароля залишається незмінною.
...
* **Примусова аутентифікація електронної пошти**: Повідомлення від відправників, які не входять до списку дозволених, повинні пройти принаймні SPF або DKIM (аналогічно вимогам Gmail, Outlook та Yahoo з 2024 року). Повідомлення без жодної успішної аутентифікації відхиляються з кодом помилки 550. Застосовуються політики DMARC `p=reject` та `p=quarantine`.
* **Перевірка HELO/EHLO по списку блокування**: Ім'я хоста, представлене під час привітання SMTP HELO/EHLO, перевіряється по нашому списку блокування, забезпечуючи захист навіть коли у відправників IPv6 відсутні зворотні DNS-записи
...
* **Контрольоване використання swap**: Хости застосунків та інші хости, що не є базами даних, залишаються без swap, а їх розмір і моніторинг налаштовані так, щоб звичайні робочі навантаження залишалися в RAM. Хости MongoDB та Redis використовують файл swap лише для root з `vm.swappiness=1` суворо як захисну сітку від нестачі пам'яті, а не як звичайну ємність. Вимкнення swap усюди може перетворити тимчасове навантаження на пам'ять бази даних на негайне завершення через OOM. Активність swap бази даних відстежується та розслідується.
...
* Контрпідписана копія та індивідуальні умови DPA доступні через [Enterprise License](#what-is-included-in-the-enterprise-license)
* Щоб зазначити нас у власному додатку щодо субобробників, див. [Як зазначити Forward Email як субобробника у моєму власному DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa)
...
### Чи можу я зберігати обробку та зберігання електронної пошти в ЄС (резидентність даних) {#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency}

> [!IMPORTANT]
> Ще ні. Наразі немає розгортання лише в ЄС або можливості вибору ЄС, і ви не повинні описувати Forward Email як обробника лише в ЄС у ваших документах або угоді про обробку даних.

**Де сьогодні відбувається обробка:** наші сервери розміщені переважно в Denver, Colorado, United States (див. [Де розміщені ваші сервери](#where-are-your-servers-located) та <https://forwardemail.net/ips>). Це стосується кожної функції: вхідної переадресації, журналів помилок, сховища поштової скриньки (IMAP/POP3/CalDAV/CardDAV) і вихідного SMTP.

**Що заплановано:** дата-центр в ЄС у Амстердамі, розміщений під [forwardemail.eu](https://forwardemail.eu). Щоб зберігати дані ЄС справді ізольованими — юридично та технічно — він має працювати через окрему компанію, що базується в ЄС, а не лише через сервери, розташовані в ЄС. Дати запуску поки немає, і приєднання до списку очікування не змінює того, де сьогодні обробляється ваша пошта. Підпишіться на <https://github.com/orgs/forwardemail/discussions/336> на оновлення; там ми опишемо, як наявні клієнти зможуть увімкнути цю опцію, коли вона стане доступною.

**Що ви можете коректно зазначити сьогодні:**

* Обробник: Forward Email LLC, компанія, зареєстрована в Delaware, United States
* Місце обробки та зберігання: United States
* Механізм передачі та запобіжні заходи: як описано на наших сторінках [DPA](/dpa) і [GDPR](/gdpr)
* Наші субобробники: перелічені на нашій сторінці [GDPR](/gdpr) (усі провайдери зі США)

Див. [Де обробляється й зберігається вхідна пошта для мого домену та як довго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) для розбиття за функціями та [Як додати Forward Email як субобробника у власну DPA](#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa) для готового до копіювання пункту додатка.

### Де обробляється й зберігається вхідна пошта для мого домену, і як довго {#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long}

Уся обробка та зберігання відбуваються на наших серверах у Denver, Colorado, United States. Те, що зберігається і як довго, залежить від функції:

| Функція | Що зберігається | Період зберігання |
| --- | --- | --- |
| **Переадресація електронної пошти** (аліас, що переспрямовує на іншу адресу або вебхук) | Нічого. Повідомлення повністю обробляється в пам'яті та надсилається на місце призначення; воно ніколи не записується на диск чи до бази даних. | Немає – повідомлення відкидається, щойно здійснено спробу доставки |
| **Журнали помилок SMTP** (`4xx`/`5xx` відповіді) | Помилка SMTP, конверт і заголовки електронного листа. **Не** тіло повідомлення чи вкладення. | 7 днів (див. [Чи зберігаєте ви журнали помилок](#do-you-store-error-logs)) |
| **Кеші протидії зловживанням** (сірий список, заборонений список, обмеження частоти) | Ключі, отримані з даних відправника й одержувача (адреси, домени, хеші), без вмісту повідомлень. | До 30 днів (див. [Чи у вас є сірий список](#do-you-have-a-greylist) і [Чи у вас є заборонений список](#do-you-have-a-denylist)) |
| **Сховище поштової скриньки** (IMAP/POP3/CalDAV/CardDAV, коли ввімкнено на аліасі) | Повідомлення, контакти та календарі в індивідуально зашифрованій поштовій скриньці SQLite, яку може відкрити лише ваш пароль. | Доки ви не видалите повідомлення, аліас, домен або обліковий запис (див. [Як експортувати та створити резервну копію моєї поштової скриньки](#how-do-i-export-and-backup-my-mailbox)) |
| **Вихідний SMTP** | Записи черги для доставки та звітування про помилки. Тіла повідомлень за замовчуванням зберігаються 0 днів після успішної доставки або постійної помилки; ви можете збільшити це щонайбільше до 30 днів для кожного домену. | ~30 днів для записів черги (див. нашу [Політику конфіденційності](/privacy)) |

Дві речі, які варто зазначити у ваших власних записах:

* Переадресований лист доставляється до поштової скриньки призначення, яку ви налаштували (наприклад, Google Workspace або Microsoft 365). Те, що зберігає той провайдер, регулюється вашою угодою з ним, а не нами.
* Записи про домени й облікові записи (ваша адреса електронної пошти, назви доменів, аліаси, платіжні реквізити) зберігаються, доки існує ваш обліковий запис. Повний перелік див. у нашій [Політиці конфіденційності](/privacy).

Джерела: <https://forwardemail.net/privacy>, <https://forwardemail.net/technical-whitepaper.pdf>, та MX-обробник на <https://github.com/forwardemail/forwardemail.net/blob/master/helpers/on-data-mx.js>.

### Як додати Forward Email як субобробника у власну DPA {#how-do-i-list-forward-email-as-a-subprocessor-in-my-own-dpa}

Якщо ви є контролером (або самі обробником) відповідно до статті 28 GDPR і використовуєте Forward Email для обробки пошти ваших клієнтів або користувачів, зазвичай вам потрібно назвати нас у власному додатку щодо субобробників. Запис нижче — це те, що б ми написали:

| Поле | Значення |
| --- | --- |
| **Субобробник** | Forward Email LLC |
| **Зареєстровано в** | State of Delaware, United States |
| **Сервіс** | Переадресація електронної пошти (вхідний MX) та, де ввімкнено, зашифроване сховище поштової скриньки (IMAP/POP3/CalDAV/CardDAV) і вихідний SMTP |
| **Опрацьовані персональні дані** | Адреси електронної пошти, метадані конверта і заголовків, а також вміст повідомлень у транзиті (лише в пам'яті для переадресації; зашифровані під час зберігання для сховища скриньки) |
| **Місце обробки** | United States (Denver, Colorado) |
| **Період зберігання** | Див. [Де обробляється й зберігається вхідна пошта для мого домену, і як довго](#where-is-inbound-email-for-my-domain-processed-and-stored-and-for-how-long) |
| **Механізм передачі** | Як викладено на наших сторінках [Data Processing Agreement](/dpa) і [GDPR](/gdpr) |
| **Наші субобробники** | Перелічені на нашій сторінці [GDPR](/gdpr) та в [DPA](/dpa) (наразі Cloudflare, DataPacket, DigitalOcean, GitHub, Vultr, Stripe і PayPal) |
| **Представник в ЄС (ст. 27)** | Osano International Compliance Services Limited, Дублін, Ірландія – повна адреса на нашій сторінці [GDPR](/gdpr) |
| **Представник у Великій Британії** | Osano UK Compliance LTD, Белфаст – повна адреса на нашій сторінці [GDPR](/gdpr) |
| **Угода про обробку даних** | <https://forwardemail.net/dpa> (приймається разом із нашими [Terms](/terms); контрпідписаний примірник надається в межах Enterprise License) |
| **Контакт з питань безпеки** | <security@forwardemail.net> |

> [!WARNING]
> Наші [Terms](/terms) не дозволяють експлуатувати сервіс, який дає змогу третім сторонам (будь-кому, окрім ваших власних працівників і підрядників) отримувати доступ, використовувати або отримувати вигоду від Forward Email без укладання підприємницької угоди. Якщо ви вказуєте нас як субобробника, тому що опрацьовуєте пошту ваших клієнтів, вам потрібна [Enterprise License](#what-is-included-in-the-enterprise-license) – це також спосіб отримати контрпідписану DPA.
...
* `HELO` – це привітання для ідентифікації повного доменного імені відправника, IP-адреси або імені поштового обробника. Це значення можна підробити, тому ми не покладаємося на ці дані, а натомість використовуємо зворотний пошук імені хоста за IP-адресою з’єднання. Однак тепер ми також перевіряємо це значення по нашому [списку блокування](#do-you-have-a-denylist) на додаток до зворотного пошуку імені хоста, що забезпечує додатковий рівень захисту, особливо для з'єднань IPv6, де зворотні DNS-записи можуть бути недоступні.
...
Перейдіть до <a href="/my-account/billing" target="_blank" rel="noopener noreferrer">Мій обліковий запис → Білінг</a> і прокрутіть до розділу **Доповнення**, у якому є дві форми запиту:

* **Оновлення сховища** — виберіть обсяг для додавання (+10, +20, +30, +40, або +50 GB), або виберіть "Other", щоб запросити індивідуальний обсяг.
* **Оновлення ліміту вихідного SMTP** — виберіть обсяг для додавання (+1000, +2000, або +3000 електронних листів щодня), або виберіть "Other", щоб запросити індивідуальний обсяг.

Надсилання будь-якої з форм передає ваш запит нашій команді на розгляд — оплата з вас не стягується негайно. Після схвалення ми надішлемо вам електронною поштою безпечне платіжне посилання, щоб завершити оновлення. Ви можете мати по одному запиту, що очікує, для кожного типу (сховище або SMTP) одночасно; повторне подання протягом 3 днів після попереднього запиту того самого типу не дозволяється, доки цей період не мине.


### Що входить до Enterprise License {#what-is-included-in-the-enterprise-license}

Enterprise License коштує **$250/month**, виставляється в комерційній пропозиції та рахунках напряму, а не купується через сторінку тарифів. Вона включає все з плану Team плюс:

* Виняток із Правил користування для третіх сторін — ви можете експлуатувати сервіс або інтеграцію, де ваші клієнти, студенти чи замовники (не лише ваші працівники та підрядники) використовують або отримують вигоду від Forward Email. Наші стандартні [Terms](/terms) цього не дозволяють.
* Контрпідписана enterprise-угода та DPA — надсилається на перевірку та підпис через DocuSign. Так ви отримаєте підписану [Data Processing Agreement](/dpa), а не копію, прийняту разом із нашими [Terms](/terms).
* Кастомні договірні умови, де ми можемо їх підтримати, наприклад SLA щодо сповіщення про порушення (див. [Як ви обробляєте сповіщення про витік даних](#how-do-you-handle-data-breach-notifications)).
* Необмежені запити до API без лімітів швидкості.
* Виділений чат підтримки клієнтів із нашою інженерною командою.
* Допомога з онбордингом і підтримка інженерів у реальному часі.

Це не змінює місце обробки ваших даних: див. [Чи можу я зберігати обробку та зберігання електронної пошти в ЄС (резидентність даних)](#can-i-keep-my-email-processing-and-storage-in-the-eu-data-residency).

**Кому це потрібно:** усім, чиє використання сервісу виходить за межі [Terms](/terms) (доступ третіх сторін), і всім, хто вимагає контрпідписаної DPA, індивідуальних умов DPA або визначеного інженерного контакту для цілей відповідності.

**Як почати:** [зв'яжіться з нами](/help) із юридичною назвою компанії, зареєстрованою адресою, реєстраційним номером компанії та номером ПДВ, а також ім'ям, посадою й адресою електронної пошти особи, яка підписуватиме. Ми надішлемо у відповідь угоду DocuSign для ознайомлення.
