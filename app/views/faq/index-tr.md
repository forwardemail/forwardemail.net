...
### E-postayı İlet {#who-uses-forward-email} özelliğini kim kullanır?
...
5. [E-posta](https://gmail.com) adresine gidin ve [Ayarlar <i class="fa fa-angle-right"></i> Hesaplar ve İçe Aktarma <i class="fa fa-angle-right"></i> E-postayı şu şekilde gönder:](https://mail.google.com/mail/u/0/#settings/accounts) altında "Başka bir e-posta adresi ekle" seçeneğine tıklayın.
...
2. [Şimşek kuşu](https://www.thunderbird.net) dosyasını indirin, kurun ve açın.
...
Bunun yerine, mevcut MX değişiminizi, eşleşmeyen tüm takma adlara ait postaları hizmetimizin değişimlerine (`mx1.forwardemail.net` ve/veya `mx2.forwardemail.net`) yönlendirecek şekilde yapılandırmanız gerekir.

Google Workspace kullanıyorsanız ve eşleşmeyen tüm takma adları hizmetimize iletmek istiyorsanız <https://support.google.com/a/answer/6297084>. adresine bakın

Microsoft 365 (Outlook) kullanıyorsanız ve eşleşmeyen tüm takma adları hizmetimize iletmek istiyorsanız, <https://learn.microsoft.com/en-us/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/set-up-connectors-to-route-mail> ve <https://learn.microsoft.com/en-us/exchange/mail-flow-best-practices/manage-mail-flow-for-multiple-locations>. öğelerine bakın.
...
Başlangıç tarihi, bitiş tarihi, konu ve mesajı yapılandırma olanağına sahipsiniz ve bunları istediğiniz zaman etkinleştirebilir veya devre dışı bırakabilirsiniz:
...
* Tatil yanıtlayıcısı, küresel vanity alan adlarında etkinleştirilemez (örneğin, [tek kullanımlık adresler](/disposable-addresses) desteklenmez).
* Tatil yanıtlayıcısı, joker karakterli/her şeyi kapsayan (`*`) takma adlar veya düzenli ifadeler için etkinleştirilemez.

`postfix` gibi posta sistemlerinin aksine (örneğin `sieve` tatil filtresi uzantısını kullananlar) – Forward Email otomatik olarak DKIM imzanızı ekler, tatil yanıtları gönderirken bağlantı sorunlarını (örneğin yaygın SSL/TLS bağlantı sorunları ve eski bakımlı sunucular nedeniyle) önler ve hatta tatil yanıtları için Open WKD ve PGP şifrelemesini destekler.
...
* Redis önbelleğimiz `alias_id` ve `sender` parmak izlerini kullanır; `alias_id` ise takma ad MongoDB kimliğidir ve `sender` ise Kimden adresidir (izin verilenler listesindeyse) veya Kimden adresindeki kök etki alanıdır (izin verilenler listesinde değilse). Basitlik açısından, bu parmak izinin önbellekteki son kullanma tarihi 4 güne ayarlanmıştır.
...
2. Yalnızca MAIL FROM ve/veya From alanları boş olmadığında ve (büyük/küçük harfe duyarlı olmayan) [postacı kullanıcı adı](#what-are-postmaster-addresses) (bir e-postada @ işaretinden önceki kısım) içermediğinde gönderim yaparız.

3. Orijinal mesajda aşağıdaki başlıklardan herhangi biri varsa göndermiyoruz (büyük/küçük harfe duyarlı değil):

* `auto-submitted` başlığı, `no` değerine eşit değil. * `x-auto-response-suppress` başlığı, `dr`, `autoreply`, `auto-reply`, `auto_reply` veya `all` değerine sahip
* `list-id`, `list-subscribe`, `list-unsubscribe`, `list-help`, `list-post`, `list-owner`, `list-archive`, `x-autoreply` değerine sahip `x-autorespond` veya `x-auto-respond` (değerden bağımsız olarak).
* `bulk`, `autoreply`, `auto-reply`, `auto_reply` veya `list` değerine sahip `precedence` başlığı.

4. MAIL FROM veya From e-posta adresiniz `+donotreply`, `-donotreply`, `+noreply` veya `-noreply` ile bitiyorsa gönderim yapmayız.

5. E-posta adresinin kullanıcı adı kısmı `mdaemon` ise ve büyük/küçük harfe duyarlı olmayan `X-MDDSN-Message` başlığına sahipse göndermeyiz.

6. `multipart/report` başlığının büyük/küçük harfe duyarlı olmayan `content-type` başlığı varsa göndermiyoruz.

### {#how-do-i-set-up-spf-for-forward-email} E-posta Yönlendirme için SPF'yi nasıl ayarlarım?
...
### {#how-do-i-set-up-dmarc-for-forward-email} E-posta Yönlendirme için DMARC'ı nasıl ayarlarım?

<a href="/my-account/domains" class="alert-link" target="_blank" rel="noopener noreferrer">Hesabım <i class="fa fa-angle-right"></i> Alan Adları</a> <i class="fa fa-angle-right"></i> Ayarlar <i class="fa fa-angle-right"></i> Giden SMTP Yapılandırması'na gidin ve kurulum talimatlarını izleyin.

### Kişilerimi nasıl bağlayabilir ve yapılandırabilirim? {#how-do-i-connect-and-configure-my-contacts}

**Kişilerinizi yapılandırmak için şu CardDAV URL'sini kullanın:** `https://carddav.forwardemail.net` (veya istemciniz izin veriyorsa basitçe `carddav.forwardemail.net`)

### Takvimlerimi nasıl bağlayabilir ve yapılandırabilirim? {#how-do-i-connect-and-configure-my-calendars}

**Takviminizi yapılandırmak için şu CalDAV URL'sini kullanın:** `https://caldav.forwardemail.net` (veya istemciniz izin veriyorsa basitçe `caldav.forwardemail.net`)

<img width="612" height="520" src="/img/faq/calendar-setup.png" alt="E-posta Takvimini İletme CalDAV Thunderbird Örnek Kurulumu" />

### Daha fazla takvim nasıl eklerim ve mevcut takvimleri nasıl yönetirim? {#how-do-i-add-more-calendars-and-manage-existing-calendars}

Ek takvimler eklemek isterseniz, şu yeni takvim URL'sini eklemeniz yeterlidir: `https://caldav.forwardemail.net/dav/principals/calendar-name` (**`calendar-name` yerine istediğiniz takvim adını yazdığınızdan emin olun**)

Bir takvimin adını ve rengini oluşturduktan sonra değiştirebilirsiniz; bunun için tercih ettiğiniz takvim uygulamasını (örneğin Apple Mail veya [Şimşek kuşu](https://thunderbird.net)) kullanmanız yeterlidir.

### {#how-do-i-set-up-srs-for-forward-email} E-postasını İletmek için SRS'yi nasıl ayarlarım?

[Gönderen Yeniden Yazma Şeması](https://en.wikipedia.org/wiki/Sender_Rewriting_Scheme) ("SRS")'yi otomatik olarak yapılandırıyoruz; bunu kendiniz yapmanıza gerek yok.

### {#how-do-i-set-up-mta-sts-for-forward-email} E-postasını İletmek için MTA-STS'yi nasıl ayarlarım?

Daha fazla bilgi için lütfen [MTA-STS bölümümüz](#do-you-support-mta-sts) adresine bakın.
...
Evet, daha fazlasını şu adresten okuyabilirsiniz: <https://forwardemail.net/guides/newsletter-with-listmonk>.

Lütfen, IP itibarını korumak ve teslimatı garanti altına almak için Forward Email'in **bülten onayı** için alan adı bazında manuel bir inceleme süreci uyguladığını unutmayın. Onay için <support@forwardemail.net> adresine e-posta gönderin veya [yardım isteği](https://forwardemail.net/help) adresini açın. Bu işlem genellikle 24 saatten kısa sürer ve çoğu istek 1-2 saat içinde karşılanır. Yakın gelecekte, ek spam kontrolleri ve uyarılarla bu süreci anında gerçekleştirmeyi hedefliyoruz. Bu süreç, e-postalarınızın gelen kutusuna ulaşmasını ve mesajlarınızın spam olarak işaretlenmemesini sağlar.
...
Evet, 16 Ekim 2023 itibarıyla tüm ücretli kullanıcılar için eklenti olarak IMAP üzerinden e-posta almayı destekliyoruz. **Lütfen [Şifreli SQLite posta kutusu depolama özelliğimiz nasıl çalışır?](/blog/docs/best-quantum-safe-encrypted-email-service) başlıklı ayrıntılı makalemizi okuyun**.
...
<span><a class="alert-link" href="https://www.thunderbird.net/" target="_blank" rel="noopener noreferrer">Thunderbird</a>, <a class="alert-link" href="https://www.thunderbird.net/en-US/mobile/" target="_blank" rel="noopener noreferrer">Thunderbird Mobile</a>, <a class="alert-link" href="https://apps.apple.com/us/app/mail/id1108187098" target="_blank" kullanmanızı öneririz. rel="noopener noreferrer">Apple Mail</a> veya <a href="/blog/open-source" class="alert-link" target="_blank">açık kaynaklı ve gizlilik odaklı bir alternatif</a>.</span>
</div>

4. IMAP sunucu adı istendiğinde `imap.forwardemail.net` girin

5. IMAP sunucu portu istendiğinde `993` (SSL/TLS) girin - gerekirse [alternatif IMAP bağlantı noktaları](/faq#what-are-your-imap-server-configuration-settings)'ye bakın.
<div class="alert my-3 alert-warning">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong class="font-weight-bold">
İpucu:
...
Evet, 4 Aralık 2023 itibarıyla tüm ücretli kullanıcılar için [POP3](https://en.wikipedia.org/wiki/Post_Office_Protocol) eklentisini eklenti olarak destekliyoruz. **Lütfen [Şifreli SQLite posta kutusu depolama özelliğimiz nasıl çalışır?](/blog/docs/best-quantum-safe-encrypted-email-service) hakkındaki ayrıntılı makalemizi okuyun**.
...
<span><a class="alert-link" href="https://www.thunderbird.net/" target="_blank" rel="noopener noreferrer">Thunderbird</a>, <a class="alert-link" href="https://www.thunderbird.net/en-US/mobile/" target="_blank" rel="noopener noreferrer">Thunderbird Mobile</a> ve <a class="alert-link" href="https://apps.apple.com/us/app/mail/id1108187098" target="_blank" kullanmanızı öneririz. rel="noopener noreferrer">Apple Mail</a> veya <a href="/blog/open-source" class="alert-link" target="_blank">açık kaynaklı ve gizlilik odaklı bir alternatif</a>.</span>
</div>

4. POP3 sunucu adı istendiğinde `pop3.forwardemail.net` girin

5. POP3 sunucu portu istendiğinde `995` (SSL/TLS) girin - gerekirse [alternatif POP3 portları](/faq#what-are-your-pop3-server-configuration-settings)'ye bakın.
<div class="alert my-3 alert-warning">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong class="font-weight-bold">
İpucu:
...
* WKD aramaları, e-postaların zamanında teslim edilmesini sağlamak için 1 saat boyunca önbelleğe alınır → bu nedenle, WKD anahtarınızı ekler, değiştirir veya kaldırırsanız, önbelleği manuel olarak temizleyebilmemiz için lütfen e-posta adresinizle birlikte `support@forwardemail.net` adresine e-posta gönderin.
...
* Yüklenen anahtarlar, PGP onay kutusu etkin/işaretli olduğu sürece geçerlidir.
* Web kancalarına gönderilen mesajlar şu anda PGP ile şifrelenmemektedir.
* Belirli bir yönlendirme adresiyle eşleşen birden fazla takma adınız varsa (örneğin, regex/joker karakter/tam kombinasyon) ve bunlardan birden fazlası yüklenmiş bir PGP anahtarı içeriyorsa ve PGP işaretliyse → size bir hata uyarı e-postası göndeririz ve mesajı yüklenmiş PGP anahtarınızla şifrelemeyiz. Bu durum çok nadirdir ve genellikle yalnızca karmaşık takma ad kurallarına sahip ileri düzey kullanıcılar için geçerlidir.
* **Gönderenin DMARC reddetme politikası varsa, MX sunucularımız üzerinden e-posta yönlendirmede PGP şifrelemesi uygulanmaz. *Tüm* postalarda PGP şifrelemesi gerekiyorsa, IMAP hizmetimizi kullanmanızı ve gelen postalar için takma adınız için PGP anahtarınızı yapılandırmanızı öneririz.**

**Web Anahtar Dizini kurulumunuzu <https://wkd.chimbosonic.com/> (açık kaynak) veya <https://www.webkeydirectory.com/> (özel) adresinden doğrulayabilirsiniz.**
...
<span><a href="#do-you-support-sending-email-with-smtp" class="alert-link">Giden SMTP hizmetimizi</a> kullanıyor ve şifrelenmemiş mesajlar gönderiyorsanız, <a class="alert-link" href="https://wiki.gnupg.org/WKD">Web Anahtar Dizini ("WKD")</a> kullanarak mesajları alıcı bazında otomatik olarak şifrelemeye çalışırız.</span>
</div>
...
| --------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Şimşek kuşu | Masaüstü | [Configure OpenPGP in Thunderbird](https://support.mozilla.org/en-US/kb/openpgp-thunderbird-howto-and-faq#w_i-have-never-used-openpgp-with-thunderbird-before-how-do-i-setup-openpgp) | Thunderbird'ün OpenPGP desteği yerleşiktir. |
| E-posta | Tarayıcı | [Mailvelope](https://mailvelope.com/) veya [FlowCrypt](https://flowcrypt.com/download) (özel lisans) | Gmail, OpenPGP'yi desteklemez, ancak açık kaynaklı [Mailvelope](https://mailvelope.com/) veya [FlowCrypt](https://flowcrypt.com/download) eklentisini indirebilirsiniz. |
| Elma Postası | macOS | [Free-GPGMail](https://github.com/Free-GPGMail/Free-GPGMail?tab=readme-ov-file#installation) | Apple Mail OpenPGP'yi desteklemez, ancak açık kaynaklı eklenti [Free-GPGMail](https://github.com/Free-GPGMail/Free-GPGMail?tab=readme-ov-file#installation)'ı indirebilirsiniz. |
| Elma Postası | iOS | [PGPro](https://github.com/opensourceios/PGPro/) veya [FlowCrypt](https://apps.apple.com/us/app/flowcrypt-encrypted-email/id1591754995) (özel lisans) | Apple Mail, OpenPGP'yi desteklemez; ancak açık kaynaklı [PGPro](https://github.com/opensourceios/PGPro/) veya [FlowCrypt](https://flowcrypt.com/download) eklentisini indirebilirsiniz. |
...
Evet, 2 Mart 2023 itibarıyla [MTA-STS](https://www.hardenize.com/blog/mta-sts)'i destekliyoruz. Alan adınızda etkinleştirmek isterseniz [bu şablon](https://github.com/jpawlowski/mta-sts.template)'u kullanabilirsiniz.
...
### Parolaları ve WebAuthn'ı destekliyor musunuz? {#do-you-support-passkeys-and-webauthn}
...
E-posta webhook'ları hakkında doküman mı arıyorsunuz? Daha fazla bilgi için <a href="/faq#do-you-support-webhooks" class="alert-link">Webhook'ları destekliyor musunuz?</a> bölümüne bakın.
<span>
</span>
</div>

Evet, 14 Ağustos 2024 itibarıyla bu özelliği ekledik. Artık Hesabım → Alan Adları → Ayarlar → Geri Dönen Webhook URL'sine gidip, giden SMTP e-postaları geri döndüğünde `POST` isteği göndereceğimiz `http://` veya `https://` URL'sini yapılandırabilirsiniz.
...
* `recipient` (Dize) - e-posta adresi Geri dönen veya hata veren alıcının
* `message` (Dize) - geri dönen ileti için ayrıntılı bir hata mesajı
* `response` (Dize) - SMTP yanıt mesajı
* `response_code` (Sayı) - ayrıştırılmış SMTP yanıt kodu
* `truth_source` (Dize) - yanıt kodu güvenilir bir kaynaktan geliyorsa, bu değer kök etki alanı adıyla doldurulur (örneğin, `google.com` veya `yahoo.com`)
* `bounce` (Nesne) - geri dönme ve reddetme durumunu ayrıntılı olarak açıklayan aşağıdaki özellikleri içeren bir nesne
* `action` (Dize) - sıçrama eylemi (ör. `"reject"`)
* `message` (Dize) - sıçrama nedeni (ör. `"Message Sender Blocked By Receiving Server"`)
* `category` (Dize) - sıçrama kategorisi (ör. `"block"`)
* `code` (Sayı) - sıçrama durum kodu (ör. `554`)
* `status` (Dize) - yanıt mesajından sıçrama kodu (ör. `5.7.1`)
* `line` (Sayı) - varsa, ayrıştırılmış satır numarası, [Zone-MTA sıçrama ayrıştırma listesinden](https://github.com/zone-eu/zone-mta/blob/master/config/bounces.txt) (ör. `526`)
* `headers` (Nesne) - giden e-posta için başlıkların anahtar değer çifti
* `bounced_at` (Dize) - Geri dönme hatasının meydana geldiği [ISO 8601](https://en.wikipedia.org/wiki/ISO\_8601) biçimlendirilmiş Tarih
...
* `bounce.category` değeri `"block"`, `"recipient"`, `"spam"` veya `"virus"` ise, kullanıcıyı listeden mutlaka kaldırmalısınız.
* Webhook yüklerini doğrulamanız gerekiyorsa (sunucumuzdan gerçekten geldiklerinden emin olmak için), [ters arama kullanarak uzak istemci IP adresini istemci ana bilgisayar adını çözün](https://nodejs.org/api/dns.html#dnspromisesreverseip) yapabilirsiniz; bu değer `smtp.forwardemail.net` olmalıdır.
* IP'yi [yayınlanan IP adreslerimiz](#what-are-your-servers-ip-addresses) ile de karşılaştırabilirsiniz.
* Webhook anahtarınızı almak için Hesabım → Alan Adları → Ayarlar → Webhook İmza Yükü Doğrulama Anahtarı'na gidin.
...
* Daha fazla bilgi için <https://github.com/forwardemail/free-email-forwarding/issues/235> adresindeki tartışmaya bakın. * Webhook uç noktanızın `200` durum koduyla yanıt vermesi için `5` saniyeye kadar bekleyeceğiz ve `1` saniyeye kadar yeniden deneyeceğiz.
* Bir istek göndermeye çalışırken geri dönen webhook URL'nizde bir hata tespit edersek, size haftada bir kez nezaket e-postası göndereceğiz.
...
Örneğin, `alias@example.com` adresine giden tüm e-postaların yeni bir [istek kutusu](https://requestbin.com/r/en8pfhdgcculn?inspect) test uç noktasına iletilmesini istiyorsam:
...
Ya da belki de `example.com` adresine giden tüm e-postaların bu uç noktaya iletilmesini istiyorsunuz:
...
* Webhook yüklerini doğrulamanız gerekiyorsa (sunucumuzdan gerçekten geldiklerinden emin olmak için), [ters arama kullanarak uzak istemci IP adresini istemci ana bilgisayar adını çözün](https://nodejs.org/api/dns.html#dnspromisesreverseip) yapabilirsiniz; bu değer `mx1.forwardemail.net` veya `mx2.forwardemail.net` olmalıdır.
* IP'yi [yayınlanan IP adreslerimiz](#what-are-your-servers-ip-addresses) ile de karşılaştırabilirsiniz.
...
* Güvenlik nedeniyle bu anahtarı istediğiniz zaman değiştirebilirsiniz.
* Webhook isteğimizdeki `X-Webhook-Signature` değerini hesaplayın ve bu anahtarı kullanarak hesaplanan gövde değeriyle karşılaştırın. Bunun nasıl yapılacağına dair bir örnek [bu Stack Overflow gönderisi](https://stackoverflow.com/a/68885281) adresinde mevcuttur.
* Daha fazla bilgi için <https://github.com/forwardemail/free-email-forwarding/issues/235> adresindeki tartışmaya bakın.
* Bir webhook, `200` durum koduyla yanıt vermezse, yanıtını [hata günlüğü oluşturuldu](#do-you-store-error-logs) içinde saklayacağız; bu, hata ayıklama için kullanışlıdır.
...
* [süper ajanın yeniden deneme yöntemi](https://ladjs.github.io/superagent/#retrying-requests) içinde kullanılan varsayılan durum ve hata kodlarına göre otomatik olarak yeniden deneyeceğiz (biz bakımcıyız).
* Kaynak tasarrufu sağlamak ve yanıt süresini kısaltmak için aynı uç noktaya gönderilen webhook HTTP isteklerini tek bir istekte gruplandırıyoruz. Örneğin, <webhook1@example.com>, <webhook2@example.com> ve <webhook3@example.com> adreslerine birer e-posta gönderirseniz ve bunların tümü aynı *tam* uç nokta URL'sine ulaşacak şekilde yapılandırılmışsa, yalnızca bir istek yapılır. Kesin eşitlikle tam uç nokta eşleşmesine göre gruplandırıyoruz.
* Mesajı JSON uyumlu bir nesneye ayrıştırmak için [posta ayrıştırıcısı](https://nodemailer.com/extras/mailparser/) kütüphanesinin "simpleParser" yöntemini kullandığımızı unutmayın.
...
* Ayrıştırılan e-posta başlıkları "headers" özelliği olarak verilir; ancak daha kolay yineleme ve ayrıştırma için "headerLines" özelliğini de kullanabileceğinizi unutmayın.
* Bu webhook için gruplandırılmış alıcılar birlikte gruplandırılır ve "recipients" özelliği olarak verilir.
* SMTP oturum bilgileri "session" özelliği olarak verilir. Bu, mesajın göndereni, mesajın varış saati, HELO ve istemci ana bilgisayar adı hakkında bilgi içerir. `session.clientHostname` olarak istemci ana bilgisayar adı değeri, ya FQDN'dir (ters PTR aramasından) ya da köşeli parantez içinde `session.remoteAddress`'dır (örneğin, `"[127.0.0.1]"`). * `X-Original-To` değerini almanın hızlı bir yoluna ihtiyacınız varsa, `session.recipient` değerini kullanabilirsiniz (aşağıdaki örneğe bakın). `X-Original-To` başlığı, mesaj için orijinal alıcıyla (maskeli yönlendirmeden önce) hata ayıklama yapmak amacıyla mesajlara eklediğimiz bir başlıktır.
* Yük gövdesinden `attachments` ve/veya `raw` özelliklerini kaldırmanız gerekirse, webhook uç noktanıza sorgu dizesi parametresi olarak `?attachments=false`, `?raw=false` veya `?attachments=false&raw=false` eklemeniz yeterlidir (örneğin, `https://example.com/webhook?attachments=false&raw=false`).
* Ekler varsa, bunlar Arabellek değerleriyle `attachments` Dizisine eklenecektir. Bunları JavaScript ile aşağıdaki gibi bir yaklaşım kullanarak içeriğe geri dönüştürebilirsiniz:
...
<div class="alert my-3 alert-secondary">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong>Artı Sembolü Filtreleme Değiştirme Örneği:</strong> `info@example.com` veya `support@example.com` adresine giden tüm e-postaların sırasıyla `user+info@gmail.com` veya `user+support@gmail.com` adresine iletilmesini istiyorsam (değiştirme desteğiyle) (<a href="https://regexr.com/66ho7" class="alert-link">RegExr'de testi görüntüle</a>):
</div>
...
<div class="alert my-3 alert-secondary">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong>Webhook Sorgu Dizisi Değiştirme Örneği:</strong> Belki de `example.com` adresine giden tüm e-postaların bir <a href="#do-you-support-webhooks" class="alert-link">webhook</a> adresine gitmesini ve e-posta adresinin kullanıcı adı kısmının değerine sahip "to" dinamik sorgu dizisi anahtarına sahip olmasını istiyorsunuz (<a href="https://regexr.com/66ho4" class="alert-link">RegExr'de testi görüntüle</a>):
</div>
...
<div class="alert my-3 alert-secondary">
<i class="fa fa-info-circle font-weight-bold"></i>
<strong>Sessiz reddetme örneği:</strong> Belirli bir desene uyan tüm e-postaların devre dışı bırakılmasını ve sessizce reddedilmesini (gönderene mesaj başarıyla gönderilmiş gibi görünür, ancak aslında hiçbir yere gitmez) ve `250` durum koduyla (bkz. <a href="#can-i-disable-specific-aliases" class="alert-link">Belirli takma adları devre dışı bırakabilir miyim</a>) istiyorsanız, aynı yaklaşımı tek bir ünlem işareti "!" ile kullanın. Bu, gönderene mesajın başarıyla iletildiğini, ancak aslında hiçbir yere gitmediğini (örneğin, kara delik veya `/dev/null`) belirtir.
</div>
...
Düzenli ifadenin nasıl yazılacağını mı merak ediyorsunuz veya yenisini test etmeniz mi gerekiyor? Ücretsiz düzenli ifade test web sitesi <a href="https://regexr.com" class="alert-link">RegExr</a>'e <a href="https://regexr.com/" class="alert-link">https://regexr.com</a>. adresinden ulaşabilirsiniz.
<span>
</span>
</div>
...
Kullanıcı ve alan adlarını günde 300 giden SMTP mesajıyla sınırlıyoruz. Bu, bir takvim ayında ortalama 9000'den fazla e-posta anlamına gelir. Bu miktarı aşmanız gerekiyorsa veya sürekli olarak büyük e-postalarınız varsa, lütfen [bize Ulaşın](https://forwardemail.net/help).

### SMTP'yi etkinleştirmek için onaya ihtiyacım var mı? {#do-i-need-approval-to-enable-smtp}

Evet, lütfen IP itibarını korumak ve teslimatı garanti altına almak için Forward Email'in, giden SMTP onayı için alan adı bazında manuel bir inceleme süreci uyguladığını unutmayın. Onay için <support@forwardemail.net> adresine e-posta gönderin veya [yardım isteği](https://forwardemail.net/help) adresini açın. Bu işlem genellikle 24 saatten kısa sürer ve çoğu istek 1-2 saat içinde karşılanır. Yakın gelecekte, ek spam kontrolleri ve uyarılarla bu süreci anında gerçekleştirmeyi hedefliyoruz. Bu süreç, e-postalarınızın gelen kutusuna ulaşmasını ve mesajlarınızın spam olarak işaretlenmemesini sağlar.
...
Adım adım talimatlar için lütfen [SMTP ile e-posta göndermeyi destekliyor musunuz?](#do-you-support-sending-email-with-smtp) adresine bakın.
...
Adım adım talimatlar için lütfen [IMAP ile e-posta almayı destekliyor musunuz?](#do-you-support-receiving-email-with-imap) adresine bakın.

### POP3 sunucunuzun yapılandırma ayarları nelerdir? {#what-are-your-pop3-server-configuration-settings}
...
Adım adım talimatlar için lütfen [POP3'ü destekliyor musunuz?](#do-you-support-pop3) adresine bakın.

### Sonek SMTP Röle Yapılandırması {#postfix-smtp-relay-configuration}

Postfix'i, Forward Email'in SMTP sunucuları üzerinden e-postaları iletecek şekilde yapılandırabilirsiniz. Bu, e-posta göndermesi gereken sunucu uygulamaları için yararlıdır.
...
Evet. Forward Email, tüm bağlantılar (HTTPS, SMTP, IMAP, POP3) için TLS 1.2+'yi sıkı bir şekilde uygular ve gelişmiş TLS desteği için MTA-STS'yi uygular. Uygulama şunları içerir:
...
Doğrulama: Internet.nl Mail Testi, özellikle "SPF, DKIM ve DMARC" uygulaması için 100/100 puan gösteriyor. Hardenize değerlendirmesi, SPF ve DMARC için "İyi" derecelendirmelerini doğruluyor: <https://www.hardenize.com/report/forwardemail.net/1750312779>
...
`isArbitrary` yardımcısı, alan adı taklitlerinin, engellenen ifadelerin ve çeşitli kimlik avı kalıplarının tespiti de dahil olmak üzere gelişmiş kimlik avı önleme kurallarını uygular.
...
* <https://forwardemail.net/technical-whitepaper.pdf#page=59> (Sıfır bilgi içeren ayrıntılar)
* <https://forwardemail.net/technical-whitepaper.pdf#page=21> (Korumalı şifreleme)

### Sistem çökmeleri sırasında e-posta içeriği açığa çıkabilir mi? {#can-email-content-be-exposed-during-system-crashes}

Hayır. Forward Email, çökmeyle ilgili veri ifşasına karşı kapsamlı güvenlik önlemleri uygular:
...
* **Düzenli Güvenlik İncelemeleri**: Erişim günlükleri için
...
| Sağlayıcı | Veri Gizliliği Çerçevesi Onaylı | GDPR Uyumluluk Sayfası |
| ---------------- | -------------------------------- | ----------------------------------------------- |
...
Forward Email'in teknik dokümantasyonu özel bir sandbox modunu açıkça tanımlamaz. Ancak, potansiyel test yaklaşımları şunları içerir:
...
**Çalışma Süresi Taahhüdü**: <https://forwardemail.net> adresinden şeffaf izleme ile %99,9+ hizmet kullanılabilirliği
...
Hayır, diske yazmıyoruz veya günlükleri depolamıyoruz – [hata istisnası](#do-you-store-error-logs) ve [giden SMTP](#do-you-support-sending-email-with-smtp) ile (bkz. [Gizlilik Politikası](/privacy)).

Her şey bellek içinde yapılır ve [kaynak kodumuz GitHub'da](https://github.com/forwardemail).

### E-posta yönlendirme sisteminiz nasıl çalışır? {#how-does-your-email-forwarding-system-work}

E-posta, [SMTP protokolü](https://en.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol) protokolüne dayanır. Bu protokol, bir sunucuya (çoğunlukla 25 numaralı bağlantı noktasında çalışır) gönderilen komutlardan oluşur. İlk bağlantı kurulur, ardından gönderen e-postanın kimden geldiğini ("MAIL FROM"), ardından nereye gideceğini ("RCPT TO") ve son olarak e-postanın başlıklarını ve gövdesini ("DATA") belirtir. E-posta yönlendirme sistemimizin akışı, aşağıda her SMTP protokol komutuna göre açıklanmıştır:

* İlk Bağlantı (komut adı yok, örneğin `telnet example.com 25`) - Bu ilk bağlantıdır. [izin listesi](#do-you-have-an-allowlist) listemizde olmayan göndericileri [inkar listesi](#do-you-have-a-denylist) listemizle karşılaştırırız. Son olarak, bir gönderici izin verilenler listemizde değilse, [gri listeye alınmış](#do-you-have-a-greylist) listesinde olup olmadığını kontrol ederiz.

* `HELO` - Bu, gönderenin tam alan adını (FQDN), IP adresini veya posta işleyicisi adını tanımlamak için bir karşılama mesajıdır. Bu değer sahte olabilir, bu nedenle bu verilere güvenmiyoruz ve bunun yerine bağlantının IP adresinin ters ana bilgisayar adı aramasını kullanıyoruz.

* `MAIL FROM` - Bu, e-postanın zarf posta adresini belirtir. Bir değer girilirse, geçerli bir RFC 5322 e-posta adresi olmalıdır. Boş değerlere izin verilir. Burada [geri saçılmayı kontrol edin](#how-do-you-protect-against-backscatter) ve ayrıca MAIL FROM'u [inkar listesi](#do-you-have-a-denylist) ile karşılaştırıyoruz. Son olarak, izin verilenler listesinde olmayan göndericileri hız sınırlaması açısından kontrol ediyoruz (daha fazla bilgi için [Hız Sınırlaması](#do-you-have-rate-limiting) ve [izin listesi](#do-you-have-an-allowlist) bölümlerine bakın).
...
### {#how-do-you-process-an-email-for-forwarding} e-postasını yönlendirmek için nasıl işlem yapıyorsunuz?

Bu bölüm, yukarıdaki [E-posta yönlendirme sisteminiz nasıl çalışır?](#how-does-your-email-forwarding-system-work) bölümündeki SMTP protokol komutu `DATA` ile ilgili sürecimizi açıklamaktadır; bu, bir e-postanın başlıklarını, gövdesini, güvenliğini nasıl işlediğimizi, nereye teslim edilmesi gerektiğini nasıl belirlediğimizi ve bağlantıları nasıl ele aldığımızı belirtir.
...
4. E-postanın parmak izini kullanarak (bkz. [Parmak izi](#how-do-you-determine-an-email-fingerprint) bölümü), mesajın 5 günden fazla süredir yeniden denenip denenmediğini ([varsayılan son ek davranışı](http://www.postfix.org/postconf.5.html#maximal_queue_lifetime) ile eşleşir) kontrol edeceğiz ve eğer öyleyse, 550 hata koduyla reddedilecektir.

5. E-postayı [Spam Tarayıcı](https://spamscanner.net) kullanarak tarayarak elde ettiğimiz sonuçları hafızamızda saklıyoruz.

6. Spam Tarayıcısından herhangi bir keyfi sonuç çıkarsa, 554 hata koduyla reddedilir. Keyfi sonuçlar, bu yazının yazıldığı tarihte yalnızca GTUBE testini içerir. Daha fazla bilgi için <https://spamassassin.apache.org/gtube/> adresine bakın.
...
* Bu, IMAP ve/veya maskeli yönlendirme sırasında (gizliliği korumak amacıyla) alıcı bazında eklenir.
...
* `X-Forward-Email-Session-ID` - hata ayıklama amacıyla kullanılan bir oturum kimliği değeri (yalnızca üretim dışı ortamlarda geçerlidir).
* `X-Forward-Email-Sender` - orijinal zarf MAIL FROM adresini (boş değilse), ters PTR istemci FQDN'sini (varsa) ve gönderenin IP adresini içeren virgülle ayrılmış bir liste.
...
* `X-Report-Abuse` - `abuse@forwardemail.net` değerine sahiptir.
* `X-Report-Abuse-To` - `abuse@forwardemail.net` değerine sahiptir.
* `X-Complaints-To` - `abuse@forwardemail.net` değeriyle.

8. Daha sonra [DKIM](https://en.wikipedia.org/wiki/DomainKeys_Identified_Mail), [SPF](https://en.wikipedia.org/wiki/Sender_Policy_Framework), [ARC](https://en.wikipedia.org/wiki/Authenticated_Received_Chain) ve [DMARC](https://en.wikipedia.org/wiki/DMARC) mesajlarını kontrol ediyoruz.
...
* İleti SPF'yi geçemediyse ve etki alanında bir kesin başarısızlık politikası varsa (ör. SPF politikasında `-all` varken `~all` veya hiç politika yoksa), 550 hata koduyla reddedilir. Genellikle bir etki alanı için SPF politikası, kök etki alanının <strong class="notranslate">TXT</strong> kaydında bulunabilir (ör. `dig example.com txt`). SPF ile ilgili [Gmail'de olduğu gibi posta gönderme](#can-i-send-mail-as-in-gmail-with-this) hakkında daha fazla bilgi için bu bölüme bakın.

9. Şimdi, yukarıdaki [E-posta yönlendirme sisteminiz nasıl çalışır?](#how-does-your-email-forwarding-system-work) bölümündeki `RCPT TO` komutundan toplanan mesajın alıcılarını işliyoruz. Her alıcı için aşağıdaki işlemleri gerçekleştiriyoruz:

* Alan adının <strong class="notranslate">TXT</strong> kayıtlarını ararız (örneğin, `@` sembolünden sonraki kısım, örneğin e-posta adresi `test@example.com` ise `example.com`). Örneğin, alan adı `example.com` ise, `dig example.com txt` gibi bir DNS araması yaparız.
* `forward-email=` (ücretsiz planlar) veya `forward-email-site-verification=` (ücretli planlar) ile başlayan tüm <strong class="notranslate">TXT</strong> kayıtlarını ayrıştırırız. Bir kullanıcı planlarını yükseltirken veya düşürürken e-postaları işlemek için her ikisini de ayrıştırdığımızı unutmayın. * Bu ayrıştırılmış <strong class="notranslate">TXT</strong> kayıtlarından, yönlendirme yapılandırmasını çıkarmak için bunlar üzerinde yineleme yaparız (yukarıdaki [E-posta yönlendirmeye nasıl başlayabilirim ve ayarlarım?](#how-do-i-get-started-and-set-up-email-forwarding) bölümünde açıklandığı gibi). Yalnızca bir `forward-email-site-verification=` değerini desteklediğimizi ve birden fazla değer girilmesi durumunda 550 hatası oluşacağını ve gönderenin bu alıcı için geri dönüş alacağını unutmayın.
* Genel yönlendirmeyi, regex tabanlı yönlendirmeyi ve artık "Yönlendirme Adreslerimiz" olarak bilinen diğer tüm desteklenen yönlendirme yapılandırmalarını belirlemek için çıkarılan yönlendirme yapılandırması üzerinde yineleme yaparız.
* Her Yönlendirme Adresi için, bir yinelemeli aramayı destekleriz (bu, belirtilen adreste bu işlem serisini başlatır). Yinelemeli bir eşleşme bulunursa, ana sonuç Yönlendirme Adreslerinden kaldırılır ve alt sonuçlar eklenir. * Yönlendirme Adresleri benzersizlik açısından ayrıştırılır (çünkü tek bir adrese yinelenen e-postalar göndermek veya gereksiz SMTP istemci bağlantıları oluşturmak istemiyoruz).
* Her Yönlendirme Adresi için, alan adını API uç noktamız `/v1/max-forwarded-addresses` ile karşılaştırırız (alan adının takma ad başına kaç adrese e-posta iletmesine izin verildiğini belirlemek için, örneğin varsayılan olarak 10 - [takma ad başına yönlendirme için maksimum sınır](#is-there-a-limit-on-the-number-of-email-addresses-i-can-forward-to-per-alias) bölümüne bakın). Bu sınır aşılırsa, 550 hatası oluşur ve gönderen bu alıcı için geri dönüş alır.
* Orijinal alıcının ayarlarını, ücretli kullanıcılar için bir arama (ücretsiz kullanıcılar için bir geri dönüş ile) destekleyen API uç noktamız `/v1/settings` ile karşılaştırırız. Bu, `port` (Sayı, ör. `25`), `has_adult_content_protection` (Boole değeri), `has_phishing_protection` (Boole değeri), `has_executable_protection` (Boole değeri) ve `has_virus_protection` (Boole değeri) için gelişmiş ayarlara yönelik bir yapılandırma nesnesi döndürür.
* Bu ayarlara dayanarak, Spam Tarayıcısı sonuçlarına göre kontrol yaparız ve herhangi bir hata oluşursa, ileti 554 hata koduyla reddedilir (ör. `has_virus_protection` etkinleştirilmişse, Spam Tarayıcısı sonuçlarında virüs olup olmadığını kontrol ederiz). Tüm ücretsiz plan kullanıcılarının yetişkinlere yönelik içerik, kimlik avı, yürütülebilir dosyalar ve virüslere karşı kontrollere dahil olacağını unutmayın. Varsayılan olarak, tüm ücretli plan kullanıcıları da katılabilir, ancak bu yapılandırma, E-posta İletme panosundaki bir alan adı için Ayarlar sayfasından değiştirilebilir.
...
* E-posta gönderimlerindeki en düşük hata kodunu (varsa) alacağız ve bunu `DATA` komutuna yanıt kodu olarak kullanacağız. Bu, teslim edilmeyen e-postaların genellikle orijinal gönderen tarafından yeniden deneneceği, ancak teslim edilmiş e-postaların bir sonraki mesaj gönderiminde yeniden gönderilmeyeceği anlamına gelir (bizim [Parmak izi](#how-do-you-determine-an-email-fingerprint) kullandığımız gibi).
...
Yalnızca gönderenin DMARC politikası geçmiyorsa VE hiçbir DKIM imzası "From" başlığıyla hizalanmamışsa e-postalarda "Friendly-From" yeniden yazma işlemini yapacağımızı unutmayın. Bu, mesajdaki "From" başlığını değiştireceğimiz, "X-Original-From"u ayarlayacağımız ve ayrıca önceden ayarlanmamışsa bir "Reply-To" ayarlayacağımız anlamına gelir. Ayrıca bu başlıkları değiştirdikten sonra mesajdaki ARC mührünü yeniden mühürleyeceğiz.
...
Yanlış yönlendirilmiş geri dönüşleri ve izlenmeyen veya var olmayan posta kutularına tatil yanıtlayıcı mesajları gönderilmesini önlemek için, mailer-daemon benzeri kullanıcı adlarından oluşan bir liste tutuyoruz:
...
Bu tür listelerin verimli e-posta sistemleri oluşturmak için nasıl kullanıldığına dair daha fazla bilgi için [RFC 5320 Bölüm 4.6](https://datatracker.ietf.org/doc/html/rfc5230#section-4.6) sayfasına bakın.
...
Bu liste [GitHub'da açık kaynaklı bir proje olarak](https://github.com/forwardemail/reserved-email-addresses-list) ile korunmaktadır.
...
Evet, varsayılan olarak izin verilenler listesine eklenmiş [alan adı uzantılarının listesi](#what-domain-name-extensions-are-allowlisted-by-default) ve [sıkı kriterler](#what-is-your-allowlist-criteria)'a dayalı dinamik, önbelleğe alınmış ve sürekli değişen bir izin listemiz var.

Ücretli planlardaki tüm e-postalar, alan adları ve alıcılar otomatik olarak izin verilenler listemize eklenir.
...
Yüksek spam etkinliği nedeniyle `cz`, `ru` ve `ua` kodlarını özellikle dahil etmedik.

### İzin listenizdeki kriterler nelerdir? {#what-is-your-allowlist-criteria}

[alan adı uzantıları varsayılan olarak izin verilenler listesine eklendi](#what-domain-name-extensions-are-allowlisted-by-default) adlı statik bir listemiz var; ayrıca aşağıdaki katı ölçütlere dayalı dinamik, önbelleğe alınmış, sürekli değişen bir izin listesi de tutuyoruz:

* Gönderen kök etki alanı, [ücretsiz planımızda sunduğumuz listeyle eşleşen alan adı uzantısı](#what-domain-name-extensions-can-be-used-for-free) (`biz` ve `info` ekleriyle) olmalıdır. Ayrıca `edu`, `gov` ve `mil` kısmi eşleşmelerini de dahil ediyoruz; örneğin `xyz.gov.au` ve `xyz.edu.au`.
* Gönderen kök etki alanı, [Şemsiye Popülerlik Listesi](http://s3-us-west-1.amazonaws.com/umbrella-static/index.html "Umbrella Popularity List") ("UPL")'den ayrıştırılan ilk 100.000 benzersiz kök etki alanı sonucu arasında olmalıdır.
* Gönderen kök etki alanı, son 7 günün en az 4'ünde UPL'lerde görünen benzersiz kök etki alanlarından ilk 50.000 sonuç içinde olmalıdır (~%50+).
* Gönderen kök etki alanı, Cloudflare tarafından yetişkinlere yönelik içerik veya kötü amaçlı yazılım olarak [kategorize edilmiş](https://radar.cloudflare.com/categorization-feedback/) olarak işaretlenmemelidir.
* Gönderen kök etki alanında A veya MX kayıtları ayarlanmış olmalıdır.
* Gönderen kök etki alanında A kaydı(ları), MX kaydı(ları), `p=reject` veya `p=quarantine` niteleyicili DMARC kaydı veya `-all` veya `~all` niteleyicili bir SPF kaydı bulunmalıdır.
...
İzin verilenler listemizde olmayan bir göndericiyseniz, FQDN kök etki alanınız veya IP adresiniz ilk kez bir e-posta gönderdiğinde [sınırlı oran](#do-you-have-rate-limiting) ve [gri listeye alınmış](#do-you-have-a-greylist) olursunuz. Bunun, e-posta standardı olarak benimsenen standart bir uygulama olduğunu unutmayın. Çoğu e-posta sunucusu istemcisi, bir hız sınırı veya gri liste hatası (örneğin, 421 veya 4xx düzeyinde bir hata durum kodu) alırsa yeniden denemeyi dener.

**`a@gmail.com`, `b@xyz.edu` ve `c@gov.au` gibi belirli göndericilerin hala [reddedildi](#do-you-have-a-denylist)** olabileceğini unutmayın (örneğin, bu göndericilerden otomatik olarak spam, kimlik avı veya kötü amaçlı yazılım tespit edersek).
...
Evet, oldukça gevşek bir [e-posta gri listeleme](https://en.wikipedia.org/wiki/Greylisting_\(email\)) politikamız var. Gri listeleme yalnızca izin verilenler listemizde olmayan gönderenler için geçerlidir ve önbelleğimizde 30 gün kalır.
...
Örneğin, `test.example.com` adresinden bir e-posta gelirse ve `example.com` adresinden bir e-posta geldiğini görmezsek, `test.example.com` ve/veya `example.com` adresinden gelen tüm e-postalar, bağlantının ilk varış saatinden itibaren 5 dakika beklemek zorunda kalacaktır. `test.example.com` ve `example.com` adreslerinin her birinin kendi 5 dakikalık sürelerini beklemesini zorunlu kılmıyoruz (gri listeleme politikamız kök etki alanı düzeyinde geçerlidir).

Gri listelemenin [izin listesi](#do-you-have-an-allowlist) bağlantımızdaki hiçbir göndericiye (örneğin, bu yazının yazıldığı tarihte Meta, Amazon, Netflix, Google, Microsoft) uygulanmadığını unutmayın.
...
Ayrıca her saat <http://wget-mirrors.uceprotect.net/rbldnsd-all/dnsbl-1.uceprotect.net.gz> adresindeki UCEPROTECT Seviye 1 red listesinden tüm IP adreslerini çekiyoruz ve bunları 7 günlük bir süre sonunda red listemize ekliyoruz.

Reddedilenler listesinde bulunan gönderenler, [izin verilenler listesinde değil](#do-you-have-an-allowlist) kodunu kullanırlarsa 421 hata kodunu alırlar (gönderene daha sonra tekrar denemesini belirtir).
...
1. "Güvenilir" bir göndericiden (örneğin, `gmail.com`, `microsoft.com`, `apple.com`) spam veya engelleme listesi tespit edildiğinde [ilk mesaj parmak izi](#how-do-you-determine-an-email-fingerprint) gri listeye alınır.
...
2. Gönderen ve mesajdan gelen bilgilerden reddetme listesi anahtarlarını ayrıştırırız ve bu anahtarların her biri için (henüz mevcut değilse) bir sayaç oluşturur, 1 artırır ve 24 saat boyunca önbelleğe alırız. * İzin verilenler listesindeki gönderenler için:
* "MAIL FROM" zarf e-posta adresi, SPF'si geçiyorsa veya SPF'si yoksa ve [bir postacı kullanıcı adı](#what-are-postmaster-addresses) veya [cevap verilmeyen kullanıcı adı](#what-are-no-reply-addresses) değilse, bir anahtar ekleyin.
* "From" başlığı izin verilenler listesindeyse, "From" başlığı e-posta adresi, SPF'si geçiyorsa veya DKIM'si geçiyorsa, bir anahtar ekleyin.
...
* "MAIL FROM" zarf e-posta adresi, SPF'si geçiyorsa veya DKIM'si geçiyorsa, bir anahtar ekleyin.
* "From" başlığı izin verilenler listesindeyse, "From" başlığı e-posta adresi, SPF'si geçiyorsa veya DKIM'si geçiyorsa, bir anahtar ekleyin. * "Kimden" başlığı izin verilenler listesine eklenmemişse, "Kimden" başlığı e-posta adresi ve kök ayrıştırılmış alan adı için bir anahtar ekleyin.
* Gönderenin uzak IP adresi için bir anahtar ekleyin.
* Gönderenin IP adresinden ters arama yaparak istemci tarafından çözümlenen ana bilgisayar adı için bir anahtar ekleyin (varsa).
* İstemci tarafından çözümlenen ana bilgisayar adının kök alan adı için bir anahtar ekleyin (varsa ve istemci tarafından çözümlenen ana bilgisayar adından farklıysa).
...
### {#do-you-have-rate-limiting} adresinde hız sınırlamanız var mı?
...
* Gelen postaları tek tek takma adlar temelinde (ör. `you@yourdomain.com`) sınırlamak yerine, takma adın alan adına göre (ör. `yourdomain.com`) sınırlama getiriyoruz. Bu, `Senders` kodunun alan adınızdaki tüm takma adların gelen kutularını aynı anda doldurmasını önler. * Alıcıdan bağımsız olarak hizmetimiz genelindeki tüm `Senders` için geçerli genel sınırlamalarımız vardır:
* "Güvenilir" bir gerçek kaynağı olarak kabul ettiğimiz `Senders` (örneğin, `gmail.com`, `microsoft.com`, `apple.com`) için günlük 100 GB gönderim sınırı vardır.
* [izin verilenler listesinde](#do-you-have-an-allowlist) olan `Senders` için günlük 10 GB gönderim sınırı vardır.
* Diğer tüm `Senders` için günlük 1 GB ve/veya 1000 mesaj gönderim sınırı vardır.
* `Sender` ve `yourdomain.com` başına günlük 1 GB ve/veya 1000 mesajlık belirli bir sınırımız var.
...
* `Sender` çözümlenmiş FQDN kök etki alanı (veya) `Sender` uzak IP adresi (ters PTR mevcut değilse) ve zarf alıcısı başına saatte en fazla 100 bağlantıya izin veriyoruz. Hız sınırlama anahtarını Redis veritabanımızda kriptografik bir karma olarak saklıyoruz.
...
* `test.abc.123.example.com` gibi bir alan adından gönderim yapıyorsanız, oran sınırı `example.com` için geçerli olacaktır. Birçok spam gönderici, benzersiz FQDN kök alan adlarının aksine yalnızca benzersiz ana bilgisayar adlarına oran sınırı uygulayan yaygın spam filtrelerini aşmak için yüzlerce alt alan adı kullanır.

* Oran sınırını aşan `Senders` 421 hatasıyla reddedilecektir.

IMAP ve SMTP sunucularımız, takma adlarınızın aynı anda `60`'den fazla eş zamanlı bağlantıya sahip olmasını sınırlar.

MX sunucularımız [izin verilenler listesinde olmayan](#do-you-have-an-allowlist) göndericilerinin 10'dan fazla eş zamanlı bağlantı kurmasını sınırlar (sayaç için 3 dakikalık önbellek süresi dolma süresiyle, bu da 3 dakikalık soket zaman aşımımızı yansıtır).
...
Listeyi her saat <http://wget-mirrors.uceprotect.net/rbldnsd-all/ips.backscatterer.org.gz> adresindeki [Backscatter.org](https://www.backscatterer.org/) adresinden ([UCEPROTECT](https://www.uceprotect.net/) tarafından desteklenmektedir) çekiyoruz ve Redis veritabanımıza giriyoruz (ayrıca, herhangi bir IP'nin kaldırılması durumunda, dikkate alınması gereken farkları önceden karşılaştırıyoruz).

MAIL FROM boşsa VEYA (büyük/küçük harfe duyarlı değil) [posta müdürü adresleri](#what-are-postmaster-addresses) (bir e-postadaki @ işaretinden önceki kısım) kısımlarından herhangi birine eşitse, gönderici IP'sinin bu listeden biriyle eşleşip eşleşmediğini kontrol ederiz.

Gönderenin IP'si listelenmişse (ve [izin listesi](#do-you-have-an-allowlist) listemizde değilse), `The IP ${session.remoteAddress} is blocked by https://www.backscatterer.org/index.php?target=test&ip=${session.remoteAddress}` mesajıyla bir 554 hatası göndeririz. Bir gönderici hem Backscatterer listesinde hem de izin verilenler listemizdeyse, gerekirse sorunu çözebilmemiz için bize bildirim gönderilir.

Bu bölümde açıklanan teknikler, <https://www.backscatterer.org/?target=usage> adresindeki "GÜVENLİ MOD" önerisine uymaktadır; burada yalnızca belirli koşullar zaten karşılanmışsa gönderici IP'sini kontrol ederiz.
...
3. Yalnızca MAIL FROM ve/veya From alanları boş olmadığında ve (büyük/küçük harfe duyarlı olmayan) [postacı kullanıcı adı](#what-are-postmaster-addresses) (bir e-postada @ işaretinden önceki kısım) içermediğinde gönderim yaparız.

4. Orijinal mesajda aşağıdaki başlıklardan herhangi biri varsa göndermiyoruz (büyük/küçük harfe duyarlı değil):

* `auto-submitted` başlığı, `no` değerine eşit değil. * `dr`, `autoreply`, `auto-reply`, `auto_reply` veya `all` değerine sahip `x-auto-response-suppress` başlığı
* `list-id`, `list-subscribe`, `list-unsubscribe`, `list-help`, `list-post`, `list-owner`, `list-archive` başlığı `x-autoreply`, `x-autorespond` veya `x-auto-respond` (değerden bağımsız olarak).
* `bulk`, `autoreply`, `auto-reply`, `auto_reply` veya `list` değerine sahip `precedence` başlığı.

5. MAIL FROM veya From e-posta adresiniz `+donotreply`, `-donotreply`, `+noreply` veya `-noreply` ile bitiyorsa gönderim yapmayız.

6. E-posta adresinin kullanıcı adı kısmı `mdaemon` ise ve büyük/küçük harfe duyarlı olmayan `X-MDDSN-Message` başlığına sahipse göndermeyiz.
...
Bir e-postanın parmak izi, e-postanın benzersizliğini belirlemek ve yinelenen mesajların iletilmesini ve [yinelenen sıçramalar](#prevent-unnecessary-bounces-to-protect-against-backscatter) gönderilmesini önlemek için kullanılır.
...
E-postaları `foo.example.com` ile iletmek istiyorsanız, DNS ayarlarınızda (hem MX hem de <strong class="notranslate">TXT</strong> kayıtları için) ad/ana bilgisayar/takma ad değerini `foo` olarak girin.

### Bu, e-postamın başlıklarını iletir mi? {#does-this-forward-my-emails-headers}

Evet, kesinlikle.

### Bu iyi test edilmiş bir {#is-this-well-tested} mi?
...
:tangerine: [mandalina](https://github.com/forwardemail/tangerine) adında açık kaynaklı bir yazılım projesi oluşturduk ve bunu DNS aramaları için kullanıyoruz. Kullanılan varsayılan DNS sunucuları `1.1.1.1` ve `1.0.0.1`'tür ve DNS sorguları uygulama katmanında [HTTPS üzerinden DNS](https://en.wikipedia.org/wiki/DNS_over_HTTPS) ("DoH") üzerinden yapılır.

:tangerine: [mandalina](https://github.com/tangerine) varsayılan olarak [CloudFlare'in gizlilik odaklı tüketici DNS hizmetini] kullanır[cloudflare-dns].
...
### Plan değiştirirsem, ücreti orantılı olarak ödeyip aradaki farkı iade ediyor musunuz? {#if-i-switch-plans-do-you-pro-rate-and-refund-the-difference}
...
Örneğin, `alias@example.com` adresine giden tüm e-postaların `user@gmail.com` adresine akışını durdurmasını ve reddedilip geri dönmesini istiyorsam (örneğin üç ünlem işareti kullanın):
...
Örneğin, `*@example.com` adresine giden her e-postanın (yıldız işareti joker karakter anlamına gelir, yani her şeyi kapsar) `user+a@gmail.com` ve `user+b@gmail.com` adreslerine yönlendirilmesini istiyorsam, <strong class="notranslate">TXT</strong> kaydım şöyle görünecektir:
...
### {#is-there-a-maximum-limit-on-the-number-of-email-addresses-i-can-forward-to-per-alias} takma adına göre yönlendirebileceğim e-posta adresi sayısında bir üst sınır var mı?

Evet, varsayılan sınır 10'dur. Bu, alan adınızda yalnızca 10 takma adınız olabileceği anlamına GELMEZ. İstediğiniz kadar takma adınız olabilir (sınırsız miktarda). Bu, yalnızca bir takma adı 10 benzersiz e-posta adresine iletebileceğiniz anlamına gelir. `hello:user+1@gmail.com`, `hello:user+2@gmail.com`, `hello:user+3@gmail.com`, … (1-10 arası) olabilir ve `hello@example.com` adresine gönderilen tüm e-postalar `user+1@gmail.com`, `user+2@gmail.com`, `user+3@gmail.com`, … (1-10 arası) adreslerine yönlendirilir.
...
Evet, yapabilirsiniz, ancak yine de maksimum sınıra uymalısınız. `hello:linus@example.com` ve `linus:user@gmail.com` varsa, `hello@example.com` adresine gönderilen e-postalar `linus@example.com` ve `user@gmail.com` adreslerine yönlendirilir. Maksimum sınırın üzerindeki e-postaları yinelemeli olarak iletmeye çalışırsanız bir hata oluşacağını unutmayın.
...
Forward Email, açık kaynaklı geliştirme, verimli altyapı ve hizmeti destekleyen isteğe bağlı ücretli planların bir kombinasyonu yoluyla ücretsiz bir katman sunar.
...
### E-posta kayıtlarını saklıyor musunuz? {#do-you-store-logs-of-emails}
...
**Evet. Hata günlüklerine [Hesabım → Günlükler](/my-account/logs) veya [Hesabım → Alan Adları](/my-account/domains) adresinden erişebilirsiniz.**

Şubat 2023 itibarıyla, SMTP hatasını, zarfı ve e-posta başlıklarını içeren `4xx` ve `5xx` SMTP yanıt kodlarına ait hata günlüklerini 7 günlük bir süre boyunca saklıyoruz (e-posta gövdesini veya ekleri **saklamıyoruz**).
...
Daha fazla bilgi için [Gizlilik Politikası](/privacy) sayfamıza bakın.

### E-postalarımı okuyor musunuz? {#do-you-read-my-emails}

Hayır, kesinlikle hayır. [Gizlilik Politikası](/privacy) sayfamıza bakın.
...
Gizliliğinize saygı duymanız gerektiğine inanıyor ve buna kesinlikle saygı duyuyoruz. Sunucuya dağıtılan kod, şeffaflık ve güven oluşturmak amacıyla [GitHub'da açık kaynaklı yazılım](https://github.com/forwardemail) olarak belirlenmiştir.
...
### Outlook'ta bu {#can-i-send-mail-as-in-outlook-with-this} ile "postaları şu şekilde gönderebilir miyim?"
...
Daha fazla bilgi için lütfen <https://support.apple.com/en-us/102540> adresini ziyaret edin.
...
"Nispeten bilinmeyen" derken, [izin listesi](#do-you-have-an-allowlist)'te görünmeyen göndericileri kastediyoruz.
...
Tüm ödemeler Stripe veya PayPal aracılığıyla güvenli bir şekilde işlenir. Ödeme bilgileriniz asla sunucularımızda saklanmaz.
...
* [Vaka Çalışmaları ve Geliştirici Belgeleri](/blog/docs)
* [Kaynaklar](/resources)
* [Rehberler](/guides)

[gmail-2fa]: __KORUMA_URL_868__

[cloudflare-dns]: __KORUNMALI_URL_869__