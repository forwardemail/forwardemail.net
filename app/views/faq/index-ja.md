...
<div class="alert my-3 alert-danger">
  ...
  <span>Google はこの Gmail の「Send Mail As」設定を廃止しました。<a class="alert-link" href="https://support.google.com/mail/answer/17101213" target="_blank" rel="noopener noreferrer">Google announcement</a> をご確認ください。代わりに <a class="alert-link" href="https://mail.forwardemail.net" target="_blank" rel="noopener noreferrer">Forward Email Webmail</a>、<a class="alert-link" href="https://github.com/forwardemail/mail.forwardemail.net" target="_blank" rel="noopener noreferrer">Forward Email apps</a>、または別のメールクライアントを使用してください。ヘルプは <a class="alert-link" href="/faq">Forward Email FAQ</a> を参照してください。</span>
</div>
...
悪用を防ぎつつ正当な利用には柔軟性を保つため、当社では送信SMTPのレート制限を複数のレベルで実施しています。各レベルは順にチェックされ — 最初に到達した制限がそのメッセージを一時的に `421` エラー（「後でもう一度試してください」を意味します）で拒否します。

**Rate limit hierarchy:**

| レベル | 範囲 | デフォルト制限 | 説明 |
| :---- | :---- | :-----------: | :----------: |
| エイリアス単位 | 個別のエイリアス | なし（ドメイン制限を使用） | 任意です。エイリアスにカスタムの `smtp_limit` が設定されている場合は、まずそれがチェックされます。 |
| ドメイン単位 | ドメインから1日に送信されるすべてのメール | 300/day | ドメイン内のすべてのエイリアスで送信された全ての送信メールをカウントします。 |
| ユーザー単位 | ユーザーアカウントが1日に送信するすべてのメール | 300/day | エイリアスやドメインを削除して再作成することで回避することを防ぎます。 |

**How the effective limit is determined:**

* **チームプランのドメイン** — ドメインの実効日次制限は、そのドメインのすべての管理者メンバーの中で最も高い `smtp_limit` になります。例えば、ある管理者が300、別の管理者が500の制限を持っている場合、ドメインの実効制限は500になります。
* **Enhanced Protection およびその他のプラン** — 実効日次制限は送信ユーザー自身の `smtp_limit`（デフォルトは1日あたり300メッセージ）になります。
* **エイリアス単位の上書き** — ドメイン管理者は個々のエイリアスにカスタム `smtp_limit` を任意で設定できます。設定されている場合は、まずこれがチェックされます（ドメインおよびユーザー制限より前に）。特定のエイリアスをより低い送信量に制限したい場合に便利です。
* **新規エイリアスのドメインデフォルト** — ドメイン管理者はドメインに `alias_default_smtp_limit` を設定できます（API またはダッシュボードの Advanced Settings 経由）。設定されていると、そのドメインで新しく作成されるすべてのエイリアスは自動的にこの値を `smtp_limit` として継承します。これはドメインの有効な SMTP 制限を超えることはできません。既存のエイリアスには影響しません。無効にするには `0` に設定してください。

**システム管理者**（Forward Email のスタッフ）はすべてのレート制限の適用対象外です。

すべてのレート制限は、現在の日の開始（UTC の深夜）以降に作成されたメールに対して、データベースのカウント（`Emails.countDocuments`）を用いて強制されます。つまり、制限は毎日 UTC の深夜にリセットされます。

より高い制限が必要な場合は、[お問い合わせください](https://forwardemail.net/help)。ほとんどのリクエストは1〜2時間以内に対応されます。
...
* **メール認証の強制**: 許可リストに含まれない送信者からのメッセージは、SPFまたはDKIMの少なくとも1つに合格する必要があります（2024年以降のGmail、Outlook、Yahooの要件と同様）。認証に一切合格しないメッセージは550エラーコードで拒否されます。DMARCポリシー`p=reject`および`p=quarantine`が適用されます。
* **HELO/EHLO拒否リストチェック**: SMTP HELO/EHLOグリーティングで提示されたホスト名を拒否リストに対してチェックし、逆引きDNSレコードがないIPv6送信者に対しても保護を提供します
...
* **制御されたスワップの使用**: アプリケーションおよびその他の非データベースホストはスワップフリーのままであり、通常のワークロードがRAMに収まるようにサイジングおよび監視されます。MongoDBおよびRedisホストは、通常の容量としてではなく、メモリ不足時のセーフティネットとして厳密に`vm.swappiness=1`のroot専用スワップファイルを使用します。すべての場所でスワップを無効にすると、一時的なデータベースのメモリ逼迫が即座にOOMキルに変わる可能性があります。データベースのスワップアクティビティは監視および調査されます。
...
* `HELO` - 送信者のFQDN、IPアドレス、またはメールハンドラ名を識別する挨拶を示します。この値は偽装可能なため、このデータには依存せず、接続元IPアドレスの逆引きホスト名を使用します。 ただし、現在はこの値を逆引きホスト名に加えて[拒否リスト](#do-you-have-a-denylist)に対してもチェックしています。これにより、逆引きDNSが利用できないIPv6接続に対して追加の保護層を提供します。
