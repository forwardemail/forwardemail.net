...
* [Email Configuration](#email-configuration)
  ...
  * [Why do I see unknown IP addresses failing SPF and DKIM in my DMARC reports](#why-do-i-see-unknown-ip-addresses-failing-spf-and-dkim-in-my-dmarc-reports)
  * [Will external DMARC monitoring tools show different results than Forward Email](#will-external-dmarc-monitoring-tools-show-different-results-than-forward-email)
...
> \[!TIP]
> Seeing unfamiliar IP addresses failing authentication in your reports? See [Why do I see unknown IP addresses failing SPF and DKIM in my DMARC reports](#why-do-i-see-unknown-ip-addresses-failing-spf-and-dkim-in-my-dmarc-reports) and [Will external DMARC monitoring tools show different results than Forward Email](#will-external-dmarc-monitoring-tools-show-different-results-than-forward-email).

### Why do I see unknown IP addresses failing SPF and DKIM in my DMARC reports

This is normal and expected – it does **not** indicate a problem with your domain, your mail, or our service.

DMARC aggregate reports are XML reports that receiving mail services send to the reporting (`rua`) address in your DMARC record. They list **every** source that attempted to send mail using your domain, including unauthorized ones. In almost every case, the unfamiliar IP addresses you see failing authentication – whether IPv4 or IPv6 – are simply **spammers attempting to impersonate your domain**.

When a spammer tries to impersonate your domain:

* **SPF fails** because the sending IP address is not authorized by your domain.
* **DKIM fails** because they cannot produce a valid cryptographic signature for your domain.
* Because your domain publishes a DMARC policy of `p=reject`, the message is **rejected** outright.

In other words, these report entries are evidence that DMARC is doing exactly what it is designed to do: detecting and rejecting impersonation attempts. **No action is required** – these failures are not your legitimate mail.

To tell legitimate senders apart from impersonators:

* Your own authorized senders (including mail sent through Forward Email) will show SPF and/or DKIM **passing and aligned**.
...
### Will external DMARC monitoring tools show different results than Forward Email

No. An external DMARC monitoring service and Forward Email's built-in dashboard show the **same underlying data** for the same mail, because **DMARC reports are generated by the receiving mail server, not by Forward Email.**

**How reports actually flow**

Aggregate (`rua`) reports are produced by the mailbox provider that received your message (Google, Microsoft, Yahoo, etc.) and are sent to whatever addresses are listed in your domain's DMARC `rua=` tag. Forward Email does not author or modify these reports for your outbound mail – our dashboard simply ingests the copies addressed to your Forward Email reporting address, and a third-party tool ingests the copies sent to its address. If you list both addresses in `rua=` (comma-separated), both receive identical reports.

> \[!IMPORTANT]
> Our dashboard only displays reports that were sent to your Forward Email reporting address. If you point `rua=` at an external-only address, our dashboard will appear empty – this means we are not receiving copies, **not** that everything is passing. List both addresses if you want reports in both places.

**Do our in-flight changes affect what an external tool sees?**

Our in-flight changes only apply to mail that actually routes **through** Forward Email, and where they apply they are part of what the receiving server sees – so an external tool reflects the same passing/aligned result:

* **Mail you send through our Outbound SMTP relay**: we sign with aligned DKIM and use an aligned Return-Path. The receiver sees this and reports it as passing – identically in our dashboard and in any external tool. There is no discrepancy.
* **Mail you send without using us** (another provider or service): we make no changes at all, so an external tool – and our dashboard – will faithfully show the real authentication results, including any failures, which must be fixed at the sending service.
* **Inbound mail we forward to you**: aggregate reports for this mail are generated by your final mailbox provider about the *original sender's* domain and are delivered to *their* reporting address, not yours, so they generally do not appear in your own domain's reports.

**One thing that can look like a disagreement (but isn't)**

Some external dashboards display **strict** per-mechanism alignment. For Outbound SMTP we use a Return-Path on a **subdomain** of your domain, which is **relaxed** SPF alignment (an organizational-domain match). DMARC uses relaxed alignment by default, so this still passes – but a tool showing strict alignment may display something like "SPF strictly aligned: no." Your DKIM `d=` is your exact domain, so DKIM alignment is clean under both strict and relaxed modes. As long as DMARC is passing, relaxed SPF alignment is not a problem.
