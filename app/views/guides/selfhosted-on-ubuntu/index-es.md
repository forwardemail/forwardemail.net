...
ufw allow 587/tcp   # Envío SMTP (STARTTLS)
...
# nginx (el servicio sni-router) termina TLS en el puerto 443 y hace de proxy
# a la aplicación web en el 3000 (consulta docker-compose-self-hosted.yml y
# nginx.conf). Deja WEB_PORT en 3000; ponerlo en 443 entra en conflicto con
# nginx en la red del host y deja la interfaz web inaccesible.
...
# Añadir tarea cron de copia de seguridad de Redis (se ejecuta a diario a las 00:30,
# escalonada 30 minutos después de la de MongoDB para que ambas subidas no compitan por el mismo ancho de banda)
