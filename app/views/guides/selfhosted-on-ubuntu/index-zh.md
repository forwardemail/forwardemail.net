# Forward Email 自托管安装指南（适用于 Ubuntu）{#forward-email-self-hosting-installation-guide-for-ubuntu}
...
* [逐步安装](#step-by-step-installation)
  ...
  * [步骤 8：克隆 Forward Email 仓库](#step-8-clone-forward-email-repository)
  ...
  * [步骤 14：设置基本认证](#step-14-set-up-basic-authentication)
...
EOF
```
### 第3步：安装系统依赖 {#step-3-install-system-dependencies}
...
### 第4步：安装 Snap 软件包 {#step-4-install-snap-packages}
...
### 第5步：安装 Docker {#step-5-install-docker}
...
### 第6步：配置 Docker 服务 {#step-6-configure-docker-service}
...
### 第7步：配置防火墙 {#step-7-configure-firewall}
...
### 第8步：克隆 Forward Email 仓库 {#step-8-clone-forward-email-repository}
...
### 第9步：设置环境配置 {#step-9-set-up-environment-configuration}
...
### 第10步：配置您的域名 {#step-10-configure-your-domain}
...
```
### 第11步：生成 SSL 证书 {#step-11-generate-ssl-certificates}

#### 选项 A：手动 DNS 验证（推荐大多数用户） {#option-a-manual-dns-challenge-recommended-for-most-users}
...
### 第12步：生成加密密钥 {#step-12-generate-encryption-keys}
...
### 第13步：更新配置中的 SSL 路径 {#step-13-update-ssl-paths-in-configuration}
...
### 第14步：设置基本认证 {#step-14-set-up-basic-authentication}
...
### 第15步：使用 Docker Compose 部署 {#step-15-deploy-with-docker-compose}
...
```
### 第16步：验证安装 {#step-16-verify-installation}
...
1. 打开您的网页浏览器并访问 `https://yourdomain.com`
2. 输入您之前保存的基本认证凭据
3. 完成初始设置向导
4. 创建您的第一个邮箱账户
...
#### 4. 无法访问 Web 界面 {#4-web-interface-not-accessible}
