...
The recommended installation method is the automated setup script, which detects Debian and handles OS-specific differences (like installing snapd) for you. A full manual walkthrough is included as an [advanced appendix](#manual-installation-advanced) for users who want to understand or customize each step.
...
The script detects Debian automatically and handles the Debian-specific pieces (snapd installation, Debian Docker repositories) that differ from Ubuntu.
...
3. **Dependency installation**: installs snapd (not included on Debian by default), Docker CE with the Compose v2 plugin from Debian's Docker repository, plus Certbot and the AWS CLI via snap.
4. **Firewall (asks for confirmation)**: installs UFW if missing (common on minimal Debian installs), enables it with default-deny incoming, and opens **only** ports 22, 25, 80, 443, 465, 993, and 995. Internal services are fronted by the nginx SNI router on port 443 and are not exposed.
...
Install Docker CE with the Compose v2 plugin on Debian:
...
> **Note**: The `docker-compose-plugin` package provides Compose v2 (`docker compose`). The legacy standalone `docker-compose` v1 binary is no longer needed and should not be installed.
...
* **Snapd**: Not installed by default on Debian, requires manual installation (the setup script handles this automatically)
* **Docker**: Uses Debian-specific repositories and GPG keys; Compose v2 comes from the `docker-compose-plugin` package
* **UFW**: May not be included in minimal Debian installations (the setup script installs it when missing)
...
6. **Check certificate expiration**: automatic for certbot-managed methods; watch `/var/log/cert-expiry.log` if you used manual DNS certificates
...
Compose v2 ships as a Docker CLI plugin, so there is no separate `docker-compose` binary to install:

```bash
# Verify the plugin is installed
docker compose version

# If missing, install it from Docker's repository
...
8. **Delete CREDENTIALS.txt** once your admin account exists. It's only needed for first login
...
6. Monitor your system regularly and keep snapd packages updated

The main differences from Ubuntu are the snapd installation and Docker repository configuration. The setup script handles both automatically. Once these are properly set up, the Forward Email application behaves identically on both systems.
