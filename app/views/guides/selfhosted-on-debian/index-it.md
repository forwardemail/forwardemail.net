...
ufw allow 587/tcp   # Invio SMTP (STARTTLS)
...
# nginx (il servizio sni-router) termina il TLS sulla 443 e fa da proxy verso
# l'app web sulla 3000 (vedi docker-compose-self-hosted.yml e nginx.conf).
# Lascia WEB_PORT a 3000; impostarla su 443 va in conflitto con nginx sulla
# rete dell'host e rende irraggiungibile l'interfaccia web.
...
# Aggiungi il cron job di backup di Redis (viene eseguito ogni giorno alle 00:30,
# sfalsato di 30 minuti dopo il backup di MongoDB affinché i due caricamenti non si contendano la stessa banda)
