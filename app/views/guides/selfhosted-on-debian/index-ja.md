...
ufw allow 587/tcp   # SMTP サブミッション (STARTTLS)
...
# nginx（sni-router サービス）は 443 で TLS を終端し、3000 で動作する web アプリに
# プロキシします（docker-compose-self-hosted.yml と nginx.conf を参照）。WEB_PORT は
# 3000 のままにしてください。443 に設定するとホストネットワーク上で nginx と競合し、
# web インターフェースにアクセスできなくなります。
...
# Redis バックアップの cron ジョブを追加（毎日 00:30 に実行し、2 つのアップロードが同じ帯域を
# 奪い合わないよう MongoDB バックアップの 30 分後にずらしています）
