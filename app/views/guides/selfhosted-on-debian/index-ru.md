...
ufw allow 587/tcp   # Отправка SMTP (STARTTLS)
...
# nginx (служба sni-router) завершает TLS на 443 и проксирует на веб-приложение
# на 3000 (см. docker-compose-self-hosted.yml и nginx.conf). Оставьте WEB_PORT
# равным 3000; установка 443 конфликтует с nginx в сети хоста и делает
# веб-интерфейс недоступным.
...
# Добавить cron-задачу резервного копирования Redis (запускается ежедневно в 00:30,
# со сдвигом на 30 минут после копирования MongoDB, чтобы обе загрузки не конкурировали за одну полосу пропускания)
