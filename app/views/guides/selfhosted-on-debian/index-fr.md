...
ufw allow 587/tcp   # Soumission SMTP (STARTTLS)
...
# nginx (le service sni-router) termine TLS sur le 443 et fait office de proxy
# vers l'application web sur le 3000 (voir docker-compose-self-hosted.yml et
# nginx.conf). Laissez WEB_PORT à 3000 ; le régler sur 443 entre en conflit avec
# nginx sur le réseau de l'hôte et rend l'interface web inaccessible.
...
# Ajouter la tâche cron de sauvegarde Redis (s'exécute chaque jour à 00h30,
# décalée de 30 minutes après la sauvegarde MongoDB afin que les deux envois ne se disputent pas la même bande passante)
