...
* **受控的 Swap 使用**: 应用程序和其他非数据库主机保持无 Swap 状态，并对其进行规模调整和监控，以确保正常工作负载保留在 RAM 中。MongoDB 和 Redis 主机使用仅限 root 访问的 Swap 文件，并设置 `vm.swappiness=1`，严格作为内存不足时的安全网，而非正常容量。数据库的 Swap 活动会受到监控和调查。
