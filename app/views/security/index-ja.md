...
* **制御されたスワップの使用**: アプリケーションおよびその他の非データベースホストはスワップフリーのままであり、通常のワークロードがRAMに収まるようにサイジングおよび監視されます。MongoDBおよびRedisホストは、通常の容量としてではなく、メモリ不足時のセーフティネットとして厳密に`vm.swappiness=1`のroot専用スワップファイルを使用します。データベースのスワップアクティビティは監視および調査されます。
