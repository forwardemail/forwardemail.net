...
* **제어된 스왑 사용**: 애플리케이션 및 기타 비 데이터베이스 호스트는 스왑 없이 유지되며 정상적인 워크로드가 RAM에 유지되도록 크기가 조정되고 모니터링됩니다. MongoDB 및 Redis 호스트는 `vm.swappiness=1`이 설정된 root 전용 스왑 파일을 정상적인 용량이 아닌 메모리 부족 안전망으로만 엄격하게 사용합니다. 데이터베이스 스왑 활동은 모니터링되고 조사됩니다.
