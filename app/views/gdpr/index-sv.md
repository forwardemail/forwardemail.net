# GDPR-efterlevnad {#gdpr-compliance}

<!-- <img loading="lazy" src="/img/articles/gdpr.webp" alt="Forward Email GDPR compliance" class="rounded-lg" /> -->


## Innehållsförteckning {#table-of-contents}

* [Förord](#foreword)
* [Internationell dataöverföring](#international-data-transfer)
* [GDPR-representant](#gdpr-representative)
  * [EU-representant](#eu-representative)
  * [UK-representant](#uk-representative)


## Förord {#foreword}

Forward Email är ett integritetsfokuserat företag som är engagerat i att skydda dina uppgifter.

[Allmänna dataskyddsförordningen](https://en.wikipedia.org/wiki/General_Data_Protection_Regulation) ("GDPR") reglerar överföring av data utanför EU och EES, och dess mål är att stärka din kontroll och dina rättigheter över din personliga information. Lyckligtvis säljer vi inte någon data som vi behandlar, lagrar och/eller överför – och vi använder inte sådan data för något annat syfte än att tillhandahålla våra tjänster till dig.

När som helst kan du ändra din personliga information under [Mitt konto → Profil](/my-account/profile) och till och med radera hela ditt konto och all relaterad information under [Mitt konto → Säkerhet](/my-account/security). Eftersom **vi är 100 % öppen källkod** på <https://github.com/forwardemail> – kan vår källkod också granskas för efterlevnad av integritetslagar när som helst.

Vårt [Databehandlingsavtal](/dpa) beskriver vår efterlevnad av GDPR samt EU-US och Swiss-US Privacy Shield-ramverken, med avseende på integriteten för data som behandlas, lagras och överförs.

Genom att godkänna våra [Användarvillkor](/terms) accepterar du automatiskt vårt DPA och behöver inte skriva under något separat dokument. Vi kommer att göra vårt bästa för att skydda all din personliga data i enlighet med vår [Integritetspolicy](/privacy) – **oavsett var** den behandlas, lagras och överförs. Du kan också lära dig mer genom att läsa våra [Användarvillkor](/terms) och [Integritetspolicy](/privacy).


## Internationell dataöverföring {#international-data-transfer}

Eftersom vi behandlar, lagrar och överför data internationellt (t.ex. ett e-postmeddelande som skickas från ett land till ett annat) – är det viktigt att vi respekterar tillämpliga lokala integritetslagar.

Nedan listas våra leverantörer som vi använder för att överföra data internationellt samt länkar till deras dedikerade sidor angående deras GDPR-efterlevnad.

| Leverantör                               | [Data Privacy Framework Program](https://www.dataprivacyframework.gov/) ("DPF") Certifierad | GDPR-efterlevnadssida                                                                     |
| ---------------------------------------- | :-----------------------------------------------------------------------------------------: | ------------------------------------------------------------------------------------------ |
| [Cloudflare](https://cloudflare.com)     |                                   :white_check_mark: Ja                                   | <https://www.cloudflare.com/trust-hub/gdpr/>                                               |
| [DataPacket](https://www.datapacket.com/) |                                           :x: Nej                                          | <https://www.datapacket.com/privacy-policy>                                                |
| [Digital Ocean](https://digitalocean.com) |                                           :x: Nej                                          | <https://www.digitalocean.com/legal/gdpr>                                                  |
| [GitHub](https://github.com)             |                                   :white_check_mark: Ja                                   | <https://docs.github.com/en/site-policy/privacy-policies/github-data-protection-agreement> |
| [Vultr](https://www.vultr.com)           |                                           :x: Nej                                          | <https://www.vultr.com/legal/eea-gdpr-privacy/>                                            |
| [Stripe](https://stripe.com/)            |                                   :white_check_mark: Ja                                   | <https://stripe.com/legal/privacy-center>                                                  |
| [PayPal](https://www.paypal.com/us/home) |                                           :x: Nej                                          | <https://www.paypal.com/uk/legalhub/privacy-full>                                          |
## GDPR-representant {#gdpr-representative}

I enlighet med [Artikel 27 i GDPR](https://gdpr-info.eu/art-27-gdpr/) har vi medlemsstatsrepresentanter som anges nedan:

### EU-representant {#eu-representative}

<blockquote class="notranslate">Osano International Compliance Services Limited<br />ATTN: LFHC<br />3 Dublin Landings<br />North Wall Quay<br />Dublin 1<br />D01C4E0</blockquote>

### UK-representant {#uk-representative}

<blockquote class="notranslate">Osano UK Compliance LTD<br />ATTN: LFHC<br />42-46 Fountain Street<br />Belfast<br />Antrim<br />BT1 - 5EF</blockquote>
