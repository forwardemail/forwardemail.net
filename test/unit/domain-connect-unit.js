...
//
// Domain Connect unit tests (no web server or database required)
// Tests for:
//   - Domain Connect template JSON structure and content
//   - buildApplyUrl signing logic
//   - nsProviders Domain Connect configuration
//   - config.domainConnect section
//
...
test('Domain Connect template logoUrl uses logo-square.svg', (t) => {
  const template = require('../../assets/.well-known/domain-connect/forwardemail.net.email.json');
  t.is(
    template.logoUrl,
    'https://forwardemail.net/img/logo-square.svg',
    'logoUrl should use logo-square.svg'
  );
});
...
test('Domain Connect template has return-path CNAME record', (t) => {
  const template = require('../../assets/.well-known/domain-connect/forwardemail.net.email.json');
  const cnameRecord = template.records.find(
    (r) => r.type === 'CNAME' && r.host === 'fe-bounces'
  );
