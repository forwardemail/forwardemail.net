...
test('constructor honors DATABASE_MAP_MAX_SIZE env override', (t) => {
  // envStub sets DATABASE_MAP_MAX_SIZE = '500'
  const map = new DatabaseLRUMap();
  t.is(map.maxSize, 500);
  map.destroy();
});

test('constructor falls back to lax defaults when env is unset', (t) => {
  // Temporarily clear the env override to exercise the hard-coded lax defaults.
  const previous = envStub.DATABASE_MAP_MAX_SIZE;
  delete envStub.DATABASE_MAP_MAX_SIZE;
  try {
    const map = new DatabaseLRUMap();
    // Lax defaults restored to match historical (pre-aggressive) behavior.
    t.is(map.maxSize, 3000);
    t.is(map.activeIdleTTL, ms('10m'));
    t.is(map.inactiveIdleTTL, ms('5m'));
    t.is(map.sweepInterval, ms('60s'));
    map.destroy();
  } finally {
    envStub.DATABASE_MAP_MAX_SIZE = previous;
  }
});

test('constructor accepts an explicit sweepInterval override', (t) => {
  const map = new DatabaseLRUMap({ sweepInterval: 1234 });
  t.is(map.sweepInterval, 1234);
  map.destroy();
});
