...
// emails support@ for manual review instead of automatically banning
// users or adding to the denylist.  This avoids false positives from
// the Cloudflare Radar / Family DNS checks.
//
// The following users are EXCLUDED from flagging (but still reported
...
//   sending any review emails.  The digest email is still sent but its
//   subject and body clearly indicate it was a dry run.
//
 ...
 * @param   {Array}   ctx.bannedResults   Domains that were flagged for review
...
/**
 * Build a CSV string containing ALL flagged domains (flagged for review,
 * skipped, and review) with one row per entry.  This replaces the giant
 * inline HTML tables that could exceed email size limits.
 *
 * @param   {object}  opts
...
function buildDigestCsv(opts) {
  ...
  // Flagged domains (emailed to support@ for review) – one row per flagged user
...
function buildDigestHtml(opts) {
  ...
  const totalFlaggedDomains = bannedResults.length;
  const totalFlaggedUsers = bannedResults.reduce(
    (sum, r) => sum + r.users.length,
    0
  );
  const totalFlaggedAliases = bannedResults.reduce(
  ...
  //
  // Aggregate category counts across flagged results
  //
    ...
    html += `
      <div style="background: #fff3cd; border: 2px solid #ffc107; padding: 15px; border-radius: 5px; margin: 0 0 20px 0;">
        ...
        <p style="margin: 0; color: #856404;">
          This is a <strong>dry-run report</strong>.  No review emails were sent
          and no actions were taken.  The results below show what
          <em>would</em> have been flagged in a live run.
        </p>
      </div>
    `;
  }

  const flaggedLabel = dryRun
    ? 'Domains that would be flagged for review'
    : 'Domains flagged for review (emailed to support@)';
  const usersLabel = dryRun
    ? 'Users that would be flagged'
      ...
      <table border="1" cellpadding="8" cellspacing="0" style="width: 100%; border-collapse: collapse;">
        ...
        <tr style="background: #f9f9f9;">
          <td style="padding: 10px;"><strong>Aliases on ${
            dryRun ? 'flagged' : 'flagged'
          } domains</strong></td>
          <td style="padding: 10px;">${totalFlaggedAliases}</td>
        </tr>
  ...
  if (Object.keys(categoryCounts).length > 0) {
    html += `
      <h3>Category Breakdown (${dryRun ? 'Would-Be ' : ''}Flagged Domains)</h3>
      <table border="1" cellpadding="8" cellspacing="0" style="width: 100%; border-collapse: collapse;">
        <tr style="background: #f0f0f0;">
  ...
  html += `
    ...
    <p style="margin-top: 10px; background: #e8f5e9; padding: 10px; border-radius: 5px;">
      ...
      All flagged domains have been emailed to support@ for manual review.
      To take action, visit the <a href="${config.urls.web}/admin/users">admin panel</a>.
    </p>
  `;
...
(async () => {
  ...
  try {
    if (dryRun) {
      logger.info(
        'Running Cloudflare Family DNS & content categorisation check in DRY RUN mode – no review emails will be sent'
      );
    }
    ...
    logger.info(
      `${
        ...
        ctx.bannedResults.length
      } domains ${dryRun ? 'would be ' : ''}flagged for review, ${
    ...
    if (
      ...
      const totalFlaggedUsers = ctx.bannedResults.reduce(
        (s, r) => s + r.users.length,
        0
      );

      const subjectPrefix = dryRun ? '[DRY RUN] ' : '';
      const flaggedVerb = dryRun
      ...
      await emailHelper({
        ...
        message: {
          ...
          subject: `${subjectPrefix}Cloudflare Family DNS & Content Check: ${ctx.bannedResults.length} domains ${flaggedVerb} (${totalFlaggedUsers} users), ${ctx.skippedResults.length} protected users skipped, ${ctx.reviewResults.length} for review`,
          attachments: [
            {
